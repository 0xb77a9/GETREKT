...
do
    ...
    local function e(...)
        local f = {}
        for g = 1, select("#", ...) do f[select(g, ...)] = true end
        return f
    end
    local h = e(" ", "\t", "\r", "\n")
    local i = e(" ", "\t", "\r", "\n", "]", "}", ",")
    ...
    local function m(n, o, p, q)
        for g = o, #n do if p[n:sub(g, g)] ~= q then return g end end
        return #n + 1
    end
    ...
    local function v(w)
        ...
        elseif w <= 0x10ffff then
            return d(x(w / 262144) + 240, x(w % 262144 / 4096) + 128,
                     x(w % 4096 / 64) + 128, w % 64 + 128)
        end
    ...
    local function C(n, g)
        ...
        while D <= #n do
            ...
            elseif F == 92 then
                ...
                local G = n:sub(D, D)
                if G == "u" then
                    local H = n:match("^[dD][89aAbB]%x%x\\u%x%x%x%x", D + 1) or
                                  n:match("^%x%x%x%x", D + 1) or
                    ...
                    if not j[G] then
                        r(n, D - 1,
                          "invalid escape char '" .. G .. "' in string")
                    end
    ...
    local function I(n, g)
        ...
        local z = n:sub(g, F - 1)
        local w = tonumber(z)
        if not w then r(n, g, "invalid number '" .. z .. "'") end
        return w, F
    end
    local function J(n, g)
        local F = m(n, g, i)
        local K = n:sub(g, F - 1)
        if not k[K] then r(n, g, "invalid literal '" .. K .. "'") end
        return l[K], F
    end
    local function L(n, g)
        ...
        while 1 do
            ...
            g = g + 1
            if M == "]" then break end
            if M ~= "," then r(n, g, "expected ']' or ','") end
        end
        return f, g
    end
    ...
    function json.parse(n)
        ...
        o = m(n, o, h, true)
        if o <= #n then r(n, o, "trailing garbage") end
        return f
    end
    local b, d, c, e, f = string.byte, string.find, string.format, string.gsub,
                          string.match
    ...
    do
        ...
        if t or u then
            v = true
            if t and d(t, "%W") then t = "%" .. t end
            if u and d(u, "%W") then u = "%" .. u end
        end
        local w = function(x)
            if k < x and x < j then
                local y = tostring(x)
                if v then
                    if u then y = e(y, u, "") end
                    if t then y = e(y, t, ".") end
                end
                r[q] = y
                q = q + 1
                return
        ...
        local A = {
            ...
            ["\r"] = "\\r",
            ["\t"] = "\\t",
            __index = function(R, C) return c("\\u00%02X", b(C)) end
        }
        setmetatable(A, A)
        local function D(y)
            r[q] = '"'
            if d(y, l) then y = e(y, l, A) end
            r[q + 1] = y
            r[q + 2] = '"'
            q = q + 3
        end
...
function file.exist(filename)
    ...
    if tostring(file.read(filename)) == "nil" then file_exist = false end
    return file_exist
end

local path = "C:\\GETREKT\\lua\\"

function database.write(filename, key, value)
    local db_table = json.parse(file.read(path .. filename .. ".dat") and
                                    file.read(path .. filename .. ".dat") or
                                    "{}")
    db_table[key] = value
    file.write(path .. filename .. ".dat", json.encode_pretty(db_table))
end

function database.read(filename, key)
    local db_table = json.parse(file.read(path .. filename .. ".dat") and
                                    file.read(path .. filename .. ".dat") or
                                    "{}")
    return db_table[key]
