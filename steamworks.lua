-- Steamworks Lib
-- Made By Jaccko and BETA

local bit = require "bit"
local ffi = require "ffi"
local cast, cdef, typeof, sizeof, istype, ffi_string, ffi_copy = ffi.cast,
                                                                 ffi.cdef,
                                                                 ffi.typeof,
                                                                 ffi.sizeof,
                                                                 ffi.istype,
                                                                 ffi.string,
                                                                 ffi.copy
local string_len, string_sub, string_gsub, string_match, string_gmatch,
      string_lower, string_upper, string_format, string_reverse = string.len,
                                                                  string.sub,
                                                                  string.gsub,
                                                                  string.match,
                                                                  string.gmatch,
                                                                  ...
                                                                  string.format,
                                                                  string.reverse
local assert, tostring, tonumber, setmetatable, pairs, ipairs, pcall, xpcall,
      error, type = assert, tostring, tonumber, setmetatable, pairs, ipairs,
                    pcall, xpcall, error, type
local error_log, find_signature, timestamp = client.log, mem.find_pattern,
                                             client.timestamp
local table_insert, table_remove = table.insert, table.remove
local math_floor, math_min, math_max = math.floor, math.min, math.max
local band, bor, rshift, lshift, bswap = bit.band, bit.bor, bit.rshift,
                                         bit.lshift, bit.bswap
...
local BASE_STEAMID64 = "76561197960265728";
...
ffi.cdef "typedef void* HMODULE; HMODULE GetModuleHandleA(const char* lpModuleName);"

function vtable_entry(instance, index, type)
    return ffi.cast(type, (ffi.cast("void***", instance)[0])[index])
end

function vtable_thunk(index, typestring)
...
function get_steam_vtable_bind(module, interface, index, typestring)
	local module_handle = ffi.C.GetModuleHandleA(module)
	local instance = nil
	if (module_handle ~= nil) then 
		local steam_ = ffi.C.GetProcAddress(module_handle, interface)
		steam_ = ffi.cast("void*(__cdecl*)(void)",steam_)
		instance = steam_()
	end
	...
	error("Interface or Proc address is nil!")
	return	
end
...
local function find_sig(mdlname, pattern, typename, offset, deref_count)
    local raw_match = find_signature(mdlname, pattern) or
                          error("signature not found", 2)
    local match = cast("uintptr_t", raw_match)

    if offset ~= nil and offset ~= 0 then match = match + offset end

    if deref_count ~= nil then
        for i = 1, deref_count do
            match = cast("uintptr_t*", match)[0]
            if match == nil then return error("signature not found") end
        end
    end
...
local function pointer_key(p) return tonumber(cast(uintptr_t, p)) end

local function is_defined(typ) return (pcall(typeof, typ)) end

local function safe_cdef(name, typedef)
    if not is_defined(name) then cdef(typedef) end
end
...
local function struct_to_tbl(inst, fields, string_field_lookup)
    ...
    if inst ~= nil then
        for key, key_alt in pairs(fields) do
            ...
            if string_field_lookup ~= nil and string_field_lookup[key] and val ~=
                nil then val = ffi_string(inst[key]) end
...
local function error_log_prefix(text)
    return function(message) return error_log(text .. tostring(message)) end
end
...
do
    ...
    for key, char in pairs(TYPE_CHARS) do TYPE_CHARS_REVERSE[char] = key end
    ...
    local TYPE_INVALID, TYPE_INDIVIDUAL, TYPE_GAMESERVER, TYPE_ANONGAMESERVER,
          TYLE_MULTISEAT, TYPE_CLAN, TYPE_CHAT, TYPE_MAX = EAccountType.INVALID,
    ...
    local FRIEND_CODE_PATTERN =
        "^(" .. string.rep(FRIEND_CODE_PATTERN_CHAR, 5) .. ")%-(" ..
            string.rep(FRIEND_CODE_PATTERN_CHAR, 4) .. ")$"
    ...
    local function steamid_render_steam2(self, newer_format)
        ...
        return string_format("STEAM_%d:%d:%d", universe,
                             band(self.accountid, 1),
                             math_floor(self.accountid / 2))
    end
    ...
    local function steamid_render_steam3(self)
        ...
        local render_instance = self.type == TYPE_ANONGAMESERVER or self.type ==
                                    TYLE_MULTISEAT or
                                    (self.type == TYPE_INDIVIDUAL and
                                        self.instance ~= INSTANCE_DESKTOP)

        return string_format("[%s:%d:%d%s]", type_char, self.universe,
                             self.accountid,
                             render_instance and (":" .. self.instance) or "")
    end

    local function steamid_render_invite(self)
        if self.type ~= TYPE_INVALID and self.type ~= TYPE_INDIVIDUAL then
            return error(
                       "Cannot only render individual / invalid ID as Steam Invite")
        end

        local hex = string_gsub(string_format("%x", self.accountid), ".",
                                function(chr)
        ...
        if len > 3 then
            ...
            return string_format("%s-%s", string_sub(hex, 1, pos),
                                 string_sub(hex, pos + 1, -1))
        end
    ...
    local function steamid_is_valid(self)
        ...
        if self.universe <= 0 or self.universe > 4 then return false end

        if self.type == TYPE_INDIVIDUAL and
            (self.accountid == 0 or self.instance > 4) then return false end

        if self.type == TYPE_CLAN and
            (self.accountid == 0 or self.instance ~= 0) then return false end
    ...
    local function steamid_pack(self, big_endian)
        local res = ffi_string(cast("const char*",
                                    uint64_t_arr(1, self.steamid64)), 8)

        if big_endian then res = string_reverse(res) end
    ...
    local function steamid_mt_eq(self, other)
        ...
        elseif (t == "number" or t == "cdata") and
            (self.accountid == other or self.steamid64 == other) then
            return true
        end
    ...
    function to_steamid(steamid)
        ...
        if t == "string" then
            ...
            if id64_part ~= nil then
                return SteamID(76500000000000000 + tonumber(id64_part))
            end

            local id2_universe, id2_odd, id2_accountid =
                string_match(steamid, STEAMID2_PATTERN)
            if id2_universe ~= nil then
                local ret = SteamID()

                ret.universe = id2_universe == "0" and 1 or
                                   tonumber(id2_universe)
            ...
            local id3_typechar, id3_universe, id3_accountid, id3_instance =
                string_match(steamid, STEAMID3_SHORT_PATTERN)
            if id3_typechar ~= nil then
                ...
                id3_typechar, id3_universe, id3_accountid, id3_instance =
                    string_match(steamid, STEAMID3_LONG_PATTERN)
            end
            ...
            local fc_part1, fc_part2 =
                string_match(steamid, FRIEND_CODE_PATTERN)
...
local structs = {
    ...
    SteamNetworkingQuickConnectionStatus = typeof(
        "SteamNetworkingQuickConnectionStatus"),
    SteamNetworkingQuickConnectionStatus_arr = typeof(
        "SteamNetworkingQuickConnectionStatus [?]"),
    ...
    SteamNetworkingIdentityRender_arr = typeof(
        "SteamNetworkingIdentityRender [?]"),
    ...
    SteamDatagramGameCoordinatorServerLogin = typeof(
        "SteamDatagramGameCoordinatorServerLogin"),
    SteamDatagramGameCoordinatorServerLogin_arr = typeof(
        "SteamDatagramGameCoordinatorServerLogin [?]"),
    SteamNetAuthenticationStatus_t = typeof("SteamNetAuthenticationStatus_t"),
    SteamNetAuthenticationStatus_t_arr = typeof(
        "SteamNetAuthenticationStatus_t [?]"),
    SteamRelayNetworkStatus_t = typeof("SteamRelayNetworkStatus_t"),
    SteamRelayNetworkStatus_t_arr = typeof("SteamRelayNetworkStatus_t [?]"),
    SteamNetConnectionStatusChangedCallback_t = typeof(
        "SteamNetConnectionStatusChangedCallback_t"),
    SteamNetConnectionStatusChangedCallback_t_arr = typeof(
        "SteamNetConnectionStatusChangedCallback_t [?]"),
    SteamNetworkingMessagesSessionRequest_t = typeof(
        "SteamNetworkingMessagesSessionRequest_t"),
    SteamNetworkingMessagesSessionRequest_t_arr = typeof(
        "SteamNetworkingMessagesSessionRequest_t [?]"),
    SteamNetworkingMessagesSessionFailed_t = typeof(
        "SteamNetworkingMessagesSessionFailed_t"),
    SteamNetworkingMessagesSessionFailed_t_arr = typeof(
        "SteamNetworkingMessagesSessionFailed_t [?]")
}

for name, struct in pairs(structs) do M[name] = struct end
...
local client_context_interfaces = {
    "ISteamUser021", "ISteamFriends017", "ISteamUtils010",
    "ISteamMatchmaking009", "ISteamGameSearch001", "ISteamUserStats012",
    "ISteamApps008", "ISteamMatchmakingServers002", "ISteamNetworking006",
    "ISteamRemoteStorage014", "ISteamScreenshots003", "ISteamHTTP003",
    "ISteamController007", "ISteamUGC014", "ISteamAppList001", "ISteamMusic001",
    "ISteamMusicRemote001", "ISteamHTMLSurface005", "ISteamInventory003",
    "ISteamVideo002", "ISteamParentalSettings001", "ISteamInput001"
}

local client_context_raw = cast("uintptr_t**",
                                find_sig("client.dll",
                                         "B9 ? ? ? ? E8 ? ? ? ? 83 3D ? ? ? ? ? 0F 84",
                                         "uintptr_t", 1, 1))
...
local pGetModuleHandle_sig = find_signature("engine.dll",
                                                   "FF 15 ? ? ? ? 85 C0 74 0B") or
                                 error("Couldn't find signature #1")
local pGetProcAddress_sig = find_signature("engine.dll",
                                                  "FF 15 ? ? ? ? A3 ? ? ? ? EB 05") or
                                error("Couldn't find signature #2")

local jmp_ecx = find_signature("engine.dll", "FF E1")

local pGetProcAddress = cast("uint32_t**",
                             cast("uint32_t", pGetProcAddress_sig) + 2)[0][0]
local fnGetProcAddress = cast(
                             "uint32_t(__fastcall*)(unsigned int, unsigned int, uint32_t, const char*)",
                             jmp_ecx)

local pGetModuleHandle = cast("uint32_t**",
                              cast("uint32_t", pGetModuleHandle_sig) + 2)[0][0]
local fnGetModuleHandle = cast(
                              "uint32_t(__fastcall*)(unsigned int, unsigned int, const char*)",
                              jmp_ecx)
...
local function proc_bind(module_name, function_name, typedef)
    ...
    local proc_address = fnGetProcAddress(pGetProcAddress, 0, module_handle,
                                          function_name)
...
await_mt.__call = function(_, delay, fallback)
    if type(delay) == "function" then delay, fallback = fallback, delay end

    return setmetatable({tonumber(delay) or AWAIT_DEFAULT_DELAY, fallback},
                        await_mt)
end
...
local function is_valid_callback(callback)
    ...
    return t == "function" or
               (t == "table" and getmetatable(callback) == await_mt)
end
...
local callback_info = {
    [101] = {},
    [102] = {
        fields = {"m_eResult", "m_bStillRetrying"},
        fields_alt = {"result", "still_retrying"},
        types = {"int", "bool"}
    },
    [103] = {fields = {"m_eResult"}, fields_alt = {"result"}, types = {"int"}},
    [113] = {
        fields = {
            "m_uAppID", "m_unGameServerIP", "m_usGameServerPort", "m_bSecure",
            "m_uReason"
        },
        fields_alt = {
            "appid", "game_server_ip", "game_server_port", "secure", "reason"
        },
        types = {"uint32_t", "uint32_t", "uint16_t", "uint16_t", "uint32_t"}
    },
    [115] = {fields = {"m_bSecure"}, fields_alt = {"secure"}, types = {"bool"}},
    [117] = {
        fields = {"m_eFailureType"},
        fields_alt = {"failure_type"},
        types = {"uint8_t"}
    },
    [125] = {},
    [143] = {
        fields = {"m_SteamID", "m_eAuthSessionResponse", "m_OwnerSteamID"},
        fields_alt = {"steamid", "auth_session_response", "owner_steamid"},
        types = {"SteamID", "int", "SteamID"}
    },
    [152] = {
        fields = {"m_unAppID", "m_ulOrderID", "m_bAuthorized"},
        fields_alt = {"appid", "order_id", "authorized"},
        types = {"uint32_t", "uint64_t", "bool"}
    },
    [163] = {
        fields = {"m_hAuthTicket", "m_eResult"},
        fields_alt = {"auth_ticket", "result"},
        types = {"unsigned int", "int"}
    },
    [164] = {fields = {"m_szURL"}, fields_alt = {"url"}, types = {"char [256]"}},
    [201] = {
        fields = {"m_SteamID", "m_OwnerSteamID"},
        fields_alt = {"steamid", "owner_steamid"},
        types = {"SteamID", "SteamID"}
    },
    [202] = {
        fields = {"m_SteamID", "m_eDenyReason", "m_rgchOptionalText"},
        fields_alt = {"steamid", "deny_reason", "optional_text"},
        types = {"SteamID", "int", "char [128]"}
    },
    [203] = {
        fields = {"m_SteamID", "m_eDenyReason"},
        fields_alt = {"steamid", "deny_reason"},
        types = {"SteamID", "int"}
    },
    [206] = {
        fields = {"m_SteamID", "m_pchAchievement", "m_bUnlocked"},
        fields_alt = {"steamid", "achievement", "unlocked"},
        types = {"SteamID", "char [128]", "bool"}
    },
    [207] = {
        fields = {
            "m_eResult", "m_nRank", "m_unTotalConnects",
            "m_unTotalMinutesPlayed"
        },
        fields_alt = {
            "result", "rank", "total_connects", "total_minutes_played"
        },
        types = {"int", "int32_t", "uint32_t", "uint32_t"}
    },
    [208] = {
        fields = {"m_SteamIDUser", "m_SteamIDGroup", "m_bMember", "m_bOfficer"},
        fields_alt = {"steamid_user", "steamid_group", "member", "officer"},
        types = {"SteamID", "SteamID", "bool", "bool"}
    },
    [304] = {
        fields = {"m_ulSteamID", "m_nChangeFlags"},
        fields_alt = {"steamid", "change_flags"},
        types = {"SteamID", "int"}
    },
    [331] = {fields = {"m_bActive"}, fields_alt = {"active"}, types = {"bool"}},
    [332] = {
        fields = {"m_rgchServer", "m_rgchPassword"},
        fields_alt = {"server", "password"},
        types = {"char [64]", "char [64]"}
    },
    [333] = {
        fields = {"m_steamIDLobby", "m_steamIDFriend"},
        fields_alt = {"steamid_lobby", "steamid_friend"},
        types = {"SteamID", "SteamID"}
    },
    [334] = {
        fields = {"m_steamID", "m_iImage", "m_iWide", "m_iTall"},
        fields_alt = {"steamid", "image", "wide", "tall"},
        types = {"SteamID", "int", "int", "int"}
    },
    [336] = {
        fields = {"m_steamIDFriend", "m_nAppID"},
        fields_alt = {"steamid_friend", "appid"},
        types = {"SteamID", "unsigned int"}
    },
    [337] = {
        fields = {"m_steamIDFriend", "m_rgchConnect"},
        fields_alt = {"steamid_friend", "connect"},
        types = {"SteamID", "char [256]"}
    },
    [338] = {
        fields = {"m_steamIDClanChat", "m_steamIDUser", "m_iMessageID"},
        fields_alt = {"steamid_clan_chat", "steamid_user", "message_id"},
        types = {"SteamID", "SteamID", "int"}
    },
    [339] = {
        fields = {"m_steamIDClanChat", "m_steamIDUser"},
        fields_alt = {"steamid_clan_chat", "steamid_user"},
        types = {"SteamID", "SteamID"}
    },
    [340] = {
        fields = {
            "m_steamIDClanChat", "m_steamIDUser", "m_bKicked", "m_bDropped"
        },
        fields_alt = {"steamid_clan_chat", "steamid_user", "kicked", "dropped"},
        types = {"SteamID", "SteamID", "bool", "bool"}
    },
    [343] = {
        fields = {"m_steamIDUser", "m_iMessageID"},
        fields_alt = {"steamid_user", "message_id"},
        types = {"SteamID", "int"}
    },
    [348] = {},
    [349] = {
    ...
    [502] = {
        fields = {
            "m_nIP", "m_nQueryPort", "m_nConnPort", "m_nAppID", "m_nFlags",
            "m_bAdd", "m_unAccountId"
        },
        fields_alt = {
            "ip", "query_port", "conn_port", "appid", "flags", "add",
            "account_id"
        },
        types = {
            "uint32_t", "uint32_t", "uint32_t", "uint32_t", "uint32_t", "bool",
            "unsigned int"
        }
    },
    [503] = {
        fields = {"m_ulSteamIDUser", "m_ulSteamIDLobby", "m_ulGameID"},
        fields_alt = {"steamid_user", "steamid_lobby", "game_id"},
        types = {"SteamID", "SteamID", "uint64_t"}
    },
    [505] = {
        fields = {"m_ulSteamIDLobby", "m_ulSteamIDMember", "m_bSuccess"},
        fields_alt = {"steamid_lobby", "steamid_member", "success"},
        types = {"SteamID", "SteamID", "bool"}
    },
    [506] = {
        fields = {
            "m_ulSteamIDLobby", "m_ulSteamIDUserChanged",
            "m_ulSteamIDMakingChange", "m_rgfChatMemberStateChange"
        },
        fields_alt = {
            "steamid_lobby", "steamid_user_changed", "steamid_making_change",
            "chat_member_state_change"
        },
        types = {"SteamID", "SteamID", "SteamID", "uint32_t"}
    },
    [507] = {
        fields = {
            "m_ulSteamIDLobby", "m_ulSteamIDUser", "m_eChatEntryType",
            "m_iChatID"
        },
        fields_alt = {
            "steamid_lobby", "steamid_user", "chat_entry_type", "chat_id"
        },
        types = {"SteamID", "SteamID", "uint8_t", "uint32_t"}
    },
    [509] = {
        fields = {
            "m_ulSteamIDLobby", "m_ulSteamIDGameServer", "m_unIP", "m_usPort"
        },
        fields_alt = {"steamid_lobby", "steamid_game_server", "ip", "port"},
        types = {"SteamID", "SteamID", "uint32_t", "uint16_t"}
    },
    [512] = {
        fields = {
            "m_ulSteamIDLobby", "m_ulSteamIDAdmin", "m_bKickedDueToDisconnect"
        },
        fields_alt = {
            "steamid_lobby", "steamid_admin", "kicked_due_to_disconnect"
        },
        types = {"SteamID", "SteamID", "bool"}
    },
    [515] = {
        fields = {"m_bGameBootInviteExists", "m_steamIDLobby"},
        fields_alt = {"game_boot_invite_exists", "steamid_lobby"},
        types = {"bool", "SteamID"}
    },
    [516] = {fields = {"m_eResult"}, fields_alt = {"result"}, types = {"int"}},
    [701] = {},
    [702] = {
        fields = {"m_nMinutesBatteryLeft"},
        fields_alt = {"minutes_battery_left"},
        types = {"uint8_t"}
    },
    [703] = {
        fields = {"m_hAsyncCall", "m_iCallback", "m_cubParam"},
        fields_alt = {"async_call", "callback", "param"},
        types = {"uint64_t", "int", "uint32_t"}
    },
    [704] = {},
    [714] = {
        fields = {"m_bSubmitted", "m_unSubmittedText"},
        fields_alt = {"submitted", "submitted_text"},
        types = {"bool", "uint32_t"}
    },
    [1005] = {
        fields = {"m_nAppID"},
    ...
    [1008] = {
        fields = {"m_eResult", "m_unPackageRegistered"},
        fields_alt = {"result", "package_registered"},
        types = {"int", "uint32_t"}
    },
    [1014] = {},
    [1021] = {
        fields = {"m_eResult", "m_nAppID", "m_cchKeyLength", "m_rgchKey"},
        fields_alt = {"result", "appid", "key_length", "key"},
        types = {"int", "uint32_t", "uint32_t", "char [240]"}
    },
    [1030] = {
        fields = {
            "m_unAppID", "m_bIsOffline", "m_unSecondsAllowed",
            "m_unSecondsPlayed"
        },
        fields_alt = {
            "appid", "is_offline", "seconds_allowed", "seconds_played"
        },
        types = {"unsigned int", "bool", "uint32_t", "uint32_t"}
    },
    [1101] = {
        fields = {"m_nGameID", "m_eResult", "m_steamIDUser"},
        fields_alt = {"game_id", "result", "steamid_user"},
        types = {"uint64_t", "int", "SteamID"}
    },
    [1102] = {
        fields = {"m_nGameID", "m_eResult"},
        fields_alt = {"game_id", "result"},
        types = {"uint64_t", "int"}
    },
    [1103] = {
        fields = {
            "m_nGameID", "m_bGroupAchievement", "m_rgchAchievementName",
            "m_nCurProgress", "m_nMaxProgress"
        },
        fields_alt = {
            "game_id", "group_achievement", "achievement_name", "cur_progress",
            "max_progress"
        },
        types = {"uint64_t", "bool", "char [128]", "uint32_t", "uint32_t"}
    },
    [1108] = {
        fields = {"m_steamIDUser"},
        fields_alt = {"steamid_user"},
        types = {"SteamID"}
    },
    [1109] = {
        fields = {
            "m_nGameID", "m_rgchAchievementName", "m_bAchieved", "m_nIconHandle"
        },
        fields_alt = {"game_id", "achievement_name", "achieved", "icon_handle"},
        types = {"uint64_t", "char [128]", "bool", "int"}
    },
    [1112] = {
        fields = {"m_nGameID", "m_eResult", "m_ulRequiredDiskSpace"},
        fields_alt = {"game_id", "result", "required_disk_space"},
        types = {"uint64_t", "int", "uint64_t"}
    },
    [1201] = {
        fields = {
            "m_hSocket", "m_hListenSocket", "m_steamIDRemote",
            "m_eSNetSocketState"
        },
        fields_alt = {
            "socket", "listen_socket", "steamid_remote", "snet_socket_state"
        },
        types = {"unsigned int", "unsigned int", "SteamID", "int"}
    },
    [1202] = {
        fields = {"m_steamIDRemote"},
        fields_alt = {"steamid_remote"},
        types = {"SteamID"}
    },
    [1203] = {
        fields = {"m_steamIDRemote", "m_eP2PSessionError"},
        fields_alt = {"steamid_remote", "p2p_session_error"},
        types = {"SteamID", "uint8_t"}
    },
    [1221] = {
        fields = {"m_hConn", "m_info", "m_eOldState"},
        fields_alt = {"conn", "info", "old_state"},
        types = {"unsigned int", "SteamNetConnectionInfo_t", "int"}
    },
    [1222] = {
        fields = {"m_eAvail", "m_debugMsg"},
        fields_alt = {"avail", "debug_msg"},
        types = {"int", "char [256]"}
    },
    [1251] = {
        fields = {"m_identityRemote"},
        fields_alt = {"identity_remote"},
        types = {"SteamNetworkingIdentity"}
    },
    [1252] = {
        fields = {"m_info"},
        fields_alt = {"info"},
        types = {"SteamNetConnectionInfo_t"}
    },
    [1281] = {
        fields = {
            "m_eAvail", "m_bPingMeasurementInProgress", "m_eAvailNetworkConfig",
            "m_eAvailAnyRelay", "m_debugMsg"
        },
        fields_alt = {
            "avail", "ping_measurement_in_progress", "avail_network_config",
            "avail_any_relay", "debug_msg"
        },
        types = {"int", "int", "int", "int", "char [256]"}
    },
    [1301] = {
        fields = {"m_nAppID", "m_eResult", "m_unNumDownloads"},
        fields_alt = {"appid", "result", "num_downloads"},
        types = {"unsigned int", "int", "int"}
    },
    [1302] = {
        fields = {"m_nAppID", "m_eResult", "m_unNumUploads"},
        fields_alt = {"appid", "result", "num_uploads"},
        types = {"unsigned int", "int", "int"}
    },
    [1303] = {
        fields = {
            "m_rgchCurrentFile", "m_nAppID", "m_uBytesTransferredThisChunk",
            "m_dAppPercentComplete", "m_bUploading"
        },
        fields_alt = {
            "current_file", "appid", "bytes_transferred_this_chunk",
            "app_percent_complete", "uploading"
        },
        types = {"char [260]", "unsigned int", "uint32_t", "double", "bool"}
    },
    [1305] = {
        fields = {"m_nAppID", "m_eResult"},
        fields_alt = {"appid", "result"},
        types = {"unsigned int", "int"}
    },
    [1309] = {
        fields = {
            "m_eResult", "m_nPublishedFileId",
        ...
        types = {"int", "uint64_t", "bool"}
    },
    [1321] = {
        fields = {"m_nPublishedFileId", "m_nAppID"},
        fields_alt = {"published_file_id", "appid"},
        types = {"uint64_t", "unsigned int"}
    },
    [1322] = {
        fields = {"m_nPublishedFileId", "m_nAppID"},
        fields_alt = {"published_file_id", "appid"},
    ...
    [1325] = {
        fields = {"m_eResult", "m_nPublishedFileId", "m_eVote"},
        fields_alt = {"result", "published_file_id", "vote"},
        types = {"int", "uint64_t", "int"}
    },
    [1326] = {
        fields = {
            "m_eResult", "m_nResultsReturned", "m_nTotalResultCount",
            "m_rgPublishedFileId"
        },
        fields_alt = {
            "result", "results_returned", "total_result_count",
            "published_file_id"
        },
        types = {"int", "int32_t", "int32_t", "uint64_t [50]"}
    },
    [1330] = {
        fields = {"m_nPublishedFileId", "m_nAppID", "m_ulUnused"},
        fields_alt = {"published_file_id", "appid", "unused"},
        types = {"uint64_t", "unsigned int", "uint64_t"}
    },
    [2101] = {
        fields = {
            "m_hRequest", "m_ulContextValue", "m_bRequestSuccessful",
            "m_eStatusCode", "m_unBodySize"
        },
        fields_alt = {
            "request", "context_value", "request_successful", "status_code",
            "body_size"
        },
        types = {"unsigned int", "uint64_t", "bool", "int", "uint32_t"}
    },
    [2102] = {
        fields = {"m_hRequest", "m_ulContextValue"},
        fields_alt = {"request", "context_value"},
        types = {"unsigned int", "uint64_t"}
    },
    [2103] = {
        fields = {
            "m_hRequest", "m_ulContextValue", "m_cOffset", "m_cBytesReceived"
        },
        fields_alt = {"request", "context_value", "offset", "bytes_received"},
        types = {"unsigned int", "uint64_t", "uint32_t", "uint32_t"}
    },
    [2301] = {
        fields = {"m_hLocal", "m_eResult"},
        fields_alt = {"local", "result"},
        types = {"unsigned int", "int"}
    },
    [2302] = {},
    [3405] = {
        fields = {"m_unAppID", "m_nPublishedFileId"},
        fields_alt = {"appid", "published_file_id"},
        types = {"unsigned int", "uint64_t"}
    },
    [3406] = {
        fields = {"m_unAppID", "m_nPublishedFileId", "m_eResult"},
        fields_alt = {"appid", "published_file_id", "result"},
        types = {"unsigned int", "uint64_t", "int"}
    },
    [3901] = {
        fields = {"m_nAppID"},
        fields_alt = {"appid"},
    ...
    [4001] = {},
    [4002] = {
        fields = {"m_flNewVolume"},
        fields_alt = {"new_volume"},
        types = {"float"}
    },
    [4011] = {
        fields = {"m_flNewVolume"},
    ...
    [4114] = {
        fields = {"m_nPlayingRepeatStatus"},
        fields_alt = {"playing_repeat_status"},
        types = {"int"}
    },
    [4502] = {
        fields = {
            "unBrowserHandle", "pBGRA", "unWide", "unTall", "unUpdateX",
            "unUpdateY", "unUpdateWide", "unUpdateTall", "unScrollX",
            "unScrollY", "flPageScale", "unPageSerial"
        },
        fields_alt = {
            "browser_handle", "bgra", "wide", "tall", "update_x", "update_y",
            "update_wide", "update_tall", "scroll_x", "scroll_y", "page_scale",
            "page_serial"
        },
        types = {
            "unsigned int", "const char *", "uint32_t", "uint32_t", "uint32_t",
    ...
    [4503] = {
        fields = {
            "unBrowserHandle", "pchURL", "pchTarget", "pchPostData",
            "bIsRedirect"
        },
        fields_alt = {
            "browser_handle", "url", "target", "post_data", "is_redirect"
        },
        types = {
            "unsigned int", "const char *", "const char *", "const char *",
            "bool"
        },
        string_fields = {"pchURL", "pchTarget", "pchPostData"}
    },
    [4504] = {
        fields = {"unBrowserHandle"},
        fields_alt = {"browser_handle"},
        types = {"unsigned int"}
    },
    [4505] = {
        fields = {
            "unBrowserHandle", "pchURL", "pchPostData", "bIsRedirect",
            "pchPageTitle", "bNewNavigation"
        },
        fields_alt = {
            "browser_handle", "url", "post_data", "is_redirect", "page_title",
            "new_navigation"
        },
        types = {
            "unsigned int", "const char *", "const char *", "bool",
            "const char *", "bool"
        },
        string_fields = {"pchURL", "pchPostData", "pchPageTitle"}
    },
    [4506] = {
        fields = {"unBrowserHandle", "pchURL", "pchPageTitle"},
        fields_alt = {"browser_handle", "url", "page_title"},
        types = {"unsigned int", "const char *", "const char *"},
        string_fields = {"pchURL", "pchPageTitle"}
    },
    [4507] = {
        fields = {"unBrowserHandle", "pchURL"},
        fields_alt = {"browser_handle", "url"},
        types = {"unsigned int", "const char *"},
        string_fields = {"pchURL"}
    },
    [4508] = {
        fields = {"unBrowserHandle", "pchTitle"},
        fields_alt = {"browser_handle", "title"},
        types = {"unsigned int", "const char *"},
        string_fields = {"pchTitle"}
    },
    [4509] = {
        fields = {"unBrowserHandle", "unResults", "unCurrentMatch"},
        fields_alt = {"browser_handle", "results", "current_match"},
        types = {"unsigned int", "uint32_t", "uint32_t"}
    },
    [4510] = {
        fields = {"unBrowserHandle", "bCanGoBack", "bCanGoForward"},
        fields_alt = {"browser_handle", "can_go_back", "can_go_forward"},
        types = {"unsigned int", "bool", "bool"}
    },
    [4511] = {
        fields = {
            "unBrowserHandle", "unScrollMax", "unScrollCurrent", "flPageScale",
            "bVisible", "unPageSize"
        },
        fields_alt = {
            "browser_handle", "scroll_max", "scroll_current", "page_scale",
            "visible", "page_size"
        },
        types = {
            "unsigned int", "uint32_t", "uint32_t", "float", "bool", "uint32_t"
        }
    },
    [4512] = {
        fields = {
    ...
    [4513] = {
        fields = {"unBrowserHandle", "x", "y", "pchURL", "bInput", "bLiveLink"},
        fields_alt = {"browser_handle", "x", "y", "url", "input", "live_link"},
        types = {
            "unsigned int", "uint32_t", "uint32_t", "const char *", "bool",
            "bool"
        },
        string_fields = {"pchURL"}
    },
    [4514] = {
        fields = {"unBrowserHandle", "pchMessage"},
        fields_alt = {"browser_handle", "message"},
        types = {"unsigned int", "const char *"},
        string_fields = {"pchMessage"}
    },
    [4515] = {
        fields = {"unBrowserHandle", "pchMessage"},
        fields_alt = {"browser_handle", "message"},
    ...
    [4516] = {
        fields = {"unBrowserHandle", "pchTitle", "pchInitialFile"},
        fields_alt = {"browser_handle", "title", "initial_file"},
        types = {"unsigned int", "const char *", "const char *"},
        string_fields = {"pchTitle", "pchInitialFile"}
    },
    [4521] = {
        fields = {
            "unBrowserHandle", "pchURL", "unX", "unY", "unWide", "unTall",
            "unNewWindow_BrowserHandle_IGNORE"
        },
        fields_alt = {
            "browser_handle", "url", "x", "y", "wide", "tall",
    ...
    [4522] = {
        fields = {"unBrowserHandle", "eMouseCursor"},
        fields_alt = {"browser_handle", "mouse_cursor"},
        types = {"unsigned int", "uint32_t"}
    },
    [4523] = {
        fields = {"unBrowserHandle", "pchMsg"},
        fields_alt = {"browser_handle", "msg"},
        types = {"unsigned int", "const char *"},
        string_fields = {"pchMsg"}
    },
    [4524] = {
        fields = {"unBrowserHandle", "pchMsg"},
        fields_alt = {"browser_handle", "msg"},
    ...
    [4527] = {
        fields = {"unBrowserHandle", "unOldBrowserHandle"},
        fields_alt = {"browser_handle", "old_browser_handle"},
        types = {"unsigned int", "unsigned int"}
    },
    [4611] = {
        fields = {"m_eResult", "m_unVideoAppID", "m_rgchURL"},
        fields_alt = {"result", "video_appid", "url"},
        types = {"int", "unsigned int", "char [256]"}
    },
    [4624] = {
        fields = {"m_eResult", "m_unVideoAppID"},
        fields_alt = {"result", "video_appid"},
        types = {"int", "unsigned int"}
    },
    [4700] = {
        fields = {"m_handle", "m_result"},
        fields_alt = {"handle", "result"},
        types = {"int", "int"}
    },
    ...
    [5201] = {
        fields = {
            "m_ullSearchID", "m_eResult", "m_lobbyID", "m_steamIDEndedSearch",
            "m_nSecondsRemainingEstimate", "m_cPlayersSearching"
        },
        fields_alt = {
            "search_id", "result", "lobby_id", "steamid_ended_search",
            "seconds_remaining_estimate", "players_searching"
        },
        types = {"uint64_t", "int", "SteamID", "SteamID", "int32_t", "int32_t"}
    },
    [5202] = {
        fields = {
            "m_ullSearchID", "m_eResult", "m_nCountPlayersInGame",
            "m_nCountAcceptedGame", "m_steamIDHost", "m_bFinalCallback"
        },
        fields_alt = {
            "search_id", "result", "count_players_in_game",
            "count_accepted_game", "steamid_host", "final_callback"
        },
        types = {"uint64_t", "int", "int32_t", "int32_t", "SteamID", "bool"}
    },
    [5211] = {
        fields = {"m_eResult", "m_ullSearchID"},
        fields_alt = {"result", "search_id"},
        types = {"int", "uint64_t"}
    },
    [5212] = {
        fields = {
            "m_eResult", "m_ullSearchID", "m_SteamIDPlayerFound",
            "m_SteamIDLobby", "m_ePlayerAcceptState", "m_nPlayerIndex",
            "m_nTotalPlayersFound", "m_nTotalPlayersAcceptedGame",
            "m_nSuggestedTeamIndex", "m_ullUniqueGameID"
        },
        fields_alt = {
            "result", "search_id", "steamid_player_found", "steamid_lobby",
            "player_accept_state", "player_index", "total_players_found",
            "total_players_accepted_game", "suggested_team_index",
            "unique_game_id"
        },
        types = {
            "int", "uint64_t", "SteamID", "SteamID", "int", "int32_t",
            "int32_t", "int32_t", "int32_t", "uint64_t"
        }
    },
    [5213] = {
        fields = {"m_eResult", "m_ullSearchID", "m_ullUniqueGameID"},
        fields_alt = {"result", "search_id", "unique_game_id"},
        types = {"int", "uint64_t", "uint64_t"}
    },
    [5214] = {
        fields = {"m_eResult", "ullUniqueGameID", "steamIDPlayer"},
        fields_alt = {"result", "unique_game_id", "steamid_player"},
        types = {"int", "uint64_t", "SteamID"}
    },
    [5215] = {
        fields = {"m_eResult", "ullUniqueGameID"},
        fields_alt = {"result", "unique_game_id"},
        types = {"int", "uint64_t"}
    },
    [5303] = {
        fields = {"m_ulBeaconID", "m_steamIDJoiner"},
        fields_alt = {"beacon_id", "steamid_joiner"},
        types = {"uint64_t", "SteamID"}
    },
    [5305] = {},
    [5306] = {},
    [5701] = {
        fields = {"m_unSessionID"},
        fields_alt = {"session_id"},
        types = {"unsigned int"}
    },
    [5702] = {
        fields = {"m_unSessionID"},
...
local native_GetAPICallFailureReason = vtable_entry(steamutils, 12,
                                                    "int(__thiscall*)(void*, SteamAPICall_t)")
local native_IsAPICallCompleted = vtable_entry(steamutils, 11,
                                               "bool(__thiscall*)(void*, SteamAPICall_t, bool*)")
...
local function callback_base_run_common(self, param, io_failure)
    if io_failure then
        io_failure = ESteamAPICallFailure[GetAPICallFailureReason(
                         self.api_call_handle)] or true
    end
    ...
    if handler ~= nil then
        ...
        if info ~= nil then
            if param ~= nil then param = cast(info.struct, param) end
            local data =
                struct_to_tbl(param, info.keys, info.string_keys_lookup)

            if io_failure ~= false then data.io_failure = io_failure end
...
local function callback_base_run1(self, param, io_failure, api_call_handle)
    if api_call_handle == self.api_call_handle then
        xpcall(callback_base_run_common, callback_base_error, self, param,
               io_failure)
    end
end
...
local function callback_base_get_size() return sizeof_callback_base end
...
pcall(ffi.metatype, callback_base,
      {__gc = call_result_cancel, __index = {cancel = call_result_cancel}})

local callback_base_run1_ct = cast(
                                  "void(__thiscall *)(struct SteamAPI_callback_base *, void *, bool, uint64_t)",
                                  callback_base_run1)
local callback_base_run2_ct = cast(
                                  "void(__thiscall *)(struct SteamAPI_callback_base *, void *)",
                                  callback_base_run2)
local callback_base_get_size_ct = cast(
                                      "int(__thiscall *)(struct SteamAPI_callback_base *)",
                                      callback_base_get_size)

SteamAPI_RegisterCallResult = find_sig("steam_api.dll",
                                       "55 8B EC 83 3D ? ? ? ? ? 7E 0D 68 ? ? ? ? FF 15 ? ? ? ? 5D C3 FF 75 10",
                                       "void(__cdecl*)(struct SteamAPI_callback_base *, uint64_t)")
SteamAPI_UnregisterCallResult = find_sig("steam_api.dll",
                                         "55 8B EC FF 75 10 FF 75 0C",
                                         "void(__cdecl*)(struct SteamAPI_callback_base *, uint64_t)")

SteamAPI_RegisterCallback = find_sig("steam_api.dll",
                                     "55 8B EC 83 3D ? ? ? ? ? 7E 0D 68 ? ? ? ? FF 15 ? ? ? ? 5D C3 C7 05",
                                     "void(__cdecl*)(struct SteamAPI_callback_base *, int)")
SteamAPI_UnregisterCallback = find_sig("steam_api.dll",
                                       "55 8B EC 83 EC 08 80 3D",
                                       "void(__cdecl*)(struct SteamAPI_callback_base *)")

local SteamAPI_RunCallbacks = find_sig("steam_api.dll", "32 C9 83 3D ? ? ? ? ?",
                                       "void(__cdecl*)(void)")
...
local function register_call_result(api_call_handle, handler, id, info)
    ...
    local await = (type(handler) == "table" and getmetatable(handler) ==
                      await_mt) and handler or nil
    if await then
        jit.off(true, true)
    ...
    if await then
        ...
        local timeout = math_min(999, math_max(0, tonumber(await[1]) or
                                                   AWAIT_DEFAULT_DELAY))
...
local function get_global_callback_handler(id, user_callbacks)
    ...
    for i, key in ipairs(fields) do
        ...
        if type_match ~= nil then
            struct_text =
                struct_text .. type_match .. " " .. key .. arr_match .. "; "
        else
    ...
    return function(param, io_failure)
        if param ~= nil then param = cast(struct, param) end
...
local function set_callback(id, callback)
    if type(id) == "string" and
        callback_name_lookup[string_lower(string_gsub(id, "_t$", ""))] ~= nil then
        id = callback_name_lookup[string_lower(string_gsub(id, "_t$", ""))]
    end

    if callback_info[id] == nil then return error("Invalid Steam callback") end
    ...
    if registered_user_callbacks[id] == nil then
        ...
        local key = pointer_key(instance)
        api_call_handlers[key] = get_global_callback_handler(id,
                                                             registered_user_callbacks[id])
        ...
        for _, user_callback in ipairs(registered_user_callbacks[id]) do
            if user_callback == callback then return false end
        end
    end
...
local function unset_callback(id, callback)
    ...
    if registered_user_callbacks[id] == nil then return false end
...
function M.await_callback(id, timeout, callback)
    ...
    if callback ~= nil then registered = set_callback(id, callback) end

    -- wait for callback to be called

    -- if the callback was successfully registered (so not already registered) then unregister it
    if registered then unset_callback(id, callback) end
end
...
local function to_enum(int_or_string, enum)
    ...
    if t == "number" or t == "string" then
        ...
        if e ~= nil then return t == "string" and e or int_or_string end
    end
end
...
local function ipv4_int_to_string(ip)
    ...
    return string_format("%d.%d.%d.%d", strbuf[3], strbuf[2], strbuf[1],
                         strbuf[0])
end
...
local function ipv4_string_to_int(ip)
    ...
    if a ~= nil then
        local buf = uint8_t_arr(4, tonumber(d), tonumber(c), tonumber(b),
                                tonumber(a))
...
-- local native_ConnectToGlobalUser = vtable_bind("steam_api.dll", "SteamClient", 2, "int(__thiscall*)(void*, int)")
-- local hSteamUser = native_ConnectToGlobalUser(hSteamPipe)

local imports_match = cast("char*",
                           find_signature("client.dll",
                                          "FF 15 ? ? ? ? 8B D8 FF 15") or
                               error("Invalid SteamAPI_GetHSteamUser signature"))

local SteamAPI_GetHSteamUser =
    cast("int(__cdecl***)()", imports_match + 2)[0][0]
local SteamAPI_GetHSteamPipe =
    cast("int(__cdecl***)()", imports_match + 10)[0][0]
...
index_funcs.ISteamUser = function()
    local this = get_steam_vtable_bind("steam_api.dll", "SteamClient", 5,
                             "void*(__thiscall*)(void*, int, int, const char *)")(
                     hSteamUser, hSteamPipe, "SteamUser021")

    local GetHSteamUser_native =
        vtable_entry(this, 0, "int(__thiscall*)(void*)")
    function ISteamUser.GetHSteamUser() return GetHSteamUser_native(this) end
    ISteamUser.get_hsteamuser = ISteamUser.GetHSteamUser

    local BLoggedOn_native = vtable_entry(this, 1, "bool(__thiscall*)(void*)")
    function ISteamUser.BLoggedOn() return BLoggedOn_native(this) end
    ISteamUser.logged_on = ISteamUser.BLoggedOn

    local GetSteamID_native = vtable_entry(this, 2,
                                           "void(__thiscall*)(void*, SteamID *)")
    ...
    local InitiateGameConnection_native =
        vtable_entry(this, 3,
                     "int(__thiscall*)(void*, void *, int, SteamID, uint32_t, uint16_t, bool)")
    function ISteamUser.InitiateGameConnection(pAuthBlob, cbMaxAuthBlob,
                                               steamIDGameServer, unIPServer,
                                               usPortServer, bSecure)
        steamIDGameServer = to_steamid_required(steamIDGameServer,
                                                "steamid_game_server is required")
        unIPServer = to_ip_int_required(unIPServer, "ip_server is required")
        return InitiateGameConnection_native(this, pAuthBlob, cbMaxAuthBlob,
                                             steamIDGameServer, unIPServer,
                                             usPortServer, bSecure)
    end
    ISteamUser.initiate_game_connection = ISteamUser.InitiateGameConnection

    local TerminateGameConnection_native =
        vtable_entry(this, 4, "void(__thiscall*)(void*, uint32_t, uint16_t)")
    ...
    local TrackAppUsageEvent_native = vtable_entry(this, 5,
                                                   "void(__thiscall*)(void*, uint64_t, int, const char *)")
    function ISteamUser.TrackAppUsageEvent(gameID, eAppUsageEvent, pchExtraInfo)
        return TrackAppUsageEvent_native(this, gameID, eAppUsageEvent,
                                         pchExtraInfo)
    end
    ISteamUser.track_app_usage_event = ISteamUser.TrackAppUsageEvent

    local GetUserDataFolder_native = vtable_entry(this, 6,
    ...
    local StartVoiceRecording_native = vtable_entry(this, 7,
    ...
    local StopVoiceRecording_native = vtable_entry(this, 8,
                                                   "void(__thiscall*)(void*)")
    ...
    local GetAvailableVoice_native = vtable_entry(this, 9,
                                                  "int(__thiscall*)(void*, uint32_t *, uint32_t *, uint32_t)")
    function ISteamUser.GetAvailableVoice(pcbUncompressed_Deprecated,
                                          nUncompressedVoiceDesiredSampleRate_Deprecated)
        local pcbCompressed_out = new_uint32_arr()
        local res = GetAvailableVoice_native(this, pcbCompressed_out,
    ...
    local GetVoice_native = vtable_entry(this, 10,
                                         "int(__thiscall*)(void*, bool, void *, uint32_t, uint32_t *, bool, void *, uint32_t, uint32_t *, uint32_t)")
    function ISteamUser.GetVoice(bWantCompressed, pDestBuffer, cbDestBufferSize,
        ...
        local res = GetVoice_native(this, bWantCompressed, pDestBuffer,
                                    cbDestBufferSize, nBytesWritten_out,
                                    bWantUncompressed_Deprecated,
                                    pUncompressedDestBuffer_Deprecated,
                                    cbUncompressedDestBufferSize_Deprecated,
                                    nUncompressBytesWritten_Deprecated,
    ...
    local DecompressVoice_native = vtable_entry(this, 11,
                                                "int(__thiscall*)(void*, const void *, uint32_t, void *, uint32_t, uint32_t *, uint32_t)")
    function ISteamUser.DecompressVoice(pCompressed, cbCompressed, pDestBuffer,
                                        cbDestBufferSize, nDesiredSampleRate)
        local nBytesWritten_out = new_uint32_arr()
        local res = DecompressVoice_native(this, pCompressed, cbCompressed,
                                           pDestBuffer, cbDestBufferSize,
                                           nBytesWritten_out, nDesiredSampleRate)

        return res, DEREF_GCSAFE(nBytesWritten_out)
    end
    ISteamUser.decompress_voice = ISteamUser.DecompressVoice

    local GetVoiceOptimalSampleRate_native =
        vtable_entry(this, 12, "uint32_t(__thiscall*)(void*)")
    function ISteamUser.GetVoiceOptimalSampleRate()
        return GetVoiceOptimalSampleRate_native(this)
    end
    ISteamUser.get_voice_optimal_sample_rate =
        ISteamUser.GetVoiceOptimalSampleRate

    local GetAuthSessionTicket_native = vtable_entry(this, 13,
                                                     "unsigned int(__thiscall*)(void*, void *, int, uint32_t *)")
    function ISteamUser.GetAuthSessionTicket(pTicket, cbMaxTicket)
        local pcbTicket_out = new_uint32_arr()
        local res = GetAuthSessionTicket_native(this, pTicket, cbMaxTicket,
                                                pcbTicket_out)
    ...
    local BeginAuthSession_native = vtable_entry(this, 14,
                                                 "int(__thiscall*)(void*, const void *, int, SteamID)")
    ...
    local EndAuthSession_native = vtable_entry(this, 15,
                                               "void(__thiscall*)(void*, SteamID)")
    ...
    local CancelAuthTicket_native = vtable_entry(this, 16,
    ...
    local UserHasLicenseForApp_native = vtable_entry(this, 17,
                                                     "int(__thiscall*)(void*, SteamID, unsigned int)")
    ...
    local BIsBehindNAT_native = vtable_entry(this, 18,
                                             "bool(__thiscall*)(void*)")
    function ISteamUser.BIsBehindNAT() return BIsBehindNAT_native(this) end
    ISteamUser.is_behind_nat = ISteamUser.BIsBehindNAT

    local AdvertiseGame_native = vtable_entry(this, 19,
                                              "void(__thiscall*)(void*, SteamID, uint32_t, uint16_t)")
    function ISteamUser.AdvertiseGame(steamIDGameServer, unIPServer,
                                      usPortServer)
        steamIDGameServer = to_steamid_required(steamIDGameServer,
                                                "steamid_game_server is required")
        unIPServer = to_ip_int_required(unIPServer, "ip_server is required")
        return AdvertiseGame_native(this, steamIDGameServer, unIPServer,
                                    usPortServer)
    end
    ISteamUser.advertise_game = ISteamUser.AdvertiseGame

    local RequestEncryptedAppTicket_native =
        vtable_entry(this, 20, "uint64_t(__thiscall*)(void*, void *, int)")
    ...
    function ISteamUser.RequestEncryptedAppTicket(pDataToInclude,
                                                  cbDataToInclude, callback)
        if callback ~= nil and not is_valid_callback(callback) then
            return error("Invalid callback, expected function or await")
        end

        local res = RequestEncryptedAppTicket_native(this, pDataToInclude,
                                                     cbDataToInclude)

        if callback ~= nil then
            res = register_call_result(res, callback, 154,
                                       RequestEncryptedAppTicket_info)
        end

        return res
    end
    ISteamUser.request_encrypted_app_ticket =
        ISteamUser.RequestEncryptedAppTicket

    local GetEncryptedAppTicket_native =
        vtable_entry(this, 21,
                     "bool(__thiscall*)(void*, void *, int, uint32_t *)")
    function ISteamUser.GetEncryptedAppTicket(pTicket, cbMaxTicket)
        local pcbTicket_out = new_uint32_arr()
        local res = GetEncryptedAppTicket_native(this, pTicket, cbMaxTicket,
                                                 pcbTicket_out)
    ...
    local GetGameBadgeLevel_native = vtable_entry(this, 22,
                                                  "int(__thiscall*)(void*, int, bool)")
    ...
    local GetPlayerSteamLevel_native = vtable_entry(this, 23,
                                                    "int(__thiscall*)(void*)")
    ...
    local RequestStoreAuthURL_native = vtable_entry(this, 24,
                                                    "uint64_t(__thiscall*)(void*, const char *)")
    ...
    function ISteamUser.RequestStoreAuthURL(pchRedirectURL, callback)
        ...
        if callback ~= nil then
            res = register_call_result(res, callback, 165,
                                       RequestStoreAuthURL_info)
        end
    ...
    local BIsPhoneVerified_native = vtable_entry(this, 25,
                                                 "bool(__thiscall*)(void*)")
    ...
    local BIsTwoFactorEnabled_native = vtable_entry(this, 26,
                                                    "bool(__thiscall*)(void*)")
    ...
    local BIsPhoneIdentifying_native = vtable_entry(this, 27,
                                                    "bool(__thiscall*)(void*)")
    ...
    local BIsPhoneRequiringVerification_native =
        vtable_entry(this, 28, "bool(__thiscall*)(void*)")
    function ISteamUser.BIsPhoneRequiringVerification()
        return BIsPhoneRequiringVerification_native(this)
    end
    ISteamUser.is_phone_requiring_verification =
        ISteamUser.BIsPhoneRequiringVerification

    local GetMarketEligibility_native = vtable_entry(this, 29,
                                                     "uint64_t(__thiscall*)(void*)")
    ...
    function ISteamUser.GetMarketEligibility(callback)
        ...
        if callback ~= nil then
            res = register_call_result(res, callback, 166,
                                       GetMarketEligibility_info)
        end
    ...
    local GetDurationControl_native = vtable_entry(this, 30,
                                                   "uint64_t(__thiscall*)(void*)")
    ...
    function ISteamUser.GetDurationControl(callback)
        ...
        if callback ~= nil then
            res = register_call_result(res, callback, 167,
                                       GetDurationControl_info)
        end
    ...
    local BSetDurationControlOnlineState_native =
        vtable_entry(this, 31, "bool(__thiscall*)(void*, int)")
    function ISteamUser.BSetDurationControlOnlineState(eNewState)
        eNewState = to_enum_required(eNewState,
                                     enums.EDurationControlOnlineState,
                                     "new_state is required")
        return BSetDurationControlOnlineState_native(this, eNewState)
    end
    ISteamUser.set_duration_control_online_state =
        ISteamUser.BSetDurationControlOnlineState
...
index_funcs.ISteamFriends = function()
    local this = get_steam_vtable_bind("steam_api.dll", "SteamClient", 8,
                             "void*(__thiscall*)(void*, int, int, const char *)")(
                     hSteamUser, hSteamPipe, "SteamFriends017")

    local GetPersonaName_native = vtable_entry(this, 0,
    ...
    local SetPersonaName_native = vtable_entry(this, 1,
                                               "uint64_t(__thiscall*)(void*, const char *)")
    ...
    local GetPersonaState_native = vtable_entry(this, 2,
                                                "int(__thiscall*)(void*)")
    ...
    local GetFriendCount_native = vtable_entry(this, 3,
                                               "int(__thiscall*)(void*, int)")
    function ISteamFriends.GetFriendCount(iFriendFlags)
        iFriendFlags = to_enum_required(iFriendFlags, enums.EFriendFlags,
                                        "friend_flags is required")
        return GetFriendCount_native(this, iFriendFlags)
    end
    ISteamFriends.get_friend_count = ISteamFriends.GetFriendCount

    local GetFriendByIndex_native = vtable_entry(this, 4,
                                                 "void(__thiscall*)(void*, SteamID *, int, int)")
    ...
    local GetFriendRelationship_native =
        vtable_entry(this, 5, "int(__thiscall*)(void*, SteamID)")
    function ISteamFriends.GetFriendRelationship(steamIDFriend)
        steamIDFriend = to_steamid_required(steamIDFriend,
                                            "steamid_friend is required")
    ...
    local GetFriendPersonaState_native =
        vtable_entry(this, 6, "int(__thiscall*)(void*, SteamID)")
    ...
    local GetFriendPersonaName_native = vtable_entry(this, 7,
                                                     "const char *(__thiscall*)(void*, SteamID)")
    ...
    local GetFriendGamePlayed_native = vtable_entry(this, 8,
                                                    "bool(__thiscall*)(void*, SteamID, FriendGameInfo_t *)")
    function ISteamFriends.GetFriendGamePlayed(steamIDFriend)
        steamIDFriend = to_steamid_required(steamIDFriend,
                                            "steamid_friend is required")
        local pFriendGameInfo_out = new_FriendGameInfo_arr()
        local res = GetFriendGamePlayed_native(this, steamIDFriend,
                                               pFriendGameInfo_out)
    ...
    local GetFriendPersonaNameHistory_native =
        vtable_entry(this, 9, "const char *(__thiscall*)(void*, SteamID, int)")
    function ISteamFriends.GetFriendPersonaNameHistory(steamIDFriend,
                                                       iPersonaName)
        steamIDFriend = to_steamid_required(steamIDFriend,
                                            "steamid_friend is required")
        local res = GetFriendPersonaNameHistory_native(this, steamIDFriend,
                                                       iPersonaName)

        return res ~= nil and ffi_string(res) or nil
    end
    ISteamFriends.get_friend_persona_name_history =
        ISteamFriends.GetFriendPersonaNameHistory

    local GetFriendSteamLevel_native = vtable_entry(this, 10,
                                                    "int(__thiscall*)(void*, SteamID)")
    ...
    local GetPlayerNickname_native = vtable_entry(this, 11,
                                                  "const char *(__thiscall*)(void*, SteamID)")
    function ISteamFriends.GetPlayerNickname(steamIDPlayer)
        steamIDPlayer = to_steamid_required(steamIDPlayer,
                                            "steamid_player is required")
    ...
    local GetFriendsGroupCount_native = vtable_entry(this, 12,
                                                     "int(__thiscall*)(void*)")
    ...
    local GetFriendsGroupIDByIndex_native =
        vtable_entry(this, 13, "short(__thiscall*)(void*, int)")
    function ISteamFriends.GetFriendsGroupIDByIndex(iFG)
        return GetFriendsGroupIDByIndex_native(this, iFG)
    end
    ISteamFriends.get_friends_group_id_by_index =
        ISteamFriends.GetFriendsGroupIDByIndex

    local GetFriendsGroupName_native = vtable_entry(this, 14,
                                                    "const char *(__thiscall*)(void*, short)")
    ...
    local GetFriendsGroupMembersCount_native =
        vtable_entry(this, 15, "int(__thiscall*)(void*, short)")
    ...
    ISteamFriends.get_friends_group_members_count =
        ISteamFriends.GetFriendsGroupMembersCount

    local GetFriendsGroupMembersList_native =
        vtable_entry(this, 16, "void(__thiscall*)(void*, short, SteamID *, int)")
    function ISteamFriends.GetFriendsGroupMembersList(friendsGroupID,
                                                      pOutSteamIDMembers,
                                                      nMembersCount)
        return GetFriendsGroupMembersList_native(this, friendsGroupID,
                                                 pOutSteamIDMembers,
                                                 nMembersCount)
    end
    ISteamFriends.get_friends_group_members_list =
        ISteamFriends.GetFriendsGroupMembersList

    local HasFriend_native = vtable_entry(this, 17,
                                          "bool(__thiscall*)(void*, SteamID, int)")
    ...
    local GetClanCount_native =
        vtable_entry(this, 18, "int(__thiscall*)(void*)")
    function ISteamFriends.GetClanCount() return GetClanCount_native(this) end
    ISteamFriends.get_clan_count = ISteamFriends.GetClanCount

    local GetClanByIndex_native = vtable_entry(this, 19,
                                               "void(__thiscall*)(void*, SteamID *, int)")
    ...
    local GetClanName_native = vtable_entry(this, 20,
                                            "const char *(__thiscall*)(void*, SteamID)")
    function ISteamFriends.GetClanName(steamIDClan)
        steamIDClan = to_steamid_required(steamIDClan,
                                          "steamid_clan is required")
    ...
    local GetClanTag_native = vtable_entry(this, 21,
                                           "const char *(__thiscall*)(void*, SteamID)")
    ...
    local GetClanActivityCounts_native =
        vtable_entry(this, 22,
                     "bool(__thiscall*)(void*, SteamID, int *, int *, int *)")
    function ISteamFriends.GetClanActivityCounts(steamIDClan)
        ...
        local res = GetClanActivityCounts_native(this, steamIDClan,
                                                 pnOnline_out, pnInGame_out,
                                                 pnChatting_out)

        return res, DEREF_GCSAFE(pnOnline_out), DEREF_GCSAFE(pnInGame_out),
               DEREF_GCSAFE(pnChatting_out)
    end
    ISteamFriends.get_clan_activity_counts = ISteamFriends.GetClanActivityCounts

    local DownloadClanActivityCounts_native =
        vtable_entry(this, 23, "uint64_t(__thiscall*)(void*, SteamID *, int)")
    ...
    function ISteamFriends.DownloadClanActivityCounts(psteamIDClans,
                                                      cClansToRequest, callback)
        if callback ~= nil and not is_valid_callback(callback) then
            return error("Invalid callback, expected function or await")
        end

        local res = DownloadClanActivityCounts_native(this, psteamIDClans,
                                                      cClansToRequest)

        if callback ~= nil then
            res = register_call_result(res, callback, 341,
                                       DownloadClanActivityCounts_info)
        end

        return res
    end
    ISteamFriends.download_clan_activity_counts =
        ISteamFriends.DownloadClanActivityCounts

    local GetFriendCountFromSource_native =
        vtable_entry(this, 24, "int(__thiscall*)(void*, SteamID)")
    function ISteamFriends.GetFriendCountFromSource(steamIDSource)
        steamIDSource = to_steamid_required(steamIDSource,
                                            "steamid_source is required")
        return GetFriendCountFromSource_native(this, steamIDSource)
    end
    ISteamFriends.get_friend_count_from_source =
        ISteamFriends.GetFriendCountFromSource

    local GetFriendFromSourceByIndex_native =
        vtable_entry(this, 25,
                     "void(__thiscall*)(void*, SteamID *, SteamID, int)")
    function ISteamFriends.GetFriendFromSourceByIndex(steamIDSource, iFriend)
        steamIDSource = to_steamid_required(steamIDSource,
                                            "steamid_source is required")
        local CSteamID_out = new_SteamID_arr()
        GetFriendFromSourceByIndex_native(this, CSteamID_out, steamIDSource,
                                          iFriend)

        return DEREF_GCSAFE(CSteamID_out)
    end
    ISteamFriends.get_friend_from_source_by_index =
        ISteamFriends.GetFriendFromSourceByIndex

    local IsUserInSource_native = vtable_entry(this, 26,
                                               "bool(__thiscall*)(void*, SteamID, SteamID)")
    function ISteamFriends.IsUserInSource(steamIDUser, steamIDSource)
        steamIDUser = to_steamid_required(steamIDUser,
                                          "steamid_user is required")
        steamIDSource = to_steamid_required(steamIDSource,
                                            "steamid_source is required")
    ...
    local SetInGameVoiceSpeaking_native =
        vtable_entry(this, 27, "void(__thiscall*)(void*, SteamID, bool)")
    ...
    ISteamFriends.set_in_game_voice_speaking =
        ISteamFriends.SetInGameVoiceSpeaking

    local ActivateGameOverlay_native = vtable_entry(this, 28,
    ...
    local ActivateGameOverlayToUser_native =
        vtable_entry(this, 29, "void(__thiscall*)(void*, const char *, SteamID)")
    ...
    ISteamFriends.activate_game_overlay_to_user =
        ISteamFriends.ActivateGameOverlayToUser

    local ActivateGameOverlayToWebPage_native =
        vtable_entry(this, 30, "void(__thiscall*)(void*, const char *, int)")
    function ISteamFriends.ActivateGameOverlayToWebPage(pchURL, eMode)
        eMode = to_enum_required(eMode, enums.EActivateGameOverlayToWebPageMode,
                                 "mode is required")
        return ActivateGameOverlayToWebPage_native(this, pchURL, eMode)
    end
    ISteamFriends.activate_game_overlay_to_web_page =
        ISteamFriends.ActivateGameOverlayToWebPage

    local ActivateGameOverlayToStore_native =
        vtable_entry(this, 31, "void(__thiscall*)(void*, unsigned int, int)")
    function ISteamFriends.ActivateGameOverlayToStore(nAppID, eFlag)
        eFlag = to_enum_required(eFlag, enums.EOverlayToStoreFlag,
                                 "flag is required")
        return ActivateGameOverlayToStore_native(this, nAppID, eFlag)
    end
    ISteamFriends.activate_game_overlay_to_store =
        ISteamFriends.ActivateGameOverlayToStore

    local SetPlayedWith_native = vtable_entry(this, 32,
                                              "void(__thiscall*)(void*, SteamID)")
    function ISteamFriends.SetPlayedWith(steamIDUserPlayedWith)
        steamIDUserPlayedWith = to_steamid_required(steamIDUserPlayedWith,
                                                    "steamid_user_played_with is required")
        return SetPlayedWith_native(this, steamIDUserPlayedWith)
    end
    ISteamFriends.set_played_with = ISteamFriends.SetPlayedWith

    local ActivateGameOverlayInviteDialog_native =
        vtable_entry(this, 33, "void(__thiscall*)(void*, SteamID)")
    function ISteamFriends.ActivateGameOverlayInviteDialog(steamIDLobby)
        steamIDLobby = to_steamid_required(steamIDLobby,
                                           "steamid_lobby is required")
        return ActivateGameOverlayInviteDialog_native(this, steamIDLobby)
    end
    ISteamFriends.activate_game_overlay_invite_dialog =
        ISteamFriends.ActivateGameOverlayInviteDialog

    local GetSmallFriendAvatar_native = vtable_entry(this, 34,
                                                     "int(__thiscall*)(void*, SteamID)")
    ...
    local GetMediumFriendAvatar_native =
        vtable_entry(this, 35, "int(__thiscall*)(void*, SteamID)")
    ...
    local GetLargeFriendAvatar_native = vtable_entry(this, 36,
                                                     "int(__thiscall*)(void*, SteamID)")
    ...
    local RequestUserInformation_native =
        vtable_entry(this, 37, "bool(__thiscall*)(void*, SteamID, bool)")
    function ISteamFriends.RequestUserInformation(steamIDUser, bRequireNameOnly)
        steamIDUser = to_steamid_required(steamIDUser,
                                          "steamid_user is required")
        return
            RequestUserInformation_native(this, steamIDUser, bRequireNameOnly)
    end
    ISteamFriends.request_user_information =
        ISteamFriends.RequestUserInformation

    local RequestClanOfficerList_native =
        vtable_entry(this, 38, "uint64_t(__thiscall*)(void*, SteamID)")
    ...
    function ISteamFriends.RequestClanOfficerList(steamIDClan, callback)
        ...
        if callback ~= nil then
            res = register_call_result(res, callback, 335,
                                       RequestClanOfficerList_info)
        end

        return res
    end
    ISteamFriends.request_clan_officer_list =
        ISteamFriends.RequestClanOfficerList

    local GetClanOwner_native = vtable_entry(this, 39,
                                             "void(__thiscall*)(void*, SteamID *, SteamID)")
    ...
    local GetClanOfficerCount_native = vtable_entry(this, 40,
                                                    "int(__thiscall*)(void*, SteamID)")
    ...
    local GetClanOfficerByIndex_native =
        vtable_entry(this, 41,
                     "void(__thiscall*)(void*, SteamID *, SteamID, int)")
    ...
    ISteamFriends.get_clan_officer_by_index =
        ISteamFriends.GetClanOfficerByIndex

    local GetUserRestrictions_native = vtable_entry(this, 42,
    ...
    local SetRichPresence_native = vtable_entry(this, 43,
    ...
    local ClearRichPresence_native = vtable_entry(this, 44,
                                                  "void(__thiscall*)(void*)")
    ...
    local GetFriendRichPresence_native =
        vtable_entry(this, 45,
                     "const char *(__thiscall*)(void*, SteamID, const char *)")
    ...
    local GetFriendRichPresenceKeyCount_native =
        vtable_entry(this, 46, "int(__thiscall*)(void*, SteamID)")
    ...
    ISteamFriends.get_friend_rich_presence_key_count =
        ISteamFriends.GetFriendRichPresenceKeyCount

    local GetFriendRichPresenceKeyByIndex_native =
        vtable_entry(this, 47, "const char *(__thiscall*)(void*, SteamID, int)")
    function ISteamFriends.GetFriendRichPresenceKeyByIndex(steamIDFriend, iKey)
        steamIDFriend = to_steamid_required(steamIDFriend,
                                            "steamid_friend is required")
        local res = GetFriendRichPresenceKeyByIndex_native(this, steamIDFriend,
                                                           iKey)

        return res ~= nil and ffi_string(res) or nil
    end
    ISteamFriends.get_friend_rich_presence_key_by_index =
        ISteamFriends.GetFriendRichPresenceKeyByIndex

    local RequestFriendRichPresence_native =
        vtable_entry(this, 48, "void(__thiscall*)(void*, SteamID)")
    function ISteamFriends.RequestFriendRichPresence(steamIDFriend)
        steamIDFriend = to_steamid_required(steamIDFriend,
                                            "steamid_friend is required")
        return RequestFriendRichPresence_native(this, steamIDFriend)
    end
    ISteamFriends.request_friend_rich_presence =
        ISteamFriends.RequestFriendRichPresence

    local InviteUserToGame_native = vtable_entry(this, 49,
                                                 "bool(__thiscall*)(void*, SteamID, const char *)")
    ...
    local GetCoplayFriendCount_native = vtable_entry(this, 50,
                                                     "int(__thiscall*)(void*)")
    ...
    local GetCoplayFriend_native = vtable_entry(this, 51,
                                                "void(__thiscall*)(void*, SteamID *, int)")
    ...
    local GetFriendCoplayTime_native = vtable_entry(this, 52,
                                                    "int(__thiscall*)(void*, SteamID)")
    ...
    local GetFriendCoplayGame_native = vtable_entry(this, 53,
                                                    "unsigned int(__thiscall*)(void*, SteamID)")
    ...
    local JoinClanChatRoom_native = vtable_entry(this, 54,
                                                 "uint64_t(__thiscall*)(void*, SteamID)")
    ...
    function ISteamFriends.JoinClanChatRoom(steamIDClan, callback)
        ...
        if callback ~= nil then
            res =
                register_call_result(res, callback, 342, JoinClanChatRoom_info)
        end
    ...
    local LeaveClanChatRoom_native = vtable_entry(this, 55,
                                                  "bool(__thiscall*)(void*, SteamID)")
    ...
    local GetClanChatMemberCount_native =
        vtable_entry(this, 56, "int(__thiscall*)(void*, SteamID)")
    function ISteamFriends.GetClanChatMemberCount(steamIDClan)
        steamIDClan = to_steamid_required(steamIDClan,
                                          "steamid_clan is required")
        return GetClanChatMemberCount_native(this, steamIDClan)
    end
    ISteamFriends.get_clan_chat_member_count =
        ISteamFriends.GetClanChatMemberCount

    local GetChatMemberByIndex_native = vtable_entry(this, 57,
                                                     "void(__thiscall*)(void*, SteamID *, SteamID, int)")
    ...
    local SendClanChatMessage_native = vtable_entry(this, 58,
                                                    "bool(__thiscall*)(void*, SteamID, const char *)")
    function ISteamFriends.SendClanChatMessage(steamIDClanChat, pchText)
        steamIDClanChat = to_steamid_required(steamIDClanChat,
                                              "steamid_clan_chat is required")
    ...
    local GetClanChatMessage_native = vtable_entry(this, 59,
                                                   "int(__thiscall*)(void*, SteamID, int, void *, int, int *, SteamID *)")
    function ISteamFriends.GetClanChatMessage(steamIDClanChat, iMessage,
                                              prgchText, cchTextMax)
        steamIDClanChat = to_steamid_required(steamIDClanChat,
                                              "steamid_clan_chat is required")
        local peChatEntryType_out = new_int_arr()
        local psteamidChatter_out = new_SteamID_arr()
        local res = GetClanChatMessage_native(this, steamIDClanChat, iMessage,
                                              prgchText, cchTextMax,
                                              peChatEntryType_out,
                                              psteamidChatter_out)

        return res, DEREF_GCSAFE(peChatEntryType_out),
               DEREF_GCSAFE(psteamidChatter_out)
    end
    ISteamFriends.get_clan_chat_message = ISteamFriends.GetClanChatMessage

    local IsClanChatAdmin_native = vtable_entry(this, 60,
                                                "bool(__thiscall*)(void*, SteamID, SteamID)")
    ...
    local IsClanChatWindowOpenInSteam_native =
        vtable_entry(this, 61, "bool(__thiscall*)(void*, SteamID)")
    ...
    ISteamFriends.is_clan_chat_window_open_in_steam =
        ISteamFriends.IsClanChatWindowOpenInSteam

    local OpenClanChatWindowInSteam_native =
        vtable_entry(this, 62, "bool(__thiscall*)(void*, SteamID)")
    ...
    ISteamFriends.open_clan_chat_window_in_steam =
        ISteamFriends.OpenClanChatWindowInSteam

    local CloseClanChatWindowInSteam_native =
        vtable_entry(this, 63, "bool(__thiscall*)(void*, SteamID)")
    ...
    ISteamFriends.close_clan_chat_window_in_steam =
        ISteamFriends.CloseClanChatWindowInSteam

    local SetListenForFriendsMessages_native =
        vtable_entry(this, 64, "bool(__thiscall*)(void*, bool)")
    ...
    ISteamFriends.set_listen_for_friends_messages =
        ISteamFriends.SetListenForFriendsMessages

    local ReplyToFriendMessage_native = vtable_entry(this, 65,
                                                     "bool(__thiscall*)(void*, SteamID, const char *)")
    ...
    local GetFriendMessage_native = vtable_entry(this, 66,
                                                 "int(__thiscall*)(void*, SteamID, int, void *, int, int *)")
    function ISteamFriends.GetFriendMessage(steamIDFriend, iMessageID, pvData,
                                            cubData)
        steamIDFriend = to_steamid_required(steamIDFriend,
                                            "steamid_friend is required")
        local peChatEntryType_out = new_int_arr()
        local res = GetFriendMessage_native(this, steamIDFriend, iMessageID,
                                            pvData, cubData, peChatEntryType_out)

        return res, DEREF_GCSAFE(peChatEntryType_out)
    end
    ISteamFriends.get_friend_message = ISteamFriends.GetFriendMessage

    local GetFollowerCount_native = vtable_entry(this, 67,
                                                 "uint64_t(__thiscall*)(void*, SteamID)")
    ...
    function ISteamFriends.GetFollowerCount(steamID, callback)
        ...
        if callback ~= nil then
            res =
                register_call_result(res, callback, 344, GetFollowerCount_info)
        end
    ...
    local IsFollowing_native = vtable_entry(this, 68,
                                            "uint64_t(__thiscall*)(void*, SteamID)")
    ...
    local EnumerateFollowingList_native =
        vtable_entry(this, 69, "uint64_t(__thiscall*)(void*, uint32_t)")
    ...
    function ISteamFriends.EnumerateFollowingList(unStartIndex, callback)
        ...
        if callback ~= nil then
            res = register_call_result(res, callback, 346,
                                       EnumerateFollowingList_info)
        end

        return res
    end
    ISteamFriends.enumerate_following_list =
        ISteamFriends.EnumerateFollowingList

    local IsClanPublic_native = vtable_entry(this, 70,
                                             "bool(__thiscall*)(void*, SteamID)")
    ...
    local IsClanOfficialGameGroup_native =
        vtable_entry(this, 71, "bool(__thiscall*)(void*, SteamID)")
    function ISteamFriends.IsClanOfficialGameGroup(steamIDClan)
        steamIDClan = to_steamid_required(steamIDClan,
                                          "steamid_clan is required")
        return IsClanOfficialGameGroup_native(this, steamIDClan)
    end
    ISteamFriends.is_clan_official_game_group =
        ISteamFriends.IsClanOfficialGameGroup

    local GetNumChatsWithUnreadPriorityMessages_native =
        vtable_entry(this, 72, "int(__thiscall*)(void*)")
    function ISteamFriends.GetNumChatsWithUnreadPriorityMessages()
        return GetNumChatsWithUnreadPriorityMessages_native(this)
    end
    ISteamFriends.get_num_chats_with_unread_priority_messages =
        ISteamFriends.GetNumChatsWithUnreadPriorityMessages

    local ActivateGameOverlayRemotePlayTogetherInviteDialog_native =
        vtable_entry(this, 73, "void(__thiscall*)(void*, SteamID)")
    function ISteamFriends.ActivateGameOverlayRemotePlayTogetherInviteDialog(
        steamIDLobby)
        steamIDLobby = to_steamid_required(steamIDLobby,
                                           "steamid_lobby is required")
        return ActivateGameOverlayRemotePlayTogetherInviteDialog_native(this,
                                                                        steamIDLobby)
    end
    ISteamFriends.activate_game_overlay_remote_play_together_invite_dialog =
        ISteamFriends.ActivateGameOverlayRemotePlayTogetherInviteDialog

    local RegisterProtocolInOverlayBrowser_native =
        vtable_entry(this, 74, "bool(__thiscall*)(void*, const char *)")
    ...
    ISteamFriends.register_protocol_in_overlay_browser =
        ISteamFriends.RegisterProtocolInOverlayBrowser

    local ActivateGameOverlayInviteDialogConnectString_native = vtable_entry(
                                                                    this, 75,
                                                                    "void(__thiscall*)(void*, const char *)")
    function ISteamFriends.ActivateGameOverlayInviteDialogConnectString(
        pchConnectString)
        return ActivateGameOverlayInviteDialogConnectString_native(this,
                                                                   pchConnectString)
    end
    ISteamFriends.activate_game_overlay_invite_dialog_connect_string =
...
index_funcs.ISteamUtils = function()
    local this = get_steam_vtable_bind("steam_api.dll", "SteamClient", 9,
                             "void*(__thiscall*)(void*, int, const char *)")(
                     hSteamPipe, "SteamUtils010")

    local GetSecondsSinceAppActive_native =
        vtable_entry(this, 0, "uint32_t(__thiscall*)(void*)")
    function ISteamUtils.GetSecondsSinceAppActive()
        return GetSecondsSinceAppActive_native(this)
    end
    ISteamUtils.get_seconds_since_app_active =
        ISteamUtils.GetSecondsSinceAppActive

    local GetSecondsSinceComputerActive_native =
        vtable_entry(this, 1, "uint32_t(__thiscall*)(void*)")
    function ISteamUtils.GetSecondsSinceComputerActive()
        return GetSecondsSinceComputerActive_native(this)
    end
    ISteamUtils.get_seconds_since_computer_active =
        ISteamUtils.GetSecondsSinceComputerActive

    local GetConnectedUniverse_native = vtable_entry(this, 2,
                                                     "int(__thiscall*)(void*)")
    ...
    local GetServerRealTime_native = vtable_entry(this, 3,
                                                  "uint32_t(__thiscall*)(void*)")
    ...
    local GetIPCountry_native = vtable_entry(this, 4,
                                             "const char *(__thiscall*)(void*)")
    ...
    local GetImageSize_native = vtable_entry(this, 5,
    ...
    local GetImageRGBA_native = vtable_entry(this, 6,
                                             "bool(__thiscall*)(void*, int, uint8_t *, int)")
    ...
    local GetCurrentBatteryPower_native =
        vtable_entry(this, 8, "uint8_t(__thiscall*)(void*)")
    ...
    local GetAppID_native =
        vtable_entry(this, 9, "uint32_t(__thiscall*)(void*)")
    function ISteamUtils.GetAppID() return GetAppID_native(this) end
    ISteamUtils.get_appid = ISteamUtils.GetAppID

    local SetOverlayNotificationPosition_native =
        vtable_entry(this, 10, "void(__thiscall*)(void*, int)")
    function ISteamUtils.SetOverlayNotificationPosition(eNotificationPosition)
        eNotificationPosition = to_enum_required(eNotificationPosition,
                                                 enums.ENotificationPosition,
                                                 "notification_position is required")
        return
            SetOverlayNotificationPosition_native(this, eNotificationPosition)
    end
    ISteamUtils.set_overlay_notification_position =
        ISteamUtils.SetOverlayNotificationPosition

    local IsAPICallCompleted_native = vtable_entry(this, 11,
                                                   "bool(__thiscall*)(void*, uint64_t, bool *)")
    ...
    local GetAPICallFailureReason_native =
        vtable_entry(this, 12, "int(__thiscall*)(void*, uint64_t)")
    function ISteamUtils.GetAPICallFailureReason(hSteamAPICall)
        return GetAPICallFailureReason_native(this, hSteamAPICall)
    end
    ISteamUtils.get_api_call_failure_reason =
        ISteamUtils.GetAPICallFailureReason

    local GetAPICallResult_native = vtable_entry(this, 13,
                                                 "bool(__thiscall*)(void*, uint64_t, void *, int, int, bool *)")
    function ISteamUtils.GetAPICallResult(hSteamAPICall, pCallback, cubCallback,
                                          iCallbackExpected)
        local pbFailed_out = new_bool_arr()
        local res = GetAPICallResult_native(this, hSteamAPICall, pCallback,
                                            cubCallback, iCallbackExpected,
                                            pbFailed_out)
    ...
    local GetIPCCallCount_native = vtable_entry(this, 15,
                                                "uint32_t(__thiscall*)(void*)")
    ...
    local SetWarningMessageHook_native =
        vtable_entry(this, 16,
                     "void(__thiscall*)(void*, void*(__cdecl*)(int, const char*))")
    ...
    local IsOverlayEnabled_native = vtable_entry(this, 17,
                                                 "bool(__thiscall*)(void*)")
    ...
    local BOverlayNeedsPresent_native = vtable_entry(this, 18,
                                                     "bool(__thiscall*)(void*)")
    ...
    local CheckFileSignature_native = vtable_entry(this, 19,
                                                   "uint64_t(__thiscall*)(void*, const char *)")
    ...
    function ISteamUtils.CheckFileSignature(szFileName, callback)
        ...
        if callback ~= nil then
            res = register_call_result(res, callback, 705,
                                       CheckFileSignature_info)
        end
    ...
    local ShowGamepadTextInput_native = vtable_entry(this, 20,
                                                     "bool(__thiscall*)(void*, int, int, const char *, uint32_t, const char *)")
    function ISteamUtils.ShowGamepadTextInput(eInputMode, eLineInputMode,
                                              pchDescription, unCharMax,
                                              pchExistingText)
        eInputMode = to_enum_required(eInputMode, enums.EGamepadTextInputMode,
                                      "input_mode is required")
        eLineInputMode = to_enum_required(eLineInputMode,
                                          enums.EGamepadTextInputLineMode,
                                          "line_input_mode is required")
        return ShowGamepadTextInput_native(this, eInputMode, eLineInputMode,
                                           pchDescription, unCharMax,
                                           pchExistingText)
    end
    ISteamUtils.show_gamepad_text_input = ISteamUtils.ShowGamepadTextInput

    local GetEnteredGamepadTextLength_native =
        vtable_entry(this, 21, "uint32_t(__thiscall*)(void*)")
    function ISteamUtils.GetEnteredGamepadTextLength()
        return GetEnteredGamepadTextLength_native(this)
    end
    ISteamUtils.get_entered_gamepad_text_length =
        ISteamUtils.GetEnteredGamepadTextLength

    local GetEnteredGamepadTextInput_native =
        vtable_entry(this, 22, "bool(__thiscall*)(void*, char *, uint32_t)")
    ...
    ISteamUtils.get_entered_gamepad_text_input =
        ISteamUtils.GetEnteredGamepadTextInput

    local GetSteamUILanguage_native = vtable_entry(this, 23,
                                                   "const char *(__thiscall*)(void*)")
    ...
    local IsSteamRunningInVR_native = vtable_entry(this, 24,
                                                   "bool(__thiscall*)(void*)")
    ...
    local SetOverlayNotificationInset_native =
        vtable_entry(this, 25, "void(__thiscall*)(void*, int, int)")
    function ISteamUtils.SetOverlayNotificationInset(nHorizontalInset,
                                                     nVerticalInset)
        return SetOverlayNotificationInset_native(this, nHorizontalInset,
                                                  nVerticalInset)
    end
    ISteamUtils.set_overlay_notification_inset =
        ISteamUtils.SetOverlayNotificationInset

    local IsSteamInBigPictureMode_native =
        vtable_entry(this, 26, "bool(__thiscall*)(void*)")
    function ISteamUtils.IsSteamInBigPictureMode()
        return IsSteamInBigPictureMode_native(this)
    end
    ISteamUtils.is_steam_in_big_picture_mode =
        ISteamUtils.IsSteamInBigPictureMode

    local StartVRDashboard_native = vtable_entry(this, 27,
                                                 "void(__thiscall*)(void*)")
    ...
    local InitFilterText_native = vtable_entry(this, 31,
                                               "bool(__thiscall*)(void*, uint32_t)")
    ...
    local FilterText_native = vtable_entry(this, 32,
                                           "int(__thiscall*)(void*, int, SteamID, const char *, char *, uint32_t)")
    function ISteamUtils.FilterText(eContext, sourceSteamID, pchInputMessage,
                                    pchOutFilteredText, nByteSizeOutFilteredText)
        eContext = to_enum_required(eContext, enums.ETextFilteringContext,
                                    "context is required")
        sourceSteamID = to_steamid_required(sourceSteamID,
                                            "source_steamid is required")
        return FilterText_native(this, eContext, sourceSteamID, pchInputMessage,
                                 pchOutFilteredText, nByteSizeOutFilteredText)
    end
    ISteamUtils.filter_text = ISteamUtils.FilterText

    local GetIPv6ConnectivityState_native =
        vtable_entry(this, 33, "int(__thiscall*)(void*, int)")
    function ISteamUtils.GetIPv6ConnectivityState(eProtocol)
        eProtocol = to_enum_required(eProtocol,
                                     enums.ESteamIPv6ConnectivityProtocol,
                                     "protocol is required")
        return GetIPv6ConnectivityState_native(this, eProtocol)
    end
    ISteamUtils.get_ipv6_connectivity_state =
        ISteamUtils.GetIPv6ConnectivityState
...
index_funcs.ISteamMatchmaking = function()
    local this = get_steam_vtable_bind("steam_api.dll", "SteamClient", 10,
                             "void*(__thiscall*)(void*, int, int, const char *)")(
                     hSteamUser, hSteamPipe, "SteamMatchMaking009")

    local GetFavoriteGameCount_native = vtable_entry(this, 0,
                                                     "int(__thiscall*)(void*)")
    function ISteamMatchmaking.GetFavoriteGameCount()
        return GetFavoriteGameCount_native(this)
    end
    ISteamMatchmaking.get_favorite_game_count =
        ISteamMatchmaking.GetFavoriteGameCount

    local GetFavoriteGame_native = vtable_entry(this, 1,
                                                "bool(__thiscall*)(void*, int, unsigned int *, uint32_t *, uint16_t *, uint16_t *, uint32_t *, uint32_t *)")
    function ISteamMatchmaking.GetFavoriteGame(iGame)
        ...
        local res = GetFavoriteGame_native(this, iGame, pnAppID_out, pnIP_out,
                                           pnConnPort_out, pnQueryPort_out,
                                           punFlags_out,
                                           pRTime32LastPlayedOnServer_out)

        return res, DEREF_GCSAFE(pnAppID_out), DEREF_GCSAFE(pnIP_out),
               DEREF_GCSAFE(pnConnPort_out), DEREF_GCSAFE(pnQueryPort_out),
               DEREF_GCSAFE(punFlags_out),
               DEREF_GCSAFE(pRTime32LastPlayedOnServer_out)
    end
    ISteamMatchmaking.get_favorite_game = ISteamMatchmaking.GetFavoriteGame

    local AddFavoriteGame_native = vtable_entry(this, 2,
                                                "int(__thiscall*)(void*, unsigned int, uint32_t, uint16_t, uint16_t, uint32_t, uint32_t)")
    function ISteamMatchmaking.AddFavoriteGame(nAppID, nIP, nConnPort,
                                               nQueryPort, unFlags,
                                               rTime32LastPlayedOnServer)
        return AddFavoriteGame_native(this, nAppID, nIP, nConnPort, nQueryPort,
                                      unFlags, rTime32LastPlayedOnServer)
    end
    ISteamMatchmaking.add_favorite_game = ISteamMatchmaking.AddFavoriteGame

    local RemoveFavoriteGame_native = vtable_entry(this, 3,
                                                   "bool(__thiscall*)(void*, unsigned int, uint32_t, uint16_t, uint16_t, uint32_t)")
    function ISteamMatchmaking.RemoveFavoriteGame(nAppID, nIP, nConnPort,
                                                  nQueryPort, unFlags)
        return RemoveFavoriteGame_native(this, nAppID, nIP, nConnPort,
                                         nQueryPort, unFlags)
    end
    ISteamMatchmaking.remove_favorite_game =
        ISteamMatchmaking.RemoveFavoriteGame

    local RequestLobbyList_native = vtable_entry(this, 4,
                                                 "uint64_t(__thiscall*)(void*)")
    ...
    function ISteamMatchmaking.RequestLobbyList(callback)
        ...
        if callback ~= nil then
            res =
                register_call_result(res, callback, 510, RequestLobbyList_info)
        end
    ...
    local AddRequestLobbyListStringFilter_native =
        vtable_entry(this, 5,
                     "void(__thiscall*)(void*, const char *, const char *, int)")
    function ISteamMatchmaking.AddRequestLobbyListStringFilter(pchKeyToMatch,
                                                               pchValueToMatch,
                                                               eComparisonType)
        eComparisonType = to_enum_required(eComparisonType,
                                           enums.ELobbyComparison,
                                           "comparison_type is required")
        return AddRequestLobbyListStringFilter_native(this, pchKeyToMatch,
                                                      pchValueToMatch,
                                                      eComparisonType)
    end
    ISteamMatchmaking.add_request_lobby_list_string_filter =
        ISteamMatchmaking.AddRequestLobbyListStringFilter

    local AddRequestLobbyListNumericalFilter_native =
        vtable_entry(this, 6, "void(__thiscall*)(void*, const char *, int, int)")
    function ISteamMatchmaking.AddRequestLobbyListNumericalFilter(pchKeyToMatch,
                                                                  nValueToMatch,
                                                                  eComparisonType)
        eComparisonType = to_enum_required(eComparisonType,
                                           enums.ELobbyComparison,
                                           "comparison_type is required")
        return AddRequestLobbyListNumericalFilter_native(this, pchKeyToMatch,
                                                         nValueToMatch,
                                                         eComparisonType)
    end
    ISteamMatchmaking.add_request_lobby_list_numerical_filter =
        ISteamMatchmaking.AddRequestLobbyListNumericalFilter

    local AddRequestLobbyListNearValueFilter_native =
        vtable_entry(this, 7, "void(__thiscall*)(void*, const char *, int)")
    function ISteamMatchmaking.AddRequestLobbyListNearValueFilter(pchKeyToMatch,
                                                                  nValueToBeCloseTo)
        return AddRequestLobbyListNearValueFilter_native(this, pchKeyToMatch,
                                                         nValueToBeCloseTo)
    end
    ISteamMatchmaking.add_request_lobby_list_near_value_filter =
        ISteamMatchmaking.AddRequestLobbyListNearValueFilter

    local AddRequestLobbyListFilterSlotsAvailable_native =
        vtable_entry(this, 8, "void(__thiscall*)(void*, int)")
    function ISteamMatchmaking.AddRequestLobbyListFilterSlotsAvailable(
        nSlotsAvailable)
        return AddRequestLobbyListFilterSlotsAvailable_native(this,
                                                              nSlotsAvailable)
    end
    ISteamMatchmaking.add_request_lobby_list_filter_slots_available =
        ISteamMatchmaking.AddRequestLobbyListFilterSlotsAvailable

    local AddRequestLobbyListDistanceFilter_native =
        vtable_entry(this, 9, "void(__thiscall*)(void*, int)")
    function ISteamMatchmaking.AddRequestLobbyListDistanceFilter(
        eLobbyDistanceFilter)
        eLobbyDistanceFilter = to_enum_required(eLobbyDistanceFilter,
                                                enums.ELobbyDistanceFilter,
                                                "lobby_distance_filter is required")
        return AddRequestLobbyListDistanceFilter_native(this,
                                                        eLobbyDistanceFilter)
    end
    ISteamMatchmaking.add_request_lobby_list_distance_filter =
        ISteamMatchmaking.AddRequestLobbyListDistanceFilter

    local AddRequestLobbyListResultCountFilter_native =
        vtable_entry(this, 10, "void(__thiscall*)(void*, int)")
    ...
    ISteamMatchmaking.add_request_lobby_list_result_count_filter =
        ISteamMatchmaking.AddRequestLobbyListResultCountFilter

    local AddRequestLobbyListCompatibleMembersFilter_native = vtable_entry(this,
                                                                           11,
                                                                           "void(__thiscall*)(void*, SteamID)")
    function ISteamMatchmaking.AddRequestLobbyListCompatibleMembersFilter(
        steamIDLobby)
        steamIDLobby = to_steamid_required(steamIDLobby,
                                           "steamid_lobby is required")
        return AddRequestLobbyListCompatibleMembersFilter_native(this,
                                                                 steamIDLobby)
    end
    ISteamMatchmaking.add_request_lobby_list_compatible_members_filter =
        ISteamMatchmaking.AddRequestLobbyListCompatibleMembersFilter

    local GetLobbyByIndex_native = vtable_entry(this, 12,
                                                "void(__thiscall*)(void*, SteamID *, int)")
    ...
    local CreateLobby_native = vtable_entry(this, 13,
                                            "uint64_t(__thiscall*)(void*, int, int)")
    ...
    function ISteamMatchmaking.CreateLobby(eLobbyType, cMaxMembers, callback)
        eLobbyType = to_enum_required(eLobbyType, enums.ELobbyType,
                                      "lobby_type is required")
        if callback ~= nil and not is_valid_callback(callback) then
            return error("Invalid callback, expected function or await")
        end
    ...
    local JoinLobby_native = vtable_entry(this, 14,
                                          "uint64_t(__thiscall*)(void*, SteamID)")
    ...
    local LeaveLobby_native = vtable_entry(this, 15,
                                           "void(__thiscall*)(void*, SteamID)")
    ...
    local InviteUserToLobby_native = vtable_entry(this, 16,
                                                  "bool(__thiscall*)(void*, SteamID, SteamID)")
    function ISteamMatchmaking.InviteUserToLobby(steamIDLobby, steamIDInvitee)
        steamIDLobby = to_steamid_required(steamIDLobby,
                                           "steamid_lobby is required")
        steamIDInvitee = to_steamid_required(steamIDInvitee,
                                             "steamid_invitee is required")
    ...
    local GetNumLobbyMembers_native = vtable_entry(this, 17,
                                                   "int(__thiscall*)(void*, SteamID)")
    ...
    ISteamMatchmaking.get_num_lobby_members =
        ISteamMatchmaking.GetNumLobbyMembers

    local GetLobbyMemberByIndex_native =
        vtable_entry(this, 18,
                     "void(__thiscall*)(void*, SteamID *, SteamID, int)")
    ...
    ISteamMatchmaking.get_lobby_member_by_index =
        ISteamMatchmaking.GetLobbyMemberByIndex

    local GetLobbyData_native = vtable_entry(this, 19,
                                             "const char *(__thiscall*)(void*, SteamID, const char *)")
    ...
    local SetLobbyData_native = vtable_entry(this, 20,
                                             "bool(__thiscall*)(void*, SteamID, const char *, const char *)")
    ...
    local GetLobbyDataCount_native = vtable_entry(this, 21,
                                                  "int(__thiscall*)(void*, SteamID)")
    ...
    local GetLobbyDataByIndex_native = vtable_entry(this, 22,
                                                    "bool(__thiscall*)(void*, SteamID, int, char *, int, char *, int)")
    function ISteamMatchmaking.GetLobbyDataByIndex(steamIDLobby, iLobbyData,
                                                   pchKey, cchKeyBufferSize,
                                                   pchValue, cchValueBufferSize)
        steamIDLobby = to_steamid_required(steamIDLobby,
                                           "steamid_lobby is required")
        return GetLobbyDataByIndex_native(this, steamIDLobby, iLobbyData,
                                          pchKey, cchKeyBufferSize, pchValue,
                                          cchValueBufferSize)
    end
    ISteamMatchmaking.get_lobby_data_by_index =
        ISteamMatchmaking.GetLobbyDataByIndex

    local DeleteLobbyData_native = vtable_entry(this, 23,
                                                "bool(__thiscall*)(void*, SteamID, const char *)")
    ...
    local GetLobbyMemberData_native = vtable_entry(this, 24,
                                                   "const char *(__thiscall*)(void*, SteamID, SteamID, const char *)")
    function ISteamMatchmaking.GetLobbyMemberData(steamIDLobby, steamIDUser,
                                                  pchKey)
        steamIDLobby = to_steamid_required(steamIDLobby,
                                           "steamid_lobby is required")
        steamIDUser = to_steamid_required(steamIDUser,
                                          "steamid_user is required")
        local res = GetLobbyMemberData_native(this, steamIDLobby, steamIDUser,
                                              pchKey)

        return res ~= nil and ffi_string(res) or nil
    end
    ISteamMatchmaking.get_lobby_member_data =
        ISteamMatchmaking.GetLobbyMemberData

    local SetLobbyMemberData_native = vtable_entry(this, 25,
                                                   "void(__thiscall*)(void*, SteamID, const char *, const char *)")
    ...
    ISteamMatchmaking.set_lobby_member_data =
        ISteamMatchmaking.SetLobbyMemberData

    local SendLobbyChatMsg_native = vtable_entry(this, 26,
                                                 "bool(__thiscall*)(void*, SteamID, const void *, int)")
    function ISteamMatchmaking.SendLobbyChatMsg(steamIDLobby, pvMsgBody,
                                                cubMsgBody)
        steamIDLobby = to_steamid_required(steamIDLobby,
                                           "steamid_lobby is required")
        return
            SendLobbyChatMsg_native(this, steamIDLobby, pvMsgBody, cubMsgBody)
    end
    ISteamMatchmaking.send_lobby_chat_msg = ISteamMatchmaking.SendLobbyChatMsg

    local GetLobbyChatEntry_native = vtable_entry(this, 27,
                                                  "int(__thiscall*)(void*, SteamID, int, SteamID *, void *, int, int *)")
    function ISteamMatchmaking.GetLobbyChatEntry(steamIDLobby, iChatID, pvData,
                                                 cubData)
        steamIDLobby = to_steamid_required(steamIDLobby,
                                           "steamid_lobby is required")
        local pSteamIDUser_out = new_SteamID_arr()
        local peChatEntryType_out = new_int_arr()
        local res = GetLobbyChatEntry_native(this, steamIDLobby, iChatID,
                                             pSteamIDUser_out, pvData, cubData,
                                             peChatEntryType_out)

        return res, DEREF_GCSAFE(pSteamIDUser_out),
               DEREF_GCSAFE(peChatEntryType_out)
    end
    ISteamMatchmaking.get_lobby_chat_entry = ISteamMatchmaking.GetLobbyChatEntry

    local RequestLobbyData_native = vtable_entry(this, 28,
                                                 "bool(__thiscall*)(void*, SteamID)")
    ...
    local SetLobbyGameServer_native = vtable_entry(this, 29,
                                                   "void(__thiscall*)(void*, SteamID, uint32_t, uint16_t, SteamID)")
    function ISteamMatchmaking.SetLobbyGameServer(steamIDLobby, unGameServerIP,
                                                  unGameServerPort,
                                                  steamIDGameServer)
        steamIDLobby = to_steamid_required(steamIDLobby,
                                           "steamid_lobby is required")
        unGameServerIP = to_ip_int_required(unGameServerIP,
                                            "game_server_ip is required")
        steamIDGameServer = to_steamid_required(steamIDGameServer,
                                                "steamid_game_server is required")
        return SetLobbyGameServer_native(this, steamIDLobby, unGameServerIP,
                                         unGameServerPort, steamIDGameServer)
    end
    ISteamMatchmaking.set_lobby_game_server =
        ISteamMatchmaking.SetLobbyGameServer

    local GetLobbyGameServer_native = vtable_entry(this, 30,
                                                   "bool(__thiscall*)(void*, SteamID, uint32_t *, uint16_t *, SteamID *)")
    function ISteamMatchmaking.GetLobbyGameServer(steamIDLobby)
        ...
        local res = GetLobbyGameServer_native(this, steamIDLobby,
                                              punGameServerIP_out,
                                              punGameServerPort_out,
                                              psteamIDGameServer_out)

        return res, DEREF_GCSAFE(punGameServerIP_out),
               DEREF_GCSAFE(punGameServerPort_out),
               DEREF_GCSAFE(psteamIDGameServer_out)
    end
    ISteamMatchmaking.get_lobby_game_server =
        ISteamMatchmaking.GetLobbyGameServer

    local SetLobbyMemberLimit_native = vtable_entry(this, 31,
                                                    "bool(__thiscall*)(void*, SteamID, int)")
    ...
    ISteamMatchmaking.set_lobby_member_limit =
        ISteamMatchmaking.SetLobbyMemberLimit

    local GetLobbyMemberLimit_native = vtable_entry(this, 32,
                                                    "int(__thiscall*)(void*, SteamID)")
    ...
    ISteamMatchmaking.get_lobby_member_limit =
        ISteamMatchmaking.GetLobbyMemberLimit

    local SetLobbyType_native = vtable_entry(this, 33,
                                             "bool(__thiscall*)(void*, SteamID, int)")
    ...
    local SetLobbyJoinable_native = vtable_entry(this, 34,
                                                 "bool(__thiscall*)(void*, SteamID, bool)")
    ...
    local GetLobbyOwner_native = vtable_entry(this, 35,
                                              "void(__thiscall*)(void*, SteamID *, SteamID)")
    ...
    local SetLobbyOwner_native = vtable_entry(this, 36,
                                              "bool(__thiscall*)(void*, SteamID, SteamID)")
    function ISteamMatchmaking.SetLobbyOwner(steamIDLobby, steamIDNewOwner)
        steamIDLobby = to_steamid_required(steamIDLobby,
                                           "steamid_lobby is required")
        steamIDNewOwner = to_steamid_required(steamIDNewOwner,
                                              "steamid_new_owner is required")
    ...
    local SetLinkedLobby_native = vtable_entry(this, 37,
                                               "bool(__thiscall*)(void*, SteamID, SteamID)")
    function ISteamMatchmaking.SetLinkedLobby(steamIDLobby,
                                              steamIDLobbyDependent)
        steamIDLobby = to_steamid_required(steamIDLobby,
                                           "steamid_lobby is required")
        steamIDLobbyDependent = to_steamid_required(steamIDLobbyDependent,
                                                    "steamid_lobby_dependent is required")
...
do
    ...
    local callback_base_array = typeof(
                                    "struct _ISteamMatchmakingServerListResponse[1]")
    local callback_base_ptr = typeof(
                                  "struct _ISteamMatchmakingServerListResponse*")
    ...
    function ISteamMatchmakingServerListResponse.new(user_callbacks)
        ...
        if vtbl_callbacks == nil then
            registered_user_callbacks = {
                ServerResponded = {},
                ServerFailedToRespond = {},
                RefreshComplete = {}
            }
            vtbl_callbacks = {
                ServerResponded = cast(typeof(
                                           "void(__thiscall*)(void*, void *, int)"),
                                       get_user_interface_callback_handler(
                                           registered_user_callbacks["ServerResponded"])),
                ServerFailedToRespond = cast(typeof(
                                                 "void(__thiscall*)(void*, void *, int)"),
                                             get_user_interface_callback_handler(
                                                 registered_user_callbacks["ServerFailedToRespond"])),
                RefreshComplete = cast(typeof(
                                           "void(__thiscall*)(void*, void *, int)"),
                                       get_user_interface_callback_handler(
                                           registered_user_callbacks["RefreshComplete"]))
            }
        end
        ...
        for key, value in pairs(user_callbacks) do
            ...
            elseif type(value) ~= "function" then
                return error(string_format("Invalid callback type for %s: %s",
                                           tostring(key), type(value)), 2)
            end
        end
        ...
        for key, value in pairs(vtbl_callbacks) do
            if user_callbacks[key] ~= nil then
                registered_user_callbacks[key][instance_key] =
                    user_callbacks[key]
            end
        end
...
do
    ...
    local callback_base_array = typeof(
                                    "struct _ISteamMatchmakingPingResponse[1]")
    ...
    function ISteamMatchmakingPingResponse.new(user_callbacks)
        ...
        if vtbl_callbacks == nil then
            ...
            vtbl_callbacks = {
                ServerResponded = cast(typeof(
                                           "void(__thiscall*)(void*, gameserveritem_t &)"),
                                       get_user_interface_callback_handler(
                                           registered_user_callbacks["ServerResponded"])),
                ServerFailedToRespond = cast(typeof("void(__thiscall*)(void*)"),
                                             get_user_interface_callback_handler(
                                                 registered_user_callbacks["ServerFailedToRespond"]))
            }
        end
...
do
    ...
    local callback_base_array = typeof(
                                    "struct _ISteamMatchmakingPlayersResponse[1]")
    local callback_base_ptr =
        typeof("struct _ISteamMatchmakingPlayersResponse*")
    ...
    function ISteamMatchmakingPlayersResponse.new(user_callbacks)
        ...
        if vtbl_callbacks == nil then
            ...
            vtbl_callbacks = {
                AddPlayerToList = cast(typeof(
                                           "void(__thiscall*)(void*, const char *, int, float)"),
                                       get_user_interface_callback_handler(
                                           registered_user_callbacks["AddPlayerToList"])),
                PlayersFailedToRespond = cast(
                    typeof("void(__thiscall*)(void*)"),
                    get_user_interface_callback_handler(
                        registered_user_callbacks["PlayersFailedToRespond"])),
                PlayersRefreshComplete = cast(
                    typeof("void(__thiscall*)(void*)"),
                    get_user_interface_callback_handler(
                        registered_user_callbacks["PlayersRefreshComplete"]))
            }
        end
...
do
    ...
    local callback_base_array = typeof(
                                    "struct _ISteamMatchmakingRulesResponse[1]")
    ...
    function ISteamMatchmakingRulesResponse.new(user_callbacks)
        ...
        if vtbl_callbacks == nil then
            ...
            vtbl_callbacks = {
                RulesResponded = cast(typeof(
                                          "void(__thiscall*)(void*, const char *, const char *)"),
                                      get_user_interface_callback_handler(
                                          registered_user_callbacks["RulesResponded"])),
                RulesFailedToRespond = cast(typeof("void(__thiscall*)(void*)"),
                                            get_user_interface_callback_handler(
                                                registered_user_callbacks["RulesFailedToRespond"])),
                RulesRefreshComplete = cast(typeof("void(__thiscall*)(void*)"),
                                            get_user_interface_callback_handler(
                                                registered_user_callbacks["RulesRefreshComplete"]))
            }
        end
...
local ISteamMatchmakingServers = {
    version = "SteamMatchMakingServers002",
    version_number = 2
}

index_funcs.ISteamMatchmakingServers = function()
    local this = get_steam_vtable_bind("steam_api.dll", "SteamClient", 11,
                             "void*(__thiscall*)(void*, int, int, const char *)")(
                     hSteamUser, hSteamPipe, "SteamMatchMakingServers002")

    local RequestInternetServerList_native =
        vtable_entry(this, 0,
                     "void *(__thiscall*)(void*, unsigned int, MatchMakingKeyValuePair_t **, uint32_t, ISteamMatchmakingServerListResponse *)")
    function ISteamMatchmakingServers.RequestInternetServerList(iApp,
                                                                ppchFilters,
                                                                nFilters,
                                                                pRequestServersResponse)
        return RequestInternetServerList_native(this, iApp, ppchFilters,
                                                nFilters,
                                                pRequestServersResponse)
    end
    ISteamMatchmakingServers.request_internet_server_list =
        ISteamMatchmakingServers.RequestInternetServerList

    local RequestLANServerList_native = vtable_entry(this, 1,
                                                     "void *(__thiscall*)(void*, unsigned int, ISteamMatchmakingServerListResponse *)")
    function ISteamMatchmakingServers.RequestLANServerList(iApp,
                                                           pRequestServersResponse)
        return RequestLANServerList_native(this, iApp, pRequestServersResponse)
    end
    ISteamMatchmakingServers.request_lan_server_list =
        ISteamMatchmakingServers.RequestLANServerList

    local RequestFriendsServerList_native =
        vtable_entry(this, 2,
                     "void *(__thiscall*)(void*, unsigned int, MatchMakingKeyValuePair_t **, uint32_t, ISteamMatchmakingServerListResponse *)")
    function ISteamMatchmakingServers.RequestFriendsServerList(iApp,
                                                               ppchFilters,
                                                               nFilters,
                                                               pRequestServersResponse)
        return RequestFriendsServerList_native(this, iApp, ppchFilters,
                                               nFilters, pRequestServersResponse)
    end
    ISteamMatchmakingServers.request_friends_server_list =
        ISteamMatchmakingServers.RequestFriendsServerList

    local RequestFavoritesServerList_native =
        vtable_entry(this, 3,
                     "void *(__thiscall*)(void*, unsigned int, MatchMakingKeyValuePair_t **, uint32_t, ISteamMatchmakingServerListResponse *)")
    function ISteamMatchmakingServers.RequestFavoritesServerList(iApp,
                                                                 ppchFilters,
                                                                 nFilters,
                                                                 pRequestServersResponse)
        return RequestFavoritesServerList_native(this, iApp, ppchFilters,
                                                 nFilters,
                                                 pRequestServersResponse)
    end
    ISteamMatchmakingServers.request_favorites_server_list =
        ISteamMatchmakingServers.RequestFavoritesServerList

    local RequestHistoryServerList_native =
        vtable_entry(this, 4,
                     "void *(__thiscall*)(void*, unsigned int, MatchMakingKeyValuePair_t **, uint32_t, ISteamMatchmakingServerListResponse *)")
    function ISteamMatchmakingServers.RequestHistoryServerList(iApp,
                                                               ppchFilters,
                                                               nFilters,
                                                               pRequestServersResponse)
        return RequestHistoryServerList_native(this, iApp, ppchFilters,
                                               nFilters, pRequestServersResponse)
    end
    ISteamMatchmakingServers.request_history_server_list =
        ISteamMatchmakingServers.RequestHistoryServerList

    local RequestSpectatorServerList_native =
        vtable_entry(this, 5,
                     "void *(__thiscall*)(void*, unsigned int, MatchMakingKeyValuePair_t **, uint32_t, ISteamMatchmakingServerListResponse *)")
    function ISteamMatchmakingServers.RequestSpectatorServerList(iApp,
                                                                 ppchFilters,
                                                                 nFilters,
                                                                 pRequestServersResponse)
        return RequestSpectatorServerList_native(this, iApp, ppchFilters,
                                                 nFilters,
                                                 pRequestServersResponse)
    end
    ISteamMatchmakingServers.request_spectator_server_list =
        ISteamMatchmakingServers.RequestSpectatorServerList

    local ReleaseRequest_native = vtable_entry(this, 6,
    ...
    ISteamMatchmakingServers.release_request =
        ISteamMatchmakingServers.ReleaseRequest

    local GetServerDetails_native = vtable_entry(this, 7,
                                                 "gameserveritem_t *(__thiscall*)(void*, void *, int)")
    function ISteamMatchmakingServers.GetServerDetails(hRequest, iServer)
        return GetServerDetails_native(this, hRequest, iServer)
    end
    ISteamMatchmakingServers.get_server_details =
        ISteamMatchmakingServers.GetServerDetails

    local CancelQuery_native = vtable_entry(this, 8,
                                            "void(__thiscall*)(void*, void *)")
    ...
    ISteamMatchmakingServers.get_server_count =
        ISteamMatchmakingServers.GetServerCount

    local RefreshServer_native = vtable_entry(this, 12,
                                              "void(__thiscall*)(void*, void *, int)")
    function ISteamMatchmakingServers.RefreshServer(hRequest, iServer)
        return RefreshServer_native(this, hRequest, iServer)
    end
    ISteamMatchmakingServers.refresh_server =
        ISteamMatchmakingServers.RefreshServer

    local PingServer_native = vtable_entry(this, 13,
                                           "int(__thiscall*)(void*, uint32_t, uint16_t, ISteamMatchmakingPingResponse *)")
    function ISteamMatchmakingServers.PingServer(unIP, usPort,
                                                 pRequestServersResponse)
    ...
    local PlayerDetails_native = vtable_entry(this, 14,
                                              "int(__thiscall*)(void*, uint32_t, uint16_t, ISteamMatchmakingPlayersResponse *)")
    function ISteamMatchmakingServers.PlayerDetails(unIP, usPort,
                                                    pRequestServersResponse)
        unIP = to_ip_int_required(unIP, "ip is required")
        return PlayerDetails_native(this, unIP, usPort, pRequestServersResponse)
    end
    ISteamMatchmakingServers.player_details =
        ISteamMatchmakingServers.PlayerDetails

    local ServerRules_native = vtable_entry(this, 15,
                                            "int(__thiscall*)(void*, uint32_t, uint16_t, ISteamMatchmakingRulesResponse *)")
    function ISteamMatchmakingServers.ServerRules(unIP, usPort,
                                                  pRequestServersResponse)
        unIP = to_ip_int_required(unIP, "ip is required")
    ...
    local CancelServerQuery_native = vtable_entry(this, 16,
    ...
    ISteamMatchmakingServers.cancel_server_query =
        ISteamMatchmakingServers.CancelServerQuery
...
index_funcs.ISteamUserStats = function()
    ...
    local GetStatInt32_native = vtable_entry(this, 1,
                                             "bool(__thiscall*)(void*, const char *, int32_t *)")
    ...
    local GetStatFloat_native = vtable_entry(this, 2,
                                             "bool(__thiscall*)(void*, const char *, float *)")
    ...
    local SetStatInt32_native = vtable_entry(this, 3,
                                             "bool(__thiscall*)(void*, const char *, int32_t)")
    ...
    local SetStatFloat_native = vtable_entry(this, 4,
                                             "bool(__thiscall*)(void*, const char *, float)")
    ...
    local UpdateAvgRateStat_native = vtable_entry(this, 5,
                                                  "bool(__thiscall*)(void*, const char *, float, double)")
    function ISteamUserStats.UpdateAvgRateStat(pchName, flCountThisSession,
                                               dSessionLength)
        return UpdateAvgRateStat_native(this, pchName, flCountThisSession,
                                        dSessionLength)
    end
    ISteamUserStats.update_avg_rate_stat = ISteamUserStats.UpdateAvgRateStat

    local GetAchievement_native = vtable_entry(this, 6,
                                               "bool(__thiscall*)(void*, const char *, bool *)")
    ...
    local SetAchievement_native = vtable_entry(this, 7,
    ...
    local ClearAchievement_native = vtable_entry(this, 8,
                                                 "bool(__thiscall*)(void*, const char *)")
    ...
    local GetAchievementAndUnlockTime_native =
        vtable_entry(this, 9,
                     "bool(__thiscall*)(void*, const char *, bool *, uint32_t *)")
    function ISteamUserStats.GetAchievementAndUnlockTime(pchName)
        local pbAchieved_out = new_bool_arr()
        local punUnlockTime_out = new_uint32_arr()
        local res = GetAchievementAndUnlockTime_native(this, pchName,
                                                       pbAchieved_out,
                                                       punUnlockTime_out)

        return res, DEREF_GCSAFE(pbAchieved_out),
               DEREF_GCSAFE(punUnlockTime_out)
    end
    ISteamUserStats.get_achievement_and_unlock_time =
        ISteamUserStats.GetAchievementAndUnlockTime

    local StoreStats_native = vtable_entry(this, 10, "bool(__thiscall*)(void*)")
    function ISteamUserStats.StoreStats() return StoreStats_native(this) end
    ISteamUserStats.store_stats = ISteamUserStats.StoreStats

    local GetAchievementIcon_native = vtable_entry(this, 11,
    ...
    local GetAchievementDisplayAttribute_native =
        vtable_entry(this, 12,
    ...
    ISteamUserStats.get_achievement_display_attribute =
        ISteamUserStats.GetAchievementDisplayAttribute

    local IndicateAchievementProgress_native =
        vtable_entry(this, 13,
                     "bool(__thiscall*)(void*, const char *, uint32_t, uint32_t)")
    function ISteamUserStats.IndicateAchievementProgress(pchName, nCurProgress,
                                                         nMaxProgress)
        return IndicateAchievementProgress_native(this, pchName, nCurProgress,
                                                  nMaxProgress)
    end
    ISteamUserStats.indicate_achievement_progress =
        ISteamUserStats.IndicateAchievementProgress

    local GetNumAchievements_native = vtable_entry(this, 14,
                                                   "uint32_t(__thiscall*)(void*)")
    ...
    local GetAchievementName_native = vtable_entry(this, 15,
                                                   "const char *(__thiscall*)(void*, uint32_t)")
    ...
    local RequestUserStats_native = vtable_entry(this, 16,
                                                 "uint64_t(__thiscall*)(void*, SteamID)")
    ...
    function ISteamUserStats.RequestUserStats(steamIDUser, callback)
        ...
        if callback ~= nil then
            res = register_call_result(res, callback, 1101,
                                       RequestUserStats_info)
        end
    ...
    local GetUserStatInt32_native = vtable_entry(this, 17,
                                                 "bool(__thiscall*)(void*, SteamID, const char *, int32_t *)")
    function ISteamUserStats.GetUserStatInt32(steamIDUser, pchName)
        steamIDUser = to_steamid_required(steamIDUser,
                                          "steamid_user is required")
        local pData_out = new_int32_arr()
        local res = GetUserStatInt32_native(this, steamIDUser, pchName,
                                            pData_out)
    ...
    local GetUserStatFloat_native = vtable_entry(this, 18,
                                                 "bool(__thiscall*)(void*, SteamID, const char *, float *)")
    function ISteamUserStats.GetUserStatFloat(steamIDUser, pchName)
        steamIDUser = to_steamid_required(steamIDUser,
                                          "steamid_user is required")
        local pData_out = new_float_arr()
        local res = GetUserStatFloat_native(this, steamIDUser, pchName,
                                            pData_out)
    ...
    local GetUserAchievement_native = vtable_entry(this, 19,
                                                   "bool(__thiscall*)(void*, SteamID, const char *, bool *)")
    function ISteamUserStats.GetUserAchievement(steamIDUser, pchName)
        steamIDUser = to_steamid_required(steamIDUser,
                                          "steamid_user is required")
        local pbAchieved_out = new_bool_arr()
        local res = GetUserAchievement_native(this, steamIDUser, pchName,
                                              pbAchieved_out)
    ...
    local GetUserAchievementAndUnlockTime_native =
        vtable_entry(this, 20,
                     "bool(__thiscall*)(void*, SteamID, const char *, bool *, uint32_t *)")
    function ISteamUserStats.GetUserAchievementAndUnlockTime(steamIDUser,
                                                             pchName)
        steamIDUser = to_steamid_required(steamIDUser,
                                          "steamid_user is required")
        local pbAchieved_out = new_bool_arr()
        local punUnlockTime_out = new_uint32_arr()
        local res = GetUserAchievementAndUnlockTime_native(this, steamIDUser,
                                                           pchName,
                                                           pbAchieved_out,
                                                           punUnlockTime_out)
    ...
    ISteamUserStats.get_user_achievement_and_unlock_time =
        ISteamUserStats.GetUserAchievementAndUnlockTime

    local ResetAllStats_native = vtable_entry(this, 21,
                                              "bool(__thiscall*)(void*, bool)")
    ...
    local FindOrCreateLeaderboard_native =
        vtable_entry(this, 22,
                     "uint64_t(__thiscall*)(void*, const char *, int, int)")
    ...
    function ISteamUserStats.FindOrCreateLeaderboard(pchLeaderboardName,
                                                     eLeaderboardSortMethod,
                                                     eLeaderboardDisplayType,
                                                     callback)
        eLeaderboardSortMethod = to_enum_required(eLeaderboardSortMethod,
                                                  enums.ELeaderboardSortMethod,
                                                  "leaderboard_sort_method is required")
        eLeaderboardDisplayType = to_enum_required(eLeaderboardDisplayType,
                                                   enums.ELeaderboardDisplayType,
                                                   "leaderboard_display_type is required")
        if callback ~= nil and not is_valid_callback(callback) then
            return error("Invalid callback, expected function or await")
        end

        local res = FindOrCreateLeaderboard_native(this, pchLeaderboardName,
                                                   eLeaderboardSortMethod,
                                                   eLeaderboardDisplayType)

        if callback ~= nil then
            res = register_call_result(res, callback, 1104,
                                       FindOrCreateLeaderboard_info)
        end

        return res
    end
    ISteamUserStats.find_or_create_leaderboard =
        ISteamUserStats.FindOrCreateLeaderboard

    local FindLeaderboard_native = vtable_entry(this, 23,
                                                "uint64_t(__thiscall*)(void*, const char *)")
    ...
    function ISteamUserStats.FindLeaderboard(pchLeaderboardName, callback)
        ...
        if callback ~= nil then
            res =
                register_call_result(res, callback, 1104, FindLeaderboard_info)
        end
    ...
    local GetLeaderboardName_native = vtable_entry(this, 24,
                                                   "const char *(__thiscall*)(void*, uint64_t)")
    ...
    local GetLeaderboardEntryCount_native =
        vtable_entry(this, 25, "int(__thiscall*)(void*, uint64_t)")
    ...
    ISteamUserStats.get_leaderboard_entry_count =
        ISteamUserStats.GetLeaderboardEntryCount

    local GetLeaderboardSortMethod_native =
        vtable_entry(this, 26, "int(__thiscall*)(void*, uint64_t)")
    ...
    ISteamUserStats.get_leaderboard_sort_method =
        ISteamUserStats.GetLeaderboardSortMethod

    local GetLeaderboardDisplayType_native =
        vtable_entry(this, 27, "int(__thiscall*)(void*, uint64_t)")
    ...
    ISteamUserStats.get_leaderboard_display_type =
        ISteamUserStats.GetLeaderboardDisplayType

    local DownloadLeaderboardEntries_native =
        vtable_entry(this, 28,
                     "uint64_t(__thiscall*)(void*, uint64_t, int, int, int)")
    ...
    function ISteamUserStats.DownloadLeaderboardEntries(hSteamLeaderboard,
                                                        eLeaderboardDataRequest,
                                                        nRangeStart, nRangeEnd,
                                                        callback)
        eLeaderboardDataRequest = to_enum_required(eLeaderboardDataRequest,
                                                   enums.ELeaderboardDataRequest,
                                                   "leaderboard_data_request is required")
        if callback ~= nil and not is_valid_callback(callback) then
            return error("Invalid callback, expected function or await")
        end

        local res = DownloadLeaderboardEntries_native(this, hSteamLeaderboard,
                                                      eLeaderboardDataRequest,
                                                      nRangeStart, nRangeEnd)

        if callback ~= nil then
            res = register_call_result(res, callback, 1105,
                                       DownloadLeaderboardEntries_info)
        end

        return res
    end
    ISteamUserStats.download_leaderboard_entries =
        ISteamUserStats.DownloadLeaderboardEntries

    local DownloadLeaderboardEntriesForUsers_native =
        vtable_entry(this, 29,
                     "uint64_t(__thiscall*)(void*, uint64_t, SteamID *, int)")
    ...
    function ISteamUserStats.DownloadLeaderboardEntriesForUsers(
        hSteamLeaderboard, prgUsers, cUsers, callback)
        if callback ~= nil and not is_valid_callback(callback) then
            return error("Invalid callback, expected function or await")
        end

        local res = DownloadLeaderboardEntriesForUsers_native(this,
                                                              hSteamLeaderboard,
                                                              prgUsers, cUsers)

        if callback ~= nil then
            res = register_call_result(res, callback, 1105,
                                       DownloadLeaderboardEntriesForUsers_info)
        end

        return res
    end
    ISteamUserStats.download_leaderboard_entries_for_users =
        ISteamUserStats.DownloadLeaderboardEntriesForUsers

    local GetDownloadedLeaderboardEntry_native =
        vtable_entry(this, 30,
                     "bool(__thiscall*)(void*, uint64_t, int, LeaderboardEntry_t *, int32_t *, int)")
    function ISteamUserStats.GetDownloadedLeaderboardEntry(
        hSteamLeaderboardEntries, index, pDetails, cDetailsMax)
        local pLeaderboardEntry_out = structs.LeaderboardEntry_t_arr(1)
        local res = GetDownloadedLeaderboardEntry_native(this,
                                                         hSteamLeaderboardEntries,
                                                         index,
                                                         pLeaderboardEntry_out,
                                                         pDetails, cDetailsMax)

        return res, DEREF_GCSAFE(pLeaderboardEntry_out)
    end
    ISteamUserStats.get_downloaded_leaderboard_entry =
        ISteamUserStats.GetDownloadedLeaderboardEntry

    local UploadLeaderboardScore_native =
        vtable_entry(this, 31,
                     "uint64_t(__thiscall*)(void*, uint64_t, int, int32_t, const int32_t *, int)")
    ...
    function ISteamUserStats.UploadLeaderboardScore(hSteamLeaderboard,
                                                    eLeaderboardUploadScoreMethod,
                                                    nScore, pScoreDetails,
                                                    cScoreDetailsCount, callback)
        eLeaderboardUploadScoreMethod = to_enum_required(
                                            eLeaderboardUploadScoreMethod,
                                            enums.ELeaderboardUploadScoreMethod,
                                            "leaderboard_upload_score_method is required")
        if callback ~= nil and not is_valid_callback(callback) then
            return error("Invalid callback, expected function or await")
        end

        local res = UploadLeaderboardScore_native(this, hSteamLeaderboard,
                                                  eLeaderboardUploadScoreMethod,
                                                  nScore, pScoreDetails,
                                                  cScoreDetailsCount)

        if callback ~= nil then
            res = register_call_result(res, callback, 1106,
                                       UploadLeaderboardScore_info)
        end

        return res
    end
    ISteamUserStats.upload_leaderboard_score =
        ISteamUserStats.UploadLeaderboardScore

    local AttachLeaderboardUGC_native = vtable_entry(this, 32,
                                                     "uint64_t(__thiscall*)(void*, uint64_t, uint64_t)")
    ...
    function ISteamUserStats.AttachLeaderboardUGC(hSteamLeaderboard, hUGC,
                                                  callback)
        if callback ~= nil and not is_valid_callback(callback) then
            return error("Invalid callback, expected function or await")
        end
        ...
        if callback ~= nil then
            res = register_call_result(res, callback, 1111,
                                       AttachLeaderboardUGC_info)
        end

        return res
    end
    ISteamUserStats.attach_leaderboard_ugc =
        ISteamUserStats.AttachLeaderboardUGC

    local GetNumberOfCurrentPlayers_native =
        vtable_entry(this, 33, "uint64_t(__thiscall*)(void*)")
    ...
    function ISteamUserStats.GetNumberOfCurrentPlayers(callback)
        ...
        if callback ~= nil then
            res = register_call_result(res, callback, 1107,
                                       GetNumberOfCurrentPlayers_info)
        end
    ...
    local RequestGlobalAchievementPercentages_native =
        vtable_entry(this, 34, "uint64_t(__thiscall*)(void*)")
    ...
    function ISteamUserStats.RequestGlobalAchievementPercentages(callback)
        ...
        if callback ~= nil then
            res = register_call_result(res, callback, 1110,
                                       RequestGlobalAchievementPercentages_info)
        end

        return res
    end
    ISteamUserStats.request_global_achievement_percentages =
        ISteamUserStats.RequestGlobalAchievementPercentages

    local GetMostAchievedAchievementInfo_native =
        vtable_entry(this, 35,
                     "int(__thiscall*)(void*, char *, uint32_t, float *, bool *)")
    function ISteamUserStats.GetMostAchievedAchievementInfo(pchName,
                                                            unNameBufLen)
        local pflPercent_out = new_float_arr()
        local pbAchieved_out = new_bool_arr()
        local res = GetMostAchievedAchievementInfo_native(this, pchName,
                                                          unNameBufLen,
                                                          pflPercent_out,
                                                          pbAchieved_out)
    ...
    ISteamUserStats.get_most_achieved_achievement_info =
        ISteamUserStats.GetMostAchievedAchievementInfo

    local GetNextMostAchievedAchievementInfo_native =
        vtable_entry(this, 36,
                     "int(__thiscall*)(void*, int, char *, uint32_t, float *, bool *)")
    function ISteamUserStats.GetNextMostAchievedAchievementInfo(
        iIteratorPrevious, pchName, unNameBufLen)
        local pflPercent_out = new_float_arr()
        local pbAchieved_out = new_bool_arr()
        local res = GetNextMostAchievedAchievementInfo_native(this,
                                                              iIteratorPrevious,
                                                              pchName,
                                                              unNameBufLen,
                                                              pflPercent_out,
    ...
    ISteamUserStats.get_next_most_achieved_achievement_info =
        ISteamUserStats.GetNextMostAchievedAchievementInfo

    local GetAchievementAchievedPercent_native =
        vtable_entry(this, 37, "bool(__thiscall*)(void*, const char *, float *)")
    function ISteamUserStats.GetAchievementAchievedPercent(pchName)
        local pflPercent_out = new_float_arr()
        local res = GetAchievementAchievedPercent_native(this, pchName,
                                                         pflPercent_out)

        return res, DEREF_GCSAFE(pflPercent_out)
    end
    ISteamUserStats.get_achievement_achieved_percent =
        ISteamUserStats.GetAchievementAchievedPercent

    local RequestGlobalStats_native = vtable_entry(this, 38,
                                                   "uint64_t(__thiscall*)(void*, int)")
    ...
    function ISteamUserStats.RequestGlobalStats(nHistoryDays, callback)
        ...
        if callback ~= nil then
            res = register_call_result(res, callback, 1112,
                                       RequestGlobalStats_info)
        end
    ...
    local GetGlobalStatInt64_native = vtable_entry(this, 39,
                                                   "bool(__thiscall*)(void*, const char *, int64_t *)")
    ...
    local GetGlobalStatDouble_native = vtable_entry(this, 40,
                                                    "bool(__thiscall*)(void*, const char *, double *)")
    ...
    local GetGlobalStatHistoryInt64_native =
        vtable_entry(this, 41,
                     "int32_t(__thiscall*)(void*, const char *, int64_t *, uint32_t)")
    function ISteamUserStats.GetGlobalStatHistoryInt64(pchStatName, pData,
                                                       cubData)
        return GetGlobalStatHistoryInt64_native(this, pchStatName, pData,
                                                cubData)
    end
    ISteamUserStats.get_global_stat_history_int64 =
        ISteamUserStats.GetGlobalStatHistoryInt64

    local GetGlobalStatHistoryDouble_native =
        vtable_entry(this, 42,
                     "int32_t(__thiscall*)(void*, const char *, double *, uint32_t)")
    function ISteamUserStats.GetGlobalStatHistoryDouble(pchStatName, pData,
                                                        cubData)
        return GetGlobalStatHistoryDouble_native(this, pchStatName, pData,
                                                 cubData)
    end
    ISteamUserStats.get_global_stat_history_double =
        ISteamUserStats.GetGlobalStatHistoryDouble

    local GetAchievementProgressLimitsInt32_native =
        vtable_entry(this, 43,
                     "bool(__thiscall*)(void*, const char *, int32_t *, int32_t *)")
    function ISteamUserStats.GetAchievementProgressLimitsInt32(pchName)
        ...
        local res = GetAchievementProgressLimitsInt32_native(this, pchName,
                                                             pnMinProgress_out,
                                                             pnMaxProgress_out)

        return res, DEREF_GCSAFE(pnMinProgress_out),
               DEREF_GCSAFE(pnMaxProgress_out)
    end
    ISteamUserStats.get_achievement_progress_limits_int32 =
        ISteamUserStats.GetAchievementProgressLimitsInt32

    local GetAchievementProgressLimitsFloat_native =
        vtable_entry(this, 44,
                     "bool(__thiscall*)(void*, const char *, float *, float *)")
    function ISteamUserStats.GetAchievementProgressLimitsFloat(pchName)
        ...
        local res = GetAchievementProgressLimitsFloat_native(this, pchName,
                                                             pfMinProgress_out,
                                                             pfMaxProgress_out)

        return res, DEREF_GCSAFE(pfMinProgress_out),
               DEREF_GCSAFE(pfMaxProgress_out)
    end
    ISteamUserStats.get_achievement_progress_limits_float =
        ISteamUserStats.GetAchievementProgressLimitsFloat
...
index_funcs.ISteamApps = function()
    local this = get_steam_vtable_bind("steam_api.dll", "SteamClient", 15,
                             "void*(__thiscall*)(void*, int, int, const char *)")(
                     hSteamUser, hSteamPipe, "STEAMAPPS_INTERFACE_VERSION008")

    local BIsSubscribed_native = vtable_entry(this, 0,
                                              "bool(__thiscall*)(void*)")
    function ISteamApps.BIsSubscribed() return BIsSubscribed_native(this) end
    ISteamApps.is_subscribed = ISteamApps.BIsSubscribed

    local BIsLowViolence_native = vtable_entry(this, 1,
                                               "bool(__thiscall*)(void*)")
    function ISteamApps.BIsLowViolence() return BIsLowViolence_native(this) end
    ISteamApps.is_low_violence = ISteamApps.BIsLowViolence

    local BIsCybercafe_native =
        vtable_entry(this, 2, "bool(__thiscall*)(void*)")
    function ISteamApps.BIsCybercafe() return BIsCybercafe_native(this) end
    ISteamApps.is_cybercafe = ISteamApps.BIsCybercafe

    local BIsVACBanned_native =
        vtable_entry(this, 3, "bool(__thiscall*)(void*)")
    function ISteamApps.BIsVACBanned() return BIsVACBanned_native(this) end
    ISteamApps.is_vac_banned = ISteamApps.BIsVACBanned

    local GetCurrentGameLanguage_native =
        vtable_entry(this, 4, "const char *(__thiscall*)(void*)")
    ...
    local GetAvailableGameLanguages_native =
        vtable_entry(this, 5, "const char *(__thiscall*)(void*)")
    function ISteamApps.GetAvailableGameLanguages()
        local res = GetAvailableGameLanguages_native(this)

        return res ~= nil and ffi_string(res) or nil
    end
    ISteamApps.get_available_game_languages =
        ISteamApps.GetAvailableGameLanguages

    local BIsSubscribedApp_native = vtable_entry(this, 6,
                                                 "bool(__thiscall*)(void*, unsigned int)")
    ...
    local BIsDlcInstalled_native = vtable_entry(this, 7,
                                                "bool(__thiscall*)(void*, unsigned int)")
    ...
    local GetEarliestPurchaseUnixTime_native =
        vtable_entry(this, 8, "uint32_t(__thiscall*)(void*, unsigned int)")
    function ISteamApps.GetEarliestPurchaseUnixTime(nAppID)
        return GetEarliestPurchaseUnixTime_native(this, nAppID)
    end
    ISteamApps.get_earliest_purchase_unix_time =
        ISteamApps.GetEarliestPurchaseUnixTime

    local BIsSubscribedFromFreeWeekend_native =
        vtable_entry(this, 9, "bool(__thiscall*)(void*)")
    function ISteamApps.BIsSubscribedFromFreeWeekend()
        return BIsSubscribedFromFreeWeekend_native(this)
    end
    ISteamApps.is_subscribed_from_free_weekend =
        ISteamApps.BIsSubscribedFromFreeWeekend

    local GetDLCCount_native = vtable_entry(this, 10, "int(__thiscall*)(void*)")
    function ISteamApps.GetDLCCount() return GetDLCCount_native(this) end
    ISteamApps.get_dlc_count = ISteamApps.GetDLCCount

    local BGetDLCDataByIndex_native = vtable_entry(this, 11,
                                                   "bool(__thiscall*)(void*, int, unsigned int *, bool *, char *, int)")
    function ISteamApps.BGetDLCDataByIndex(iDLC, pchName, cchNameBufferSize)
        ...
        local res = BGetDLCDataByIndex_native(this, iDLC, pAppID_out,
                                              pbAvailable_out, pchName,
                                              cchNameBufferSize)
    ...
    local InstallDLC_native = vtable_entry(this, 12,
                                           "void(__thiscall*)(void*, unsigned int)")
    ...
    local UninstallDLC_native = vtable_entry(this, 13,
                                             "void(__thiscall*)(void*, unsigned int)")
    ...
    local RequestAppProofOfPurchaseKey_native =
        vtable_entry(this, 14, "void(__thiscall*)(void*, unsigned int)")
    function ISteamApps.RequestAppProofOfPurchaseKey(nAppID)
        return RequestAppProofOfPurchaseKey_native(this, nAppID)
    end
    ISteamApps.request_app_proof_of_purchase_key =
        ISteamApps.RequestAppProofOfPurchaseKey

    local GetCurrentBetaName_native = vtable_entry(this, 15,
                                                   "bool(__thiscall*)(void*, char *, int)")
    ...
    local MarkContentCorrupt_native = vtable_entry(this, 16,
                                                   "bool(__thiscall*)(void*, bool)")
    ...
    local GetInstalledDepots_native = vtable_entry(this, 17,
                                                   "uint32_t(__thiscall*)(void*, unsigned int, unsigned int *, uint32_t)")
    ...
    local GetAppInstallDir_native = vtable_entry(this, 18,
                                                 "uint32_t(__thiscall*)(void*, unsigned int, char *, uint32_t)")
    function ISteamApps.GetAppInstallDir(appID, pchFolder, cchFolderBufferSize)
        return GetAppInstallDir_native(this, appID, pchFolder,
                                       cchFolderBufferSize)
    end
    ISteamApps.get_app_install_dir = ISteamApps.GetAppInstallDir

    local BIsAppInstalled_native = vtable_entry(this, 19,
                                                "bool(__thiscall*)(void*, unsigned int)")
    ...
    local GetAppOwner_native = vtable_entry(this, 20,
                                            "void(__thiscall*)(void*, SteamID *)")
    ...
    local GetLaunchQueryParam_native = vtable_entry(this, 21,
                                                    "const char *(__thiscall*)(void*, const char *)")
    ...
    local GetDlcDownloadProgress_native =
        vtable_entry(this, 22,
                     "bool(__thiscall*)(void*, unsigned int, uint64_t *, uint64_t *)")
    function ISteamApps.GetDlcDownloadProgress(nAppID)
        ...
        local res = GetDlcDownloadProgress_native(this, nAppID,
                                                  punBytesDownloaded_out,
                                                  punBytesTotal_out)

        return res, DEREF_GCSAFE(punBytesDownloaded_out),
               DEREF_GCSAFE(punBytesTotal_out)
    end
    ISteamApps.get_dlc_download_progress = ISteamApps.GetDlcDownloadProgress

    local GetAppBuildId_native = vtable_entry(this, 23,
                                              "int(__thiscall*)(void*)")
    function ISteamApps.GetAppBuildId() return GetAppBuildId_native(this) end
    ISteamApps.get_app_build_id = ISteamApps.GetAppBuildId

    local RequestAllProofOfPurchaseKeys_native =
        vtable_entry(this, 24, "void(__thiscall*)(void*)")
    function ISteamApps.RequestAllProofOfPurchaseKeys()
        return RequestAllProofOfPurchaseKeys_native(this)
    end
    ISteamApps.request_all_proof_of_purchase_keys =
        ISteamApps.RequestAllProofOfPurchaseKeys

    local GetFileDetails_native = vtable_entry(this, 25,
                                               "uint64_t(__thiscall*)(void*, const char *)")
    ...
    local GetLaunchCommandLine_native = vtable_entry(this, 26,
    ...
    local BIsSubscribedFromFamilySharing_native =
        vtable_entry(this, 27, "bool(__thiscall*)(void*)")
    function ISteamApps.BIsSubscribedFromFamilySharing()
        return BIsSubscribedFromFamilySharing_native(this)
    end
    ISteamApps.is_subscribed_from_family_sharing =
        ISteamApps.BIsSubscribedFromFamilySharing

    local BIsTimedTrial_native = vtable_entry(this, 28,
                                              "bool(__thiscall*)(void*, uint32_t *, uint32_t *)")
    function ISteamApps.BIsTimedTrial()
        local punSecondsAllowed_out = new_uint32_arr()
        local punSecondsPlayed_out = new_uint32_arr()
        local res = BIsTimedTrial_native(this, punSecondsAllowed_out,
                                         punSecondsPlayed_out)

        return res, DEREF_GCSAFE(punSecondsAllowed_out),
               DEREF_GCSAFE(punSecondsPlayed_out)
    end
...
index_funcs.ISteamNetworking = function()
    local this = get_steam_vtable_bind("steam_api.dll", "SteamClient", 16,
                             "void*(__thiscall*)(void*, int, int, const char *)")(
                     hSteamUser, hSteamPipe, "SteamNetworking006")

    local SendP2PPacket_native = vtable_entry(this, 0,
                                              "bool(__thiscall*)(void*, SteamID, const void *, uint32_t, int, int)")
    function ISteamNetworking.SendP2PPacket(steamIDRemote, pubData, cubData,
                                            eP2PSendType, nChannel)
        steamIDRemote = to_steamid_required(steamIDRemote,
                                            "steamid_remote is required")
        eP2PSendType = to_enum_required(eP2PSendType, enums.EP2PSend,
                                        "p2p_send_type is required")
        return SendP2PPacket_native(this, steamIDRemote, pubData, cubData,
                                    eP2PSendType, nChannel)
    end
    ISteamNetworking.send_p2p_packet = ISteamNetworking.SendP2PPacket

    local IsP2PPacketAvailable_native = vtable_entry(this, 1,
                                                     "bool(__thiscall*)(void*, uint32_t *, int)")
    ...
    ISteamNetworking.is_p2p_packet_available =
        ISteamNetworking.IsP2PPacketAvailable

    local ReadP2PPacket_native = vtable_entry(this, 2,
                                              "bool(__thiscall*)(void*, void *, uint32_t, uint32_t *, SteamID *, int)")
    function ISteamNetworking.ReadP2PPacket(pubDest, cubDest, psteamIDRemote,
                                            nChannel)
        local pcubMsgSize_out = new_uint32_arr()
        local res = ReadP2PPacket_native(this, pubDest, cubDest,
                                         pcubMsgSize_out, psteamIDRemote,
                                         nChannel)
    ...
    local AcceptP2PSessionWithUser_native =
        vtable_entry(this, 3, "bool(__thiscall*)(void*, SteamID)")
    ...
    ISteamNetworking.accept_p2p_session_with_user =
        ISteamNetworking.AcceptP2PSessionWithUser

    local CloseP2PSessionWithUser_native =
        vtable_entry(this, 4, "bool(__thiscall*)(void*, SteamID)")
    function ISteamNetworking.CloseP2PSessionWithUser(steamIDRemote)
        steamIDRemote = to_steamid_required(steamIDRemote,
                                            "steamid_remote is required")
        return CloseP2PSessionWithUser_native(this, steamIDRemote)
    end
    ISteamNetworking.close_p2p_session_with_user =
        ISteamNetworking.CloseP2PSessionWithUser

    local CloseP2PChannelWithUser_native =
        vtable_entry(this, 5, "bool(__thiscall*)(void*, SteamID, int)")
    ...
    ISteamNetworking.close_p2p_channel_with_user =
        ISteamNetworking.CloseP2PChannelWithUser

    local GetP2PSessionState_native = vtable_entry(this, 6,
                                                   "bool(__thiscall*)(void*, SteamID, P2PSessionState_t *)")
    function ISteamNetworking.GetP2PSessionState(steamIDRemote)
        steamIDRemote = to_steamid_required(steamIDRemote,
                                            "steamid_remote is required")
        local pConnectionState_out = structs.P2PSessionState_t_arr(1)
        local res = GetP2PSessionState_native(this, steamIDRemote,
                                              pConnectionState_out)

        return res, DEREF_GCSAFE(pConnectionState_out)
    end
    ISteamNetworking.get_p2p_session_state = ISteamNetworking.GetP2PSessionState

    local AllowP2PPacketRelay_native = vtable_entry(this, 7,
                                                    "bool(__thiscall*)(void*, bool)")
    function ISteamNetworking.AllowP2PPacketRelay(bAllow)
        return AllowP2PPacketRelay_native(this, bAllow)
    end
    ISteamNetworking.allow_p2p_packet_relay =
        ISteamNetworking.AllowP2PPacketRelay

    local CreateListenSocket_native = vtable_entry(this, 8,
                                                   "unsigned int(__thiscall*)(void*, int, SteamIPAddress_t, uint16_t, bool)")
    function ISteamNetworking.CreateListenSocket(nVirtualP2PPort, nIP, nPort,
                                                 bAllowUseOfPacketRelay)
        return CreateListenSocket_native(this, nVirtualP2PPort, nIP, nPort,
                                         bAllowUseOfPacketRelay)
    end
    ISteamNetworking.create_listen_socket = ISteamNetworking.CreateListenSocket

    local CreateP2PConnectionSocket_native =
        vtable_entry(this, 9,
                     "unsigned int(__thiscall*)(void*, SteamID, int, int, bool)")
    function ISteamNetworking.CreateP2PConnectionSocket(steamIDTarget,
                                                        nVirtualPort,
                                                        nTimeoutSec,
                                                        bAllowUseOfPacketRelay)
        steamIDTarget = to_steamid_required(steamIDTarget,
                                            "steamid_target is required")
        return CreateP2PConnectionSocket_native(this, steamIDTarget,
                                                nVirtualPort, nTimeoutSec,
                                                bAllowUseOfPacketRelay)
    end
    ISteamNetworking.create_p2p_connection_socket =
        ISteamNetworking.CreateP2PConnectionSocket

    local CreateConnectionSocket_native =
        vtable_entry(this, 10,
                     "unsigned int(__thiscall*)(void*, SteamIPAddress_t, uint16_t, int)")
    ...
    ISteamNetworking.create_connection_socket =
        ISteamNetworking.CreateConnectionSocket

    local DestroySocket_native = vtable_entry(this, 11,
                                              "bool(__thiscall*)(void*, unsigned int, bool)")
    ...
    local DestroyListenSocket_native = vtable_entry(this, 12,
                                                    "bool(__thiscall*)(void*, unsigned int, bool)")
    ...
    ISteamNetworking.destroy_listen_socket =
        ISteamNetworking.DestroyListenSocket

    local SendDataOnSocket_native = vtable_entry(this, 13,
                                                 "bool(__thiscall*)(void*, unsigned int, void *, uint32_t, bool)")
    function ISteamNetworking.SendDataOnSocket(hSocket, pubData, cubData,
                                               bReliable)
        return SendDataOnSocket_native(this, hSocket, pubData, cubData,
                                       bReliable)
    end
    ISteamNetworking.send_data_on_socket = ISteamNetworking.SendDataOnSocket

    local IsDataAvailableOnSocket_native =
        vtable_entry(this, 14,
                     "bool(__thiscall*)(void*, unsigned int, uint32_t *)")
    function ISteamNetworking.IsDataAvailableOnSocket(hSocket)
        local pcubMsgSize_out = new_uint32_arr()
        local res = IsDataAvailableOnSocket_native(this, hSocket,
                                                   pcubMsgSize_out)

        return res, DEREF_GCSAFE(pcubMsgSize_out)
    end
    ISteamNetworking.is_data_available_on_socket =
        ISteamNetworking.IsDataAvailableOnSocket

    local RetrieveDataFromSocket_native =
        vtable_entry(this, 15,
                     "bool(__thiscall*)(void*, unsigned int, void *, uint32_t, uint32_t *)")
    function ISteamNetworking.RetrieveDataFromSocket(hSocket, pubDest, cubDest)
        local pcubMsgSize_out = new_uint32_arr()
        local res = RetrieveDataFromSocket_native(this, hSocket, pubDest,
                                                  cubDest, pcubMsgSize_out)

        return res, DEREF_GCSAFE(pcubMsgSize_out)
    end
    ISteamNetworking.retrieve_data_from_socket =
        ISteamNetworking.RetrieveDataFromSocket

    local IsDataAvailable_native = vtable_entry(this, 16,
                                                "bool(__thiscall*)(void*, unsigned int, uint32_t *, unsigned int *)")
    function ISteamNetworking.IsDataAvailable(hListenSocket)
        local pcubMsgSize_out = new_uint32_arr()
        local phSocket_out = new_unsigned_int_arr()
        local res = IsDataAvailable_native(this, hListenSocket, pcubMsgSize_out,
                                           phSocket_out)
    ...
    local RetrieveData_native = vtable_entry(this, 17,
                                             "bool(__thiscall*)(void*, unsigned int, void *, uint32_t, uint32_t *, unsigned int *)")
    function ISteamNetworking.RetrieveData(hListenSocket, pubDest, cubDest)
        local pcubMsgSize_out = new_uint32_arr()
        local phSocket_out = new_unsigned_int_arr()
        local res = RetrieveData_native(this, hListenSocket, pubDest, cubDest,
                                        pcubMsgSize_out, phSocket_out)

        return res, DEREF_GCSAFE(pcubMsgSize_out), DEREF_GCSAFE(phSocket_out)
    end
    ISteamNetworking.retrieve_data = ISteamNetworking.RetrieveData

    local GetSocketInfo_native = vtable_entry(this, 18,
                                              "bool(__thiscall*)(void*, unsigned int, SteamID *, int *, SteamIPAddress_t *, uint16_t *)")
    function ISteamNetworking.GetSocketInfo(hSocket, pSteamIDRemote)
        ...
        local res = GetSocketInfo_native(this, hSocket, pSteamIDRemote,
                                         peSocketStatus_out, punIPRemote_out,
                                         punPortRemote_out)

        return res, DEREF_GCSAFE(peSocketStatus_out),
               DEREF_GCSAFE(punIPRemote_out), DEREF_GCSAFE(punPortRemote_out)
    end
    ISteamNetworking.get_socket_info = ISteamNetworking.GetSocketInfo

    local GetListenSocketInfo_native = vtable_entry(this, 19,
                                                    "bool(__thiscall*)(void*, unsigned int, SteamIPAddress_t *, uint16_t *)")
    function ISteamNetworking.GetListenSocketInfo(hListenSocket)
        ...
        local res = GetListenSocketInfo_native(this, hListenSocket, pnIP_out,
                                               pnPort_out)

        return res, DEREF_GCSAFE(pnIP_out), DEREF_GCSAFE(pnPort_out)
    end
    ISteamNetworking.get_listen_socket_info =
        ISteamNetworking.GetListenSocketInfo

    local GetSocketConnectionType_native =
        vtable_entry(this, 20, "int(__thiscall*)(void*, unsigned int)")
    function ISteamNetworking.GetSocketConnectionType(hSocket)
        return GetSocketConnectionType_native(this, hSocket)
    end
    ISteamNetworking.get_socket_connection_type =
        ISteamNetworking.GetSocketConnectionType

    local GetMaxPacketSize_native = vtable_entry(this, 21,
                                                 "int(__thiscall*)(void*, unsigned int)")
...
local ISteamScreenshots = {
    version = "STEAMSCREENSHOTS_INTERFACE_VERSION003",
    version_number = 3
}

index_funcs.ISteamScreenshots = function()
    local this = get_steam_vtable_bind("steam_api.dll", "SteamClient", 18,
                             "void*(__thiscall*)(void*, int, int, const char *)")(
                     hSteamUser, hSteamPipe,
                     "STEAMSCREENSHOTS_INTERFACE_VERSION003")

    local WriteScreenshot_native = vtable_entry(this, 0,
                                                "unsigned int(__thiscall*)(void*, void *, uint32_t, int, int)")
    ...
    local AddScreenshotToLibrary_native =
        vtable_entry(this, 1,
                     "unsigned int(__thiscall*)(void*, const char *, const char *, int, int)")
    function ISteamScreenshots.AddScreenshotToLibrary(pchFilename,
                                                      pchThumbnailFilename,
                                                      nWidth, nHeight)
        return AddScreenshotToLibrary_native(this, pchFilename,
                                             pchThumbnailFilename, nWidth,
                                             nHeight)
    end
    ISteamScreenshots.add_screenshot_to_library =
        ISteamScreenshots.AddScreenshotToLibrary

    local TriggerScreenshot_native = vtable_entry(this, 2,
                                                  "void(__thiscall*)(void*)")
    ...
    local HookScreenshots_native = vtable_entry(this, 3,
                                                "void(__thiscall*)(void*, bool)")
    ...
    local SetLocation_native = vtable_entry(this, 4,
                                            "bool(__thiscall*)(void*, unsigned int, const char *)")
    ...
    local TagUser_native = vtable_entry(this, 5,
                                        "bool(__thiscall*)(void*, unsigned int, SteamID)")
    ...
    local TagPublishedFile_native = vtable_entry(this, 6,
                                                 "bool(__thiscall*)(void*, unsigned int, uint64_t)")
    ...
    local IsScreenshotsHooked_native = vtable_entry(this, 7,
                                                    "bool(__thiscall*)(void*)")
    function ISteamScreenshots.IsScreenshotsHooked()
        return IsScreenshotsHooked_native(this)
    end
    ISteamScreenshots.is_screenshots_hooked =
        ISteamScreenshots.IsScreenshotsHooked

    local AddVRScreenshotToLibrary_native =
        vtable_entry(this, 8,
                     "unsigned int(__thiscall*)(void*, int, const char *, const char *)")
    function ISteamScreenshots.AddVRScreenshotToLibrary(eType, pchFilename,
                                                        pchVRFilename)
        eType = to_enum_required(eType, enums.EVRScreenshotType,
                                 "type is required")
        return AddVRScreenshotToLibrary_native(this, eType, pchFilename,
                                               pchVRFilename)
    end
    ISteamScreenshots.add_vr_screenshot_to_library =
...
index_funcs.ISteamMusic = function()
    local this = get_steam_vtable_bind("steam_api.dll", "SteamClient", 29,
                             "void*(__thiscall*)(void*, int, int, const char *)")(
                     hSteamUser, hSteamPipe, "STEAMMUSIC_INTERFACE_VERSION001")

    local BIsEnabled_native = vtable_entry(this, 0, "bool(__thiscall*)(void*)")
    function ISteamMusic.BIsEnabled() return BIsEnabled_native(this) end
    ISteamMusic.is_enabled = ISteamMusic.BIsEnabled

    local BIsPlaying_native = vtable_entry(this, 1, "bool(__thiscall*)(void*)")
    function ISteamMusic.BIsPlaying() return BIsPlaying_native(this) end
    ISteamMusic.is_playing = ISteamMusic.BIsPlaying

    local GetPlaybackStatus_native = vtable_entry(this, 2,
                                                  "int(__thiscall*)(void*)")
    ...
    function ISteamMusic.Play() return Play_native(this) end
    ISteamMusic.play = ISteamMusic.Play

    local Pause_native = vtable_entry(this, 4, "void(__thiscall*)(void*)")
    function ISteamMusic.Pause() return Pause_native(this) end
    ISteamMusic.pause = ISteamMusic.Pause

    local PlayPrevious_native =
        vtable_entry(this, 5, "void(__thiscall*)(void*)")
    function ISteamMusic.PlayPrevious() return PlayPrevious_native(this) end
    ISteamMusic.play_previous = ISteamMusic.PlayPrevious

    local PlayNext_native = vtable_entry(this, 6, "void(__thiscall*)(void*)")
    function ISteamMusic.PlayNext() return PlayNext_native(this) end
    ISteamMusic.play_next = ISteamMusic.PlayNext

    local SetVolume_native = vtable_entry(this, 7,
    ...
    function ISteamMusic.GetVolume() return GetVolume_native(this) end
...
local ISteamMusicRemote = {
    version = "STEAMMUSICREMOTE_INTERFACE_VERSION001",
    version_number = 1
}

index_funcs.ISteamMusicRemote = function()
    local this = get_steam_vtable_bind("steam_api.dll", "SteamClient", 30,
                             "void*(__thiscall*)(void*, int, int, const char *)")(
                     hSteamUser, hSteamPipe,
                     "STEAMMUSICREMOTE_INTERFACE_VERSION001")

    local RegisterSteamMusicRemote_native =
        vtable_entry(this, 0, "bool(__thiscall*)(void*, const char *)")
    function ISteamMusicRemote.RegisterSteamMusicRemote(pchName)
        return RegisterSteamMusicRemote_native(this, pchName)
    end
    ISteamMusicRemote.register_steam_music_remote =
        ISteamMusicRemote.RegisterSteamMusicRemote

    local DeregisterSteamMusicRemote_native =
        vtable_entry(this, 1, "bool(__thiscall*)(void*)")
    function ISteamMusicRemote.DeregisterSteamMusicRemote()
        return DeregisterSteamMusicRemote_native(this)
    end
    ISteamMusicRemote.deregister_steam_music_remote =
        ISteamMusicRemote.DeregisterSteamMusicRemote

    local BIsCurrentMusicRemote_native =
        vtable_entry(this, 2, "bool(__thiscall*)(void*)")
    function ISteamMusicRemote.BIsCurrentMusicRemote()
        return BIsCurrentMusicRemote_native(this)
    end
    ISteamMusicRemote.is_current_music_remote =
        ISteamMusicRemote.BIsCurrentMusicRemote

    local BActivationSuccess_native = vtable_entry(this, 3,
                                                   "bool(__thiscall*)(void*, bool)")
    ...
    local SetDisplayName_native = vtable_entry(this, 4,
                                               "bool(__thiscall*)(void*, const char *)")
    ...
    local SetPNGIcon_64x64_native = vtable_entry(this, 5,
                                                 "bool(__thiscall*)(void*, void *, uint32_t)")
    ...
    local EnablePlayPrevious_native = vtable_entry(this, 6,
                                                   "bool(__thiscall*)(void*, bool)")
    function ISteamMusicRemote.EnablePlayPrevious(bValue)
        return EnablePlayPrevious_native(this, bValue)
    end
    ISteamMusicRemote.enable_play_previous =
        ISteamMusicRemote.EnablePlayPrevious

    local EnablePlayNext_native = vtable_entry(this, 7,
                                               "bool(__thiscall*)(void*, bool)")
    ...
    local EnableShuffled_native = vtable_entry(this, 8,
                                               "bool(__thiscall*)(void*, bool)")
    ...
    local EnableLooped_native = vtable_entry(this, 9,
                                             "bool(__thiscall*)(void*, bool)")
    ...
    local EnableQueue_native = vtable_entry(this, 10,
                                            "bool(__thiscall*)(void*, bool)")
    ...
    local EnablePlaylists_native = vtable_entry(this, 11,
                                                "bool(__thiscall*)(void*, bool)")
    ...
    local UpdatePlaybackStatus_native = vtable_entry(this, 12,
                                                     "bool(__thiscall*)(void*, int)")
    function ISteamMusicRemote.UpdatePlaybackStatus(nStatus)
        nStatus = to_enum_required(nStatus, enums.AudioPlayback_Status,
                                   "status is required")
        return UpdatePlaybackStatus_native(this, nStatus)
    end
    ISteamMusicRemote.update_playback_status =
        ISteamMusicRemote.UpdatePlaybackStatus

    local UpdateShuffled_native = vtable_entry(this, 13,
                                               "bool(__thiscall*)(void*, bool)")
    ...
    local UpdateLooped_native = vtable_entry(this, 14,
                                             "bool(__thiscall*)(void*, bool)")
    ...
    local UpdateVolume_native = vtable_entry(this, 15,
                                             "bool(__thiscall*)(void*, float)")
    ...
    local CurrentEntryWillChange_native =
        vtable_entry(this, 16, "bool(__thiscall*)(void*)")
    function ISteamMusicRemote.CurrentEntryWillChange()
        return CurrentEntryWillChange_native(this)
    end
    ISteamMusicRemote.current_entry_will_change =
        ISteamMusicRemote.CurrentEntryWillChange

    local CurrentEntryIsAvailable_native =
        vtable_entry(this, 17, "bool(__thiscall*)(void*, bool)")
    function ISteamMusicRemote.CurrentEntryIsAvailable(bAvailable)
        return CurrentEntryIsAvailable_native(this, bAvailable)
    end
    ISteamMusicRemote.current_entry_is_available =
        ISteamMusicRemote.CurrentEntryIsAvailable

    local UpdateCurrentEntryText_native =
        vtable_entry(this, 18, "bool(__thiscall*)(void*, const char *)")
    function ISteamMusicRemote.UpdateCurrentEntryText(pchText)
        return UpdateCurrentEntryText_native(this, pchText)
    end
    ISteamMusicRemote.update_current_entry_text =
        ISteamMusicRemote.UpdateCurrentEntryText

    local UpdateCurrentEntryElapsedSeconds_native =
        vtable_entry(this, 19, "bool(__thiscall*)(void*, int)")
    ...
    ISteamMusicRemote.update_current_entry_elapsed_seconds =
        ISteamMusicRemote.UpdateCurrentEntryElapsedSeconds

    local UpdateCurrentEntryCoverArt_native =
        vtable_entry(this, 20, "bool(__thiscall*)(void*, void *, uint32_t)")
    function ISteamMusicRemote.UpdateCurrentEntryCoverArt(pvBuffer,
                                                          cbBufferLength)
        return UpdateCurrentEntryCoverArt_native(this, pvBuffer, cbBufferLength)
    end
    ISteamMusicRemote.update_current_entry_cover_art =
        ISteamMusicRemote.UpdateCurrentEntryCoverArt

    local CurrentEntryDidChange_native =
        vtable_entry(this, 21, "bool(__thiscall*)(void*)")
    function ISteamMusicRemote.CurrentEntryDidChange()
        return CurrentEntryDidChange_native(this)
    end
    ISteamMusicRemote.current_entry_did_change =
        ISteamMusicRemote.CurrentEntryDidChange

    local QueueWillChange_native = vtable_entry(this, 22,
                                                "bool(__thiscall*)(void*)")
    ...
    local ResetQueueEntries_native = vtable_entry(this, 23,
                                                  "bool(__thiscall*)(void*)")
    ...
    local SetQueueEntry_native = vtable_entry(this, 24,
                                              "bool(__thiscall*)(void*, int, int, const char *)")
    ...
    local SetCurrentQueueEntry_native = vtable_entry(this, 25,
                                                     "bool(__thiscall*)(void*, int)")
    function ISteamMusicRemote.SetCurrentQueueEntry(nID)
        return SetCurrentQueueEntry_native(this, nID)
    end
    ISteamMusicRemote.set_current_queue_entry =
        ISteamMusicRemote.SetCurrentQueueEntry

    local QueueDidChange_native = vtable_entry(this, 26,
                                               "bool(__thiscall*)(void*)")
    ...
    local PlaylistWillChange_native = vtable_entry(this, 27,
                                                   "bool(__thiscall*)(void*)")
    function ISteamMusicRemote.PlaylistWillChange()
        return PlaylistWillChange_native(this)
    end
    ISteamMusicRemote.playlist_will_change =
        ISteamMusicRemote.PlaylistWillChange

    local ResetPlaylistEntries_native = vtable_entry(this, 28,
                                                     "bool(__thiscall*)(void*)")
    function ISteamMusicRemote.ResetPlaylistEntries()
        return ResetPlaylistEntries_native(this)
    end
    ISteamMusicRemote.reset_playlist_entries =
        ISteamMusicRemote.ResetPlaylistEntries

    local SetPlaylistEntry_native = vtable_entry(this, 29,
                                                 "bool(__thiscall*)(void*, int, int, const char *)")
    ...
    local SetCurrentPlaylistEntry_native =
        vtable_entry(this, 30, "bool(__thiscall*)(void*, int)")
    function ISteamMusicRemote.SetCurrentPlaylistEntry(nID)
        return SetCurrentPlaylistEntry_native(this, nID)
    end
    ISteamMusicRemote.set_current_playlist_entry =
        ISteamMusicRemote.SetCurrentPlaylistEntry

    local PlaylistDidChange_native = vtable_entry(this, 31,
                                                  "bool(__thiscall*)(void*)")
...
index_funcs.ISteamHTTP = function()
    local this = get_steam_vtable_bind("steam_api.dll", "SteamClient", 24,
                             "void*(__thiscall*)(void*, int, int, const char *)")(
                     1, 1, "STEAMHTTP_INTERFACE_VERSION003")

    local CreateHTTPRequest_native = vtable_entry(this, 0,
                                                  "unsigned int(__thiscall*)(void*, int, const char *)")
    function ISteamHTTP.CreateHTTPRequest(eHTTPRequestMethod, pchAbsoluteURL)
        eHTTPRequestMethod = to_enum_required(eHTTPRequestMethod,
                                              enums.EHTTPMethod,
                                              "http_request_method is required")
        return
            CreateHTTPRequest_native(this, eHTTPRequestMethod, pchAbsoluteURL)
    end
    ISteamHTTP.create_http_request = ISteamHTTP.CreateHTTPRequest

    local SetHTTPRequestContextValue_native =
        vtable_entry(this, 1, "bool(__thiscall*)(void*, unsigned int, uint64_t)")
    ...
    ISteamHTTP.set_http_request_context_value =
        ISteamHTTP.SetHTTPRequestContextValue

    local SetHTTPRequestNetworkActivityTimeout_native =
        vtable_entry(this, 2, "bool(__thiscall*)(void*, unsigned int, uint32_t)")
    function ISteamHTTP.SetHTTPRequestNetworkActivityTimeout(hRequest,
                                                             unTimeoutSeconds)
        return SetHTTPRequestNetworkActivityTimeout_native(this, hRequest,
                                                           unTimeoutSeconds)
    end
    ISteamHTTP.set_http_request_network_activity_timeout =
        ISteamHTTP.SetHTTPRequestNetworkActivityTimeout

    local SetHTTPRequestHeaderValue_native =
        vtable_entry(this, 3,
                     "bool(__thiscall*)(void*, unsigned int, const char *, const char *)")
    function ISteamHTTP.SetHTTPRequestHeaderValue(hRequest, pchHeaderName,
                                                  pchHeaderValue)
        return SetHTTPRequestHeaderValue_native(this, hRequest, pchHeaderName,
                                                pchHeaderValue)
    end
    ISteamHTTP.set_http_request_header_value =
        ISteamHTTP.SetHTTPRequestHeaderValue

    local SetHTTPRequestGetOrPostParameter_native =
        vtable_entry(this, 4,
                     "bool(__thiscall*)(void*, unsigned int, const char *, const char *)")
    function ISteamHTTP.SetHTTPRequestGetOrPostParameter(hRequest, pchParamName,
                                                         pchParamValue)
        return SetHTTPRequestGetOrPostParameter_native(this, hRequest,
                                                       pchParamName,
                                                       pchParamValue)
    end
    ISteamHTTP.set_http_request_get_or_post_parameter =
        ISteamHTTP.SetHTTPRequestGetOrPostParameter

    local SendHTTPRequest_native = vtable_entry(this, 5,
                                                "bool(__thiscall*)(void*, unsigned int, uint64_t *)")
    ...
    function ISteamHTTP.SendHTTPRequest(hRequest, callback)
        ...
        if callback ~= nil then
            pCallHandle_out = register_call_result(pCallHandle_out, callback,
                                                   2101, SendHTTPRequest_info)
        end
    ...
    local SendHTTPRequestAndStreamResponse_native =
        vtable_entry(this, 6,
                     "bool(__thiscall*)(void*, unsigned int, uint64_t *)")
    ...
    function ISteamHTTP.SendHTTPRequestAndStreamResponse(hRequest, callback)
        ...
        local pCallHandle_out = new_uint64_arr()
        local res = SendHTTPRequestAndStreamResponse_native(this, hRequest,
                                                            pCallHandle_out)

        if callback ~= nil then
            pCallHandle_out = register_call_result(pCallHandle_out, callback,
                                                   2101,
                                                   SendHTTPRequestAndStreamResponse_info)
        end

        return res, DEREF_GCSAFE(pCallHandle_out)
    end
    ISteamHTTP.send_http_request_and_stream_response =
        ISteamHTTP.SendHTTPRequestAndStreamResponse

    local DeferHTTPRequest_native = vtable_entry(this, 7,
                                                 "bool(__thiscall*)(void*, unsigned int)")
    ...
    local PrioritizeHTTPRequest_native =
        vtable_entry(this, 8, "bool(__thiscall*)(void*, unsigned int)")
    ...
    local GetHTTPResponseHeaderSize_native =
        vtable_entry(this, 9,
                     "bool(__thiscall*)(void*, unsigned int, const char *, uint32_t *)")
    function ISteamHTTP.GetHTTPResponseHeaderSize(hRequest, pchHeaderName)
        local unResponseHeaderSize_out = new_uint32_arr()
        local res = GetHTTPResponseHeaderSize_native(this, hRequest,
                                                     pchHeaderName,
                                                     unResponseHeaderSize_out)

        return res, DEREF_GCSAFE(unResponseHeaderSize_out)
    end
    ISteamHTTP.get_http_response_header_size =
        ISteamHTTP.GetHTTPResponseHeaderSize

    local GetHTTPResponseHeaderValue_native =
        vtable_entry(this, 10,
                     "bool(__thiscall*)(void*, unsigned int, const char *, uint8_t *, uint32_t)")
    function ISteamHTTP.GetHTTPResponseHeaderValue(hRequest, pchHeaderName,
                                                   pHeaderValueBuffer,
                                                   unBufferSize)
        return GetHTTPResponseHeaderValue_native(this, hRequest, pchHeaderName,
                                                 pHeaderValueBuffer,
                                                 unBufferSize)
    end
    ISteamHTTP.get_http_response_header_value =
        ISteamHTTP.GetHTTPResponseHeaderValue

    local GetHTTPResponseBodySize_native =
        vtable_entry(this, 11,
                     "bool(__thiscall*)(void*, unsigned int, uint32_t *)")
    function ISteamHTTP.GetHTTPResponseBodySize(hRequest)
        local unBodySize_out = new_uint32_arr()
        local res = GetHTTPResponseBodySize_native(this, hRequest,
                                                   unBodySize_out)
    ...
    local GetHTTPResponseBodyData_native =
        vtable_entry(this, 12,
                     "bool(__thiscall*)(void*, unsigned int, uint8_t *, uint32_t)")
    function ISteamHTTP.GetHTTPResponseBodyData(hRequest, pBodyDataBuffer,
                                                unBufferSize)
        return GetHTTPResponseBodyData_native(this, hRequest, pBodyDataBuffer,
                                              unBufferSize)
    end
    ISteamHTTP.get_http_response_body_data = ISteamHTTP.GetHTTPResponseBodyData

    local GetHTTPStreamingResponseBodyData_native =
        vtable_entry(this, 13,
                     "bool(__thiscall*)(void*, unsigned int, uint32_t, uint8_t *, uint32_t)")
    function ISteamHTTP.GetHTTPStreamingResponseBodyData(hRequest, cOffset,
                                                         pBodyDataBuffer,
                                                         unBufferSize)
        return GetHTTPStreamingResponseBodyData_native(this, hRequest, cOffset,
                                                       pBodyDataBuffer,
                                                       unBufferSize)
    end
    ISteamHTTP.get_http_streaming_response_body_data =
        ISteamHTTP.GetHTTPStreamingResponseBodyData

    local ReleaseHTTPRequest_native = vtable_entry(this, 14,
                                                   "bool(__thiscall*)(void*, unsigned int)")
    ...
    local GetHTTPDownloadProgressPct_native =
        vtable_entry(this, 15, "bool(__thiscall*)(void*, unsigned int, float *)")
    function ISteamHTTP.GetHTTPDownloadProgressPct(hRequest)
        local pflPercentOut_out = new_float_arr()
        local res = GetHTTPDownloadProgressPct_native(this, hRequest,
                                                      pflPercentOut_out)

        return res, DEREF_GCSAFE(pflPercentOut_out)
    end
    ISteamHTTP.get_http_download_progress_pct =
        ISteamHTTP.GetHTTPDownloadProgressPct

    local SetHTTPRequestRawPostBody_native =
        vtable_entry(this, 16,
                     "bool(__thiscall*)(void*, unsigned int, const char *, uint8_t *, uint32_t)")
    function ISteamHTTP.SetHTTPRequestRawPostBody(hRequest, pchContentType,
                                                  pubBody, unBodyLen)
        return SetHTTPRequestRawPostBody_native(this, hRequest, pchContentType,
                                                pubBody, unBodyLen)
    end
    ISteamHTTP.set_http_request_raw_post_body =
        ISteamHTTP.SetHTTPRequestRawPostBody

    local CreateCookieContainer_native =
        vtable_entry(this, 17, "unsigned int(__thiscall*)(void*, bool)")
    ...
    local ReleaseCookieContainer_native =
        vtable_entry(this, 18, "bool(__thiscall*)(void*, unsigned int)")
    ...
    local SetCookie_native = vtable_entry(this, 19,
                                          "bool(__thiscall*)(void*, unsigned int, const char *, const char *, const char *)")
    function ISteamHTTP.SetCookie(hCookieContainer, pchHost, pchUrl, pchCookie)
        return SetCookie_native(this, hCookieContainer, pchHost, pchUrl,
                                pchCookie)
    end
    ISteamHTTP.set_cookie = ISteamHTTP.SetCookie

    local SetHTTPRequestCookieContainer_native =
        vtable_entry(this, 20,
                     "bool(__thiscall*)(void*, unsigned int, unsigned int)")
    function ISteamHTTP.SetHTTPRequestCookieContainer(hRequest, hCookieContainer)
        return SetHTTPRequestCookieContainer_native(this, hRequest,
                                                    hCookieContainer)
    end
    ISteamHTTP.set_http_request_cookie_container =
        ISteamHTTP.SetHTTPRequestCookieContainer

    local SetHTTPRequestUserAgentInfo_native =
        vtable_entry(this, 21,
                     "bool(__thiscall*)(void*, unsigned int, const char *)")
    function ISteamHTTP.SetHTTPRequestUserAgentInfo(hRequest, pchUserAgentInfo)
        return SetHTTPRequestUserAgentInfo_native(this, hRequest,
                                                  pchUserAgentInfo)
    end
    ISteamHTTP.set_http_request_user_agent_info =
        ISteamHTTP.SetHTTPRequestUserAgentInfo

    local SetHTTPRequestRequiresVerifiedCertificate_native = vtable_entry(this,
                                                                          22,
                                                                          "bool(__thiscall*)(void*, unsigned int, bool)")
    function ISteamHTTP.SetHTTPRequestRequiresVerifiedCertificate(hRequest,
                                                                  bRequireVerifiedCertificate)
        return SetHTTPRequestRequiresVerifiedCertificate_native(this, hRequest,
                                                                bRequireVerifiedCertificate)
    end
    ISteamHTTP.set_http_request_requires_verified_certificate =
        ISteamHTTP.SetHTTPRequestRequiresVerifiedCertificate

    local SetHTTPRequestAbsoluteTimeoutMS_native =
        vtable_entry(this, 23,
                     "bool(__thiscall*)(void*, unsigned int, uint32_t)")
    function ISteamHTTP.SetHTTPRequestAbsoluteTimeoutMS(hRequest, unMilliseconds)
        return SetHTTPRequestAbsoluteTimeoutMS_native(this, hRequest,
                                                      unMilliseconds)
    end
    ISteamHTTP.set_http_request_absolute_timeout_ms =
        ISteamHTTP.SetHTTPRequestAbsoluteTimeoutMS

    local GetHTTPRequestWasTimedOut_native =
        vtable_entry(this, 24, "bool(__thiscall*)(void*, unsigned int, bool *)")
    function ISteamHTTP.GetHTTPRequestWasTimedOut(hRequest)
        local pbWasTimedOut_out = new_bool_arr()
        local res = GetHTTPRequestWasTimedOut_native(this, hRequest,
                                                     pbWasTimedOut_out)

        return res, DEREF_GCSAFE(pbWasTimedOut_out)
    end
    ISteamHTTP.get_http_request_was_timed_out =
...
index_funcs.ISteamHTMLSurface = function()
    ...
    local RemoveBrowser_native = vtable_entry(this, 4,
                                              "void(__thiscall*)(void*, unsigned int)")
    ...
    local LoadURL_native = vtable_entry(this, 5,
                                        "void(__thiscall*)(void*, unsigned int, const char *, const char *)")
    ...
    local SetSize_native = vtable_entry(this, 6,
                                        "void(__thiscall*)(void*, unsigned int, uint32_t, uint32_t)")
    ...
    local StopLoad_native = vtable_entry(this, 7,
                                         "void(__thiscall*)(void*, unsigned int)")
    ...
    local Reload_native = vtable_entry(this, 8,
                                       "void(__thiscall*)(void*, unsigned int)")
    ...
    local GoBack_native = vtable_entry(this, 9,
                                       "void(__thiscall*)(void*, unsigned int)")
    ...
    local GoForward_native = vtable_entry(this, 10,
                                          "void(__thiscall*)(void*, unsigned int)")
    ...
    local AddHeader_native = vtable_entry(this, 11,
                                          "void(__thiscall*)(void*, unsigned int, const char *, const char *)")
    ...
    local ExecuteJavascript_native = vtable_entry(this, 12,
                                                  "void(__thiscall*)(void*, unsigned int, const char *)")
    ...
    local MouseUp_native = vtable_entry(this, 13,
                                        "void(__thiscall*)(void*, unsigned int, int)")
    function ISteamHTMLSurface.MouseUp(unBrowserHandle, eMouseButton)
        eMouseButton = to_enum_required(eMouseButton, enums.EHTMLMouseButton,
                                        "mouse_button is required")
        return MouseUp_native(this, unBrowserHandle, eMouseButton)
    end
    ISteamHTMLSurface.mouse_up = ISteamHTMLSurface.MouseUp

    local MouseDown_native = vtable_entry(this, 14,
                                          "void(__thiscall*)(void*, unsigned int, int)")
    ...
    local MouseDoubleClick_native = vtable_entry(this, 15,
                                                 "void(__thiscall*)(void*, unsigned int, int)")
    ...
    local MouseMove_native = vtable_entry(this, 16,
                                          "void(__thiscall*)(void*, unsigned int, int, int)")
    ...
    local MouseWheel_native = vtable_entry(this, 17,
                                           "void(__thiscall*)(void*, unsigned int, int32_t)")
    ...
    local KeyDown_native = vtable_entry(this, 18,
                                        "void(__thiscall*)(void*, unsigned int, uint32_t, int, bool)")
    function ISteamHTMLSurface.KeyDown(unBrowserHandle, nNativeKeyCode,
                                       eHTMLKeyModifiers, bIsSystemKey)
        eHTMLKeyModifiers = to_enum_required(eHTMLKeyModifiers,
                                             enums.EHTMLKeyModifiers,
                                             "html_key_modifiers is required")
        return KeyDown_native(this, unBrowserHandle, nNativeKeyCode,
                              eHTMLKeyModifiers, bIsSystemKey)
    end
    ISteamHTMLSurface.key_down = ISteamHTMLSurface.KeyDown

    local KeyUp_native = vtable_entry(this, 19,
                                      "void(__thiscall*)(void*, unsigned int, uint32_t, int)")
    function ISteamHTMLSurface.KeyUp(unBrowserHandle, nNativeKeyCode,
                                     eHTMLKeyModifiers)
        eHTMLKeyModifiers = to_enum_required(eHTMLKeyModifiers,
                                             enums.EHTMLKeyModifiers,
                                             "html_key_modifiers is required")
        return KeyUp_native(this, unBrowserHandle, nNativeKeyCode,
                            eHTMLKeyModifiers)
    end
    ISteamHTMLSurface.key_up = ISteamHTMLSurface.KeyUp

    local KeyChar_native = vtable_entry(this, 20,
                                        "void(__thiscall*)(void*, unsigned int, uint32_t, int)")
    function ISteamHTMLSurface.KeyChar(unBrowserHandle, cUnicodeChar,
                                       eHTMLKeyModifiers)
        eHTMLKeyModifiers = to_enum_required(eHTMLKeyModifiers,
                                             enums.EHTMLKeyModifiers,
                                             "html_key_modifiers is required")
        return KeyChar_native(this, unBrowserHandle, cUnicodeChar,
                              eHTMLKeyModifiers)
    end
    ISteamHTMLSurface.key_char = ISteamHTMLSurface.KeyChar

    local SetHorizontalScroll_native = vtable_entry(this, 21,
                                                    "void(__thiscall*)(void*, unsigned int, uint32_t)")
    function ISteamHTMLSurface.SetHorizontalScroll(unBrowserHandle,
                                                   nAbsolutePixelScroll)
        return SetHorizontalScroll_native(this, unBrowserHandle,
                                          nAbsolutePixelScroll)
    end
    ISteamHTMLSurface.set_horizontal_scroll =
        ISteamHTMLSurface.SetHorizontalScroll

    local SetVerticalScroll_native = vtable_entry(this, 22,
                                                  "void(__thiscall*)(void*, unsigned int, uint32_t)")
    function ISteamHTMLSurface.SetVerticalScroll(unBrowserHandle,
                                                 nAbsolutePixelScroll)
        return SetVerticalScroll_native(this, unBrowserHandle,
                                        nAbsolutePixelScroll)
    end
    ISteamHTMLSurface.set_vertical_scroll = ISteamHTMLSurface.SetVerticalScroll

    local SetKeyFocus_native = vtable_entry(this, 23,
                                            "void(__thiscall*)(void*, unsigned int, bool)")
    ...
    local ViewSource_native = vtable_entry(this, 24,
                                           "void(__thiscall*)(void*, unsigned int)")
    ...
    local CopyToClipboard_native = vtable_entry(this, 25,
                                                "void(__thiscall*)(void*, unsigned int)")
    ...
    local PasteFromClipboard_native = vtable_entry(this, 26,
                                                   "void(__thiscall*)(void*, unsigned int)")
    function ISteamHTMLSurface.PasteFromClipboard(unBrowserHandle)
        return PasteFromClipboard_native(this, unBrowserHandle)
    end
    ISteamHTMLSurface.paste_from_clipboard =
        ISteamHTMLSurface.PasteFromClipboard

    local Find_native = vtable_entry(this, 27,
                                     "void(__thiscall*)(void*, unsigned int, const char *, bool, bool)")
    function ISteamHTMLSurface.Find(unBrowserHandle, pchSearchStr,
                                    bCurrentlyInFind, bReverse)
        return Find_native(this, unBrowserHandle, pchSearchStr,
                           bCurrentlyInFind, bReverse)
    end
    ISteamHTMLSurface.find = ISteamHTMLSurface.Find

    local StopFind_native = vtable_entry(this, 28,
                                         "void(__thiscall*)(void*, unsigned int)")
    ...
    local GetLinkAtPosition_native = vtable_entry(this, 29,
                                                  "void(__thiscall*)(void*, unsigned int, int, int)")
    ...
    local SetCookie_native = vtable_entry(this, 30,
                                          "void(__thiscall*)(void*, const char *, const char *, const char *, const char *, unsigned int, bool, bool)")
    function ISteamHTMLSurface.SetCookie(pchHostname, pchKey, pchValue, pchPath,
                                         nExpires, bSecure, bHTTPOnly)
        return SetCookie_native(this, pchHostname, pchKey, pchValue, pchPath,
                                nExpires, bSecure, bHTTPOnly)
    end
    ISteamHTMLSurface.set_cookie = ISteamHTMLSurface.SetCookie

    local SetPageScaleFactor_native = vtable_entry(this, 31,
                                                   "void(__thiscall*)(void*, unsigned int, float, int, int)")
    function ISteamHTMLSurface.SetPageScaleFactor(unBrowserHandle, flZoom,
                                                  nPointX, nPointY)
        return SetPageScaleFactor_native(this, unBrowserHandle, flZoom, nPointX,
                                         nPointY)
    end
    ISteamHTMLSurface.set_page_scale_factor =
        ISteamHTMLSurface.SetPageScaleFactor

    local SetBackgroundMode_native = vtable_entry(this, 32,
                                                  "void(__thiscall*)(void*, unsigned int, bool)")
    function ISteamHTMLSurface.SetBackgroundMode(unBrowserHandle,
                                                 bBackgroundMode)
    ...
    local SetDPIScalingFactor_native = vtable_entry(this, 33,
                                                    "void(__thiscall*)(void*, unsigned int, float)")
    ...
    ISteamHTMLSurface.set_dpi_scaling_factor =
        ISteamHTMLSurface.SetDPIScalingFactor

    local OpenDeveloperTools_native = vtable_entry(this, 34,
                                                   "void(__thiscall*)(void*, unsigned int)")
    function ISteamHTMLSurface.OpenDeveloperTools(unBrowserHandle)
        return OpenDeveloperTools_native(this, unBrowserHandle)
    end
    ISteamHTMLSurface.open_developer_tools =
        ISteamHTMLSurface.OpenDeveloperTools

    local AllowStartRequest_native = vtable_entry(this, 35,
                                                  "void(__thiscall*)(void*, unsigned int, bool)")
    ...
    local JSDialogResponse_native = vtable_entry(this, 36,
                                                 "void(__thiscall*)(void*, unsigned int, bool)")
    ...
    local FileLoadDialogResponse_native =
        vtable_entry(this, 37,
                     "void(__thiscall*)(void*, unsigned int, const char **)")
    function ISteamHTMLSurface.FileLoadDialogResponse(unBrowserHandle,
                                                      pchSelectedFiles)
        return FileLoadDialogResponse_native(this, unBrowserHandle,
                                             pchSelectedFiles)
    end
    ISteamHTMLSurface.file_load_dialog_response =
        ISteamHTMLSurface.FileLoadDialogResponse
...
local ISteamInventory = {
    version = "STEAMINVENTORY_INTERFACE_V003",
    version_number = 3
}

index_funcs.ISteamInventory = function()
    local this = get_steam_vtable_bind("steam_api.dll", "SteamClient", 35,
                             "void*(__thiscall*)(void*, int, int, const char *)")(
                     hSteamUser, hSteamPipe, "STEAMINVENTORY_INTERFACE_V003")

    local GetResultStatus_native = vtable_entry(this, 0,
                                                "int(__thiscall*)(void*, int)")
    ...
    local GetResultItems_native = vtable_entry(this, 1,
                                               "bool(__thiscall*)(void*, int, SteamItemDetails_t *, uint32_t *)")
    function ISteamInventory.GetResultItems(resultHandle, pOutItemsArray,
                                            punOutItemsArraySize)
        return GetResultItems_native(this, resultHandle, pOutItemsArray,
                                     punOutItemsArraySize)
    end
    ISteamInventory.get_result_items = ISteamInventory.GetResultItems

    local GetResultItemProperty_native =
        vtable_entry(this, 2,
                     "bool(__thiscall*)(void*, int, uint32_t, const char *, char *, uint32_t *)")
    function ISteamInventory.GetResultItemProperty(resultHandle, unItemIndex,
                                                   pchPropertyName,
                                                   pchValueBuffer,
                                                   punValueBufferSizeOut)
        return GetResultItemProperty_native(this, resultHandle, unItemIndex,
                                            pchPropertyName, pchValueBuffer,
                                            punValueBufferSizeOut)
    end
    ISteamInventory.get_result_item_property =
        ISteamInventory.GetResultItemProperty

    local GetResultTimestamp_native = vtable_entry(this, 3,
                                                   "uint32_t(__thiscall*)(void*, int)")
    ...
    local CheckResultSteamID_native = vtable_entry(this, 4,
                                                   "bool(__thiscall*)(void*, int, SteamID)")
    function ISteamInventory.CheckResultSteamID(resultHandle, steamIDExpected)
        steamIDExpected = to_steamid_required(steamIDExpected,
                                              "steamid_expected is required")
    ...
    local DestroyResult_native = vtable_entry(this, 5,
                                              "void(__thiscall*)(void*, int)")
    ...
    local GetAllItems_native = vtable_entry(this, 6,
                                            "bool(__thiscall*)(void*, int *)")
    ...
    local GetItemsByID_native = vtable_entry(this, 7,
                                             "bool(__thiscall*)(void*, int *, const uint64_t *, uint32_t)")
    function ISteamInventory.GetItemsByID(pInstanceIDs, unCountInstanceIDs)
        local pResultHandle_out = new_int_arr()
        local res = GetItemsByID_native(this, pResultHandle_out, pInstanceIDs,
                                        unCountInstanceIDs)
    ...
    local SerializeResult_native = vtable_entry(this, 8,
                                                "bool(__thiscall*)(void*, int, void *, uint32_t *)")
    function ISteamInventory.SerializeResult(resultHandle, pOutBuffer,
                                             punOutBufferSize)
        return SerializeResult_native(this, resultHandle, pOutBuffer,
                                      punOutBufferSize)
    end
    ISteamInventory.serialize_result = ISteamInventory.SerializeResult

    local DeserializeResult_native = vtable_entry(this, 9,
                                                  "bool(__thiscall*)(void*, int *, const void *, uint32_t, bool)")
    function ISteamInventory.DeserializeResult(pBuffer, unBufferSize,
                                               bRESERVED_MUST_BE_FALSE)
        local pOutResultHandle_out = new_int_arr()
        local res = DeserializeResult_native(this, pOutResultHandle_out,
                                             pBuffer, unBufferSize,
                                             bRESERVED_MUST_BE_FALSE)
    ...
    local GenerateItems_native = vtable_entry(this, 10,
                                              "bool(__thiscall*)(void*, int *, const int *, const uint32_t *, uint32_t)")
    function ISteamInventory.GenerateItems(pArrayItemDefs, punArrayQuantity,
                                           unArrayLength)
        local pResultHandle_out = new_int_arr()
        local res = GenerateItems_native(this, pResultHandle_out,
    ...
    local GrantPromoItems_native = vtable_entry(this, 11,
                                                "bool(__thiscall*)(void*, int *)")
    ...
    local AddPromoItem_native = vtable_entry(this, 12,
                                             "bool(__thiscall*)(void*, int *, int)")
    ...
    local AddPromoItems_native = vtable_entry(this, 13,
                                              "bool(__thiscall*)(void*, int *, const int *, uint32_t)")
    function ISteamInventory.AddPromoItems(pArrayItemDefs, unArrayLength)
        local pResultHandle_out = new_int_arr()
        local res = AddPromoItems_native(this, pResultHandle_out,
                                         pArrayItemDefs, unArrayLength)

        return res, DEREF_GCSAFE(pResultHandle_out)
    end
    ISteamInventory.add_promo_items = ISteamInventory.AddPromoItems

    local ConsumeItem_native = vtable_entry(this, 14,
                                            "bool(__thiscall*)(void*, int *, uint64_t, uint32_t)")
    function ISteamInventory.ConsumeItem(itemConsume, unQuantity)
        local pResultHandle_out = new_int_arr()
        local res = ConsumeItem_native(this, pResultHandle_out, itemConsume,
                                       unQuantity)
    ...
    local ExchangeItems_native = vtable_entry(this, 15,
                                              "bool(__thiscall*)(void*, int *, const int *, const uint32_t *, uint32_t, const uint64_t *, const uint32_t *, uint32_t)")
    function ISteamInventory.ExchangeItems(pArrayGenerate,
                                           punArrayGenerateQuantity,
                                           unArrayGenerateLength, pArrayDestroy,
                                           punArrayDestroyQuantity,
                                           unArrayDestroyLength)
        local pResultHandle_out = new_int_arr()
        local res = ExchangeItems_native(this, pResultHandle_out,
                                         pArrayGenerate,
                                         punArrayGenerateQuantity,
                                         unArrayGenerateLength, pArrayDestroy,
                                         punArrayDestroyQuantity,
    ...
    local TransferItemQuantity_native = vtable_entry(this, 16,
                                                     "bool(__thiscall*)(void*, int *, uint64_t, uint32_t, uint64_t)")
    function ISteamInventory.TransferItemQuantity(itemIdSource, unQuantity,
                                                  itemIdDest)
        local pResultHandle_out = new_int_arr()
        local res = TransferItemQuantity_native(this, pResultHandle_out,
                                                itemIdSource, unQuantity,
                                                itemIdDest)

        return res, DEREF_GCSAFE(pResultHandle_out)
    end
    ISteamInventory.transfer_item_quantity =
        ISteamInventory.TransferItemQuantity

    local SendItemDropHeartbeat_native =
        vtable_entry(this, 17, "void(__thiscall*)(void*)")
    function ISteamInventory.SendItemDropHeartbeat()
        return SendItemDropHeartbeat_native(this)
    end
    ISteamInventory.send_item_drop_heartbeat =
        ISteamInventory.SendItemDropHeartbeat

    local TriggerItemDrop_native = vtable_entry(this, 18,
                                                "bool(__thiscall*)(void*, int *, int)")
    function ISteamInventory.TriggerItemDrop(dropListDefinition)
        local pResultHandle_out = new_int_arr()
        local res = TriggerItemDrop_native(this, pResultHandle_out,
                                           dropListDefinition)
    ...
    local TradeItems_native = vtable_entry(this, 19,
                                           "bool(__thiscall*)(void*, int *, SteamID, const uint64_t *, const uint32_t *, uint32_t, const uint64_t *, const uint32_t *, uint32_t)")
    function ISteamInventory.TradeItems(steamIDTradePartner, pArrayGive,
                                        pArrayGiveQuantity, nArrayGiveLength,
                                        pArrayGet, pArrayGetQuantity,
                                        nArrayGetLength)
        steamIDTradePartner = to_steamid_required(steamIDTradePartner,
                                                  "steamid_trade_partner is required")
        local pResultHandle_out = new_int_arr()
        local res = TradeItems_native(this, pResultHandle_out,
    ...
    local LoadItemDefinitions_native = vtable_entry(this, 20,
                                                    "bool(__thiscall*)(void*)")
    ...
    local GetItemDefinitionIDs_native = vtable_entry(this, 21,
                                                     "bool(__thiscall*)(void*, int *, uint32_t *)")
    function ISteamInventory.GetItemDefinitionIDs(pItemDefIDs,
                                                  punItemDefIDsArraySize)
        return GetItemDefinitionIDs_native(this, pItemDefIDs,
                                           punItemDefIDsArraySize)
    end
    ISteamInventory.get_item_definition_ids =
        ISteamInventory.GetItemDefinitionIDs

    local GetItemDefinitionProperty_native =
        vtable_entry(this, 22,
                     "bool(__thiscall*)(void*, int, const char *, char *, uint32_t *)")
    function ISteamInventory.GetItemDefinitionProperty(iDefinition,
                                                       pchPropertyName,
                                                       pchValueBuffer,
                                                       punValueBufferSizeOut)
        return GetItemDefinitionProperty_native(this, iDefinition,
                                                pchPropertyName, pchValueBuffer,
                                                punValueBufferSizeOut)
    end
    ISteamInventory.get_item_definition_property =
        ISteamInventory.GetItemDefinitionProperty

    local RequestEligiblePromoItemDefinitionsIDs_native =
        vtable_entry(this, 23, "uint64_t(__thiscall*)(void*, SteamID)")
    ...
    function ISteamInventory.RequestEligiblePromoItemDefinitionsIDs(steamID,
                                                                    callback)
        steamID = to_steamid_required(steamID, "steamid is required")
        if callback ~= nil and not is_valid_callback(callback) then
            return error("Invalid callback, expected function or await")
        ...
        if callback ~= nil then
            res = register_call_result(res, callback, 4703,
                                       RequestEligiblePromoItemDefinitionsIDs_info)
        end

        return res
    end
    ISteamInventory.request_eligible_promo_item_definitions_ids =
        ISteamInventory.RequestEligiblePromoItemDefinitionsIDs

    local GetEligiblePromoItemDefinitionIDs_native =
        vtable_entry(this, 24,
                     "bool(__thiscall*)(void*, SteamID, int *, uint32_t *)")
    function ISteamInventory.GetEligiblePromoItemDefinitionIDs(steamID,
                                                               pItemDefIDs,
                                                               punItemDefIDsArraySize)
        steamID = to_steamid_required(steamID, "steamid is required")
        return GetEligiblePromoItemDefinitionIDs_native(this, steamID,
                                                        pItemDefIDs,
                                                        punItemDefIDsArraySize)
    end
    ISteamInventory.get_eligible_promo_item_definition_ids =
        ISteamInventory.GetEligiblePromoItemDefinitionIDs

    local StartPurchase_native = vtable_entry(this, 25,
                                              "uint64_t(__thiscall*)(void*, const int *, const uint32_t *, uint32_t)")
    ...
    function ISteamInventory.StartPurchase(pArrayItemDefs, punArrayQuantity,
                                           unArrayLength, callback)
        if callback ~= nil and not is_valid_callback(callback) then
            return error("Invalid callback, expected function or await")
        end

        local res = StartPurchase_native(this, pArrayItemDefs, punArrayQuantity,
                                         unArrayLength)
    ...
    local RequestPrices_native = vtable_entry(this, 26,
                                              "uint64_t(__thiscall*)(void*)")
    ...
    local GetNumItemsWithPrices_native =
        vtable_entry(this, 27, "uint32_t(__thiscall*)(void*)")
    function ISteamInventory.GetNumItemsWithPrices()
        return GetNumItemsWithPrices_native(this)
    end
    ISteamInventory.get_num_items_with_prices =
        ISteamInventory.GetNumItemsWithPrices

    local GetItemsWithPrices_native = vtable_entry(this, 28,
                                                   "bool(__thiscall*)(void*, int *, uint64_t *, uint64_t *, uint32_t)")
    function ISteamInventory.GetItemsWithPrices(pArrayItemDefs, pCurrentPrices,
                                                pBasePrices, unArrayLength)
        return GetItemsWithPrices_native(this, pArrayItemDefs, pCurrentPrices,
                                         pBasePrices, unArrayLength)
    end
    ISteamInventory.get_items_with_prices = ISteamInventory.GetItemsWithPrices

    local GetItemPrice_native = vtable_entry(this, 29,
                                             "bool(__thiscall*)(void*, int, uint64_t *, uint64_t *)")
    function ISteamInventory.GetItemPrice(iDefinition)
        ...
        local res = GetItemPrice_native(this, iDefinition, pCurrentPrice_out,
                                        pBasePrice_out)

        return res, DEREF_GCSAFE(pCurrentPrice_out),
               DEREF_GCSAFE(pBasePrice_out)
    end
    ISteamInventory.get_item_price = ISteamInventory.GetItemPrice

    local StartUpdateProperties_native =
        vtable_entry(this, 30, "void(__thiscall*)(void*, uint64_t *)")
    ...
    ISteamInventory.start_update_properties =
        ISteamInventory.StartUpdateProperties

    local RemoveProperty_native = vtable_entry(this, 31,
                                               "bool(__thiscall*)(void*, uint64_t, uint64_t, const char *)")
    ...
    local SetPropertyString_native = vtable_entry(this, 32,
                                                  "bool(__thiscall*)(void*, uint64_t, uint64_t, const char *, const char *)")
    function ISteamInventory.SetPropertyString(handle, nItemID, pchPropertyName,
                                               pchPropertyValue)
        return SetPropertyString_native(this, handle, nItemID, pchPropertyName,
                                        pchPropertyValue)
    end
    ISteamInventory.set_property_string = ISteamInventory.SetPropertyString

    local SetPropertyBool_native = vtable_entry(this, 33,
                                                "bool(__thiscall*)(void*, uint64_t, uint64_t, const char *, bool)")
    function ISteamInventory.SetPropertyBool(handle, nItemID, pchPropertyName,
                                             bValue)
        return SetPropertyBool_native(this, handle, nItemID, pchPropertyName,
                                      bValue)
    end
    ISteamInventory.set_property_bool = ISteamInventory.SetPropertyBool

    local SetPropertyInt64_native = vtable_entry(this, 34,
                                                 "bool(__thiscall*)(void*, uint64_t, uint64_t, const char *, int64_t)")
    function ISteamInventory.SetPropertyInt64(handle, nItemID, pchPropertyName,
                                              nValue)
        return SetPropertyInt64_native(this, handle, nItemID, pchPropertyName,
                                       nValue)
    end
    ISteamInventory.set_property_int64 = ISteamInventory.SetPropertyInt64

    local SetPropertyFloat_native = vtable_entry(this, 35,
                                                 "bool(__thiscall*)(void*, uint64_t, uint64_t, const char *, float)")
    function ISteamInventory.SetPropertyFloat(handle, nItemID, pchPropertyName,
                                              flValue)
        return SetPropertyFloat_native(this, handle, nItemID, pchPropertyName,
                                       flValue)
    end
    ISteamInventory.set_property_float = ISteamInventory.SetPropertyFloat

    local SubmitUpdateProperties_native =
        vtable_entry(this, 36, "bool(__thiscall*)(void*, uint64_t, int *)")
    function ISteamInventory.SubmitUpdateProperties(handle)
        local pResultHandle_out = new_int_arr()
        local res = SubmitUpdateProperties_native(this, handle,
                                                  pResultHandle_out)

        return res, DEREF_GCSAFE(pResultHandle_out)
    end
    ISteamInventory.submit_update_properties =
        ISteamInventory.SubmitUpdateProperties

    local InspectItem_native = vtable_entry(this, 37,
                                            "bool(__thiscall*)(void*, int *, const char *)")
...
index_funcs.ISteamVideo = function()
    local this = get_steam_vtable_bind("steam_api.dll", "SteamClient", 36,
                             "void*(__thiscall*)(void*, int, int, const char *)")(
                     hSteamUser, hSteamPipe, "STEAMVIDEO_INTERFACE_V002")

    local GetVideoURL_native = vtable_entry(this, 0,
                                            "void(__thiscall*)(void*, unsigned int)")
    ...
    local IsBroadcasting_native = vtable_entry(this, 1,
                                               "bool(__thiscall*)(void*, int *)")
    ...
    local GetOPFSettings_native = vtable_entry(this, 2,
                                               "void(__thiscall*)(void*, unsigned int)")
    ...
    local GetOPFStringForApp_native = vtable_entry(this, 3,
                                                   "bool(__thiscall*)(void*, unsigned int, char *, int32_t *)")
    function ISteamVideo.GetOPFStringForApp(unVideoAppID, pchBuffer,
                                            pnBufferSize)
        return GetOPFStringForApp_native(this, unVideoAppID, pchBuffer,
                                         pnBufferSize)
    end
...
local ISteamParentalSettings = {
    version = "STEAMPARENTALSETTINGS_INTERFACE_VERSION001",
    version_number = 1
}

index_funcs.ISteamParentalSettings = function()
    local this = get_steam_vtable_bind("steam_api.dll", "SteamClient", 37,
                             "void*(__thiscall*)(void*, int, int, const char *)")(
                     hSteamUser, hSteamPipe,
                     "STEAMPARENTALSETTINGS_INTERFACE_VERSION001")

    local BIsParentalLockEnabled_native =
        vtable_entry(this, 0, "bool(__thiscall*)(void*)")
    function ISteamParentalSettings.BIsParentalLockEnabled()
        return BIsParentalLockEnabled_native(this)
    end
    ISteamParentalSettings.is_parental_lock_enabled =
        ISteamParentalSettings.BIsParentalLockEnabled

    local BIsParentalLockLocked_native =
        vtable_entry(this, 1, "bool(__thiscall*)(void*)")
    function ISteamParentalSettings.BIsParentalLockLocked()
        return BIsParentalLockLocked_native(this)
    end
    ISteamParentalSettings.is_parental_lock_locked =
        ISteamParentalSettings.BIsParentalLockLocked

    local BIsAppBlocked_native = vtable_entry(this, 2,
                                              "bool(__thiscall*)(void*, unsigned int)")
    ...
    local BIsAppInBlockList_native = vtable_entry(this, 3,
                                                  "bool(__thiscall*)(void*, unsigned int)")
    function ISteamParentalSettings.BIsAppInBlockList(nAppID)
        return BIsAppInBlockList_native(this, nAppID)
    end
    ISteamParentalSettings.is_app_in_block_list =
        ISteamParentalSettings.BIsAppInBlockList

    local BIsFeatureBlocked_native = vtable_entry(this, 4,
                                                  "bool(__thiscall*)(void*, int)")
    function ISteamParentalSettings.BIsFeatureBlocked(eFeature)
        eFeature = to_enum_required(eFeature, enums.EParentalFeature,
                                    "feature is required")
        return BIsFeatureBlocked_native(this, eFeature)
    end
    ISteamParentalSettings.is_feature_blocked =
        ISteamParentalSettings.BIsFeatureBlocked

    local BIsFeatureInBlockList_native =
        vtable_entry(this, 5, "bool(__thiscall*)(void*, int)")
    function ISteamParentalSettings.BIsFeatureInBlockList(eFeature)
        eFeature = to_enum_required(eFeature, enums.EParentalFeature,
                                    "feature is required")
        return BIsFeatureInBlockList_native(this, eFeature)
    end
    ISteamParentalSettings.is_feature_in_block_list =
        ISteamParentalSettings.BIsFeatureInBlockList
...
local ISteamRemotePlay = {
    version = "STEAMREMOTEPLAY_INTERFACE_VERSION001",
    version_number = 1
}

index_funcs.ISteamRemotePlay = function()
    local this = get_steam_vtable_bind("steam_api.dll", "SteamClient", 40,
                             "void*(__thiscall*)(void*, int, int, const char *)")(
                     hSteamUser, hSteamPipe,
                     "STEAMREMOTEPLAY_INTERFACE_VERSION001")

    local GetSessionCount_native = vtable_entry(this, 0,
                                                "uint32_t(__thiscall*)(void*)")
    ...
    local GetSessionID_native = vtable_entry(this, 1,
                                             "unsigned int(__thiscall*)(void*, int)")
    ...
    local GetSessionSteamID_native = vtable_entry(this, 2,
                                                  "void(__thiscall*)(void*, SteamID *, unsigned int)")
    ...
    local GetSessionClientName_native = vtable_entry(this, 3,
                                                     "const char *(__thiscall*)(void*, unsigned int)")
    function ISteamRemotePlay.GetSessionClientName(unSessionID)
        local res = GetSessionClientName_native(this, unSessionID)

        return res ~= nil and ffi_string(res) or nil
    end
    ISteamRemotePlay.get_session_client_name =
        ISteamRemotePlay.GetSessionClientName

    local GetSessionClientFormFactor_native =
        vtable_entry(this, 4, "int(__thiscall*)(void*, unsigned int)")
    ...
    ISteamRemotePlay.get_session_client_form_factor =
        ISteamRemotePlay.GetSessionClientFormFactor

    local BGetSessionClientResolution_native =
        vtable_entry(this, 5,
                     "bool(__thiscall*)(void*, unsigned int, int *, int *)")
    function ISteamRemotePlay.BGetSessionClientResolution(unSessionID)
        ...
        local res = BGetSessionClientResolution_native(this, unSessionID,
                                                       pnResolutionX_out,
                                                       pnResolutionY_out)

        return res, DEREF_GCSAFE(pnResolutionX_out),
               DEREF_GCSAFE(pnResolutionY_out)
    end
    ISteamRemotePlay.get_session_client_resolution =
        ISteamRemotePlay.BGetSessionClientResolution

    local BSendRemotePlayTogetherInvite_native =
        vtable_entry(this, 6, "bool(__thiscall*)(void*, SteamID)")
    ...
    ISteamRemotePlay.send_remote_play_together_invite =
...
local ISteamNetworkingMessages = {
    version = "SteamNetworkingMessages002",
    version_number = 2
}

index_funcs.ISteamNetworkingMessages = function()
    local this = proc_bind("steamnetworkingsockets.dll",
                           "SteamNetworkingMessages_LibV2",
                           "void*(__thiscall*)(unsigned int, unsigned int)")()

    local SendMessageToUser_native = vtable_entry(this, 0,
                                                  "int(__thiscall*)(void*, const SteamNetworkingIdentity &, const void *, uint32_t, int, int)")
    function ISteamNetworkingMessages.SendMessageToUser(identityRemote, pubData,
                                                        cubData, nSendFlags,
                                                        nRemoteChannel)
        return SendMessageToUser_native(this, identityRemote, pubData, cubData,
                                        nSendFlags, nRemoteChannel)
    end
    ISteamNetworkingMessages.send_message_to_user =
        ISteamNetworkingMessages.SendMessageToUser

    local ReceiveMessagesOnChannel_native =
        vtable_entry(this, 1,
                     "int(__thiscall*)(void*, int, SteamNetworkingMessage_t **, int)")
    function ISteamNetworkingMessages.ReceiveMessagesOnChannel(nLocalChannel,
                                                               ppOutMessages,
                                                               nMaxMessages)
        return ReceiveMessagesOnChannel_native(this, nLocalChannel,
                                               ppOutMessages, nMaxMessages)
    end
    ISteamNetworkingMessages.receive_messages_on_channel =
        ISteamNetworkingMessages.ReceiveMessagesOnChannel

    local AcceptSessionWithUser_native =
        vtable_entry(this, 2,
                     "bool(__thiscall*)(void*, const SteamNetworkingIdentity &)")
    ...
    ISteamNetworkingMessages.accept_session_with_user =
        ISteamNetworkingMessages.AcceptSessionWithUser

    local CloseSessionWithUser_native = vtable_entry(this, 3,
                                                     "bool(__thiscall*)(void*, const SteamNetworkingIdentity &)")
    ...
    ISteamNetworkingMessages.close_session_with_user =
        ISteamNetworkingMessages.CloseSessionWithUser

    local CloseChannelWithUser_native = vtable_entry(this, 4,
                                                     "bool(__thiscall*)(void*, const SteamNetworkingIdentity &, int)")
    function ISteamNetworkingMessages.CloseChannelWithUser(identityRemote,
                                                           nLocalChannel)
        return CloseChannelWithUser_native(this, identityRemote, nLocalChannel)
    end
    ISteamNetworkingMessages.close_channel_with_user =
        ISteamNetworkingMessages.CloseChannelWithUser

    local GetSessionConnectionInfo_native =
        vtable_entry(this, 5,
                     "int(__thiscall*)(void*, const SteamNetworkingIdentity &, SteamNetConnectionInfo_t *, SteamNetworkingQuickConnectionStatus *)")
    function ISteamNetworkingMessages.GetSessionConnectionInfo(identityRemote)
        ...
        local pQuickStatus_out =
            structs.SteamNetworkingQuickConnectionStatus_arr(1)
        local res = GetSessionConnectionInfo_native(this, identityRemote,
                                                    pConnectionInfo_out,
                                                    pQuickStatus_out)

        return res, DEREF_GCSAFE(pConnectionInfo_out),
               DEREF_GCSAFE(pQuickStatus_out)
    end
    ISteamNetworkingMessages.get_session_connection_info =
        ISteamNetworkingMessages.GetSessionConnectionInfo
...
local ISteamNetworkingSockets = {
    version = "SteamNetworkingSockets009",
    version_number = 9
}
...
index_funcs.ISteamNetworkingSockets = function()
    ...
    local CreateListenSocketIP_native = vtable_entry(this, 0,
                                                     "unsigned int(__thiscall*)(void*, const SteamNetworkingIPAddr &, int, const SteamNetworkingConfigValue_t *)")
    function ISteamNetworkingSockets.CreateListenSocketIP(localAddress,
                                                          nOptions, pOptions)
        return CreateListenSocketIP_native(this, localAddress, nOptions,
                                           pOptions)
    end
    ISteamNetworkingSockets.create_listen_socket_ip =
        ISteamNetworkingSockets.CreateListenSocketIP

    local ConnectByIPAddress_native = vtable_entry(this, 1,
                                                   "unsigned int(__thiscall*)(void*, const SteamNetworkingIPAddr &, int, const SteamNetworkingConfigValue_t *)")
    function ISteamNetworkingSockets.ConnectByIPAddress(address, nOptions,
                                                        pOptions)
        return ConnectByIPAddress_native(this, address, nOptions, pOptions)
    end
    ISteamNetworkingSockets.connect_by_ip_address =
        ISteamNetworkingSockets.ConnectByIPAddress

    local CreateListenSocketP2P_native =
        vtable_entry(this, 2,
                     "unsigned int(__thiscall*)(void*, int, int, const SteamNetworkingConfigValue_t *)")
    function ISteamNetworkingSockets.CreateListenSocketP2P(nLocalVirtualPort,
                                                           nOptions, pOptions)
        return CreateListenSocketP2P_native(this, nLocalVirtualPort, nOptions,
                                            pOptions)
    end
    ISteamNetworkingSockets.create_listen_socket_p2p =
        ISteamNetworkingSockets.CreateListenSocketP2P

    local ConnectP2P_native = vtable_entry(this, 3,
                                           "unsigned int(__thiscall*)(void*, const SteamNetworkingIdentity &, int, int, const SteamNetworkingConfigValue_t *)")
    function ISteamNetworkingSockets.ConnectP2P(identityRemote,
                                                nRemoteVirtualPort, nOptions,
                                                pOptions)
        return ConnectP2P_native(this, identityRemote, nRemoteVirtualPort,
                                 nOptions, pOptions)
    end
    ISteamNetworkingSockets.connect_p2p = ISteamNetworkingSockets.ConnectP2P

    local AcceptConnection_native = vtable_entry(this, 4,
                                                 "int(__thiscall*)(void*, unsigned int)")
    function ISteamNetworkingSockets.AcceptConnection(hConn)
        return AcceptConnection_native(this, hConn)
    end
    ISteamNetworkingSockets.accept_connection =
        ISteamNetworkingSockets.AcceptConnection

    local CloseConnection_native = vtable_entry(this, 5,
                                                "bool(__thiscall*)(void*, unsigned int, int, const char *, bool)")
    function ISteamNetworkingSockets.CloseConnection(hPeer, nReason, pszDebug,
                                                     bEnableLinger)
        return CloseConnection_native(this, hPeer, nReason, pszDebug,
                                      bEnableLinger)
    end
    ISteamNetworkingSockets.close_connection =
        ISteamNetworkingSockets.CloseConnection

    local CloseListenSocket_native = vtable_entry(this, 6,
                                                  "bool(__thiscall*)(void*, unsigned int)")
    function ISteamNetworkingSockets.CloseListenSocket(hSocket)
        return CloseListenSocket_native(this, hSocket)
    end
    ISteamNetworkingSockets.close_listen_socket =
        ISteamNetworkingSockets.CloseListenSocket

    local SetConnectionUserData_native =
        vtable_entry(this, 7, "bool(__thiscall*)(void*, unsigned int, int64_t)")
    ...
    ISteamNetworkingSockets.set_connection_user_data =
        ISteamNetworkingSockets.SetConnectionUserData

    local GetConnectionUserData_native =
        vtable_entry(this, 8, "int64_t(__thiscall*)(void*, unsigned int)")
    function ISteamNetworkingSockets.GetConnectionUserData(hPeer)
        return GetConnectionUserData_native(this, hPeer)
    end
    ISteamNetworkingSockets.get_connection_user_data =
        ISteamNetworkingSockets.GetConnectionUserData

    local SetConnectionName_native = vtable_entry(this, 9,
                                                  "void(__thiscall*)(void*, unsigned int, const char *)")
    function ISteamNetworkingSockets.SetConnectionName(hPeer, pszName)
        return SetConnectionName_native(this, hPeer, pszName)
    end
    ISteamNetworkingSockets.set_connection_name =
        ISteamNetworkingSockets.SetConnectionName

    local GetConnectionName_native = vtable_entry(this, 10,
                                                  "bool(__thiscall*)(void*, unsigned int, char *, int)")
    ...
    ISteamNetworkingSockets.get_connection_name =
        ISteamNetworkingSockets.GetConnectionName

    local SendMessageToConnection_native =
        vtable_entry(this, 11,
                     "int(__thiscall*)(void*, unsigned int, const void *, uint32_t, int, int64_t *)")
    function ISteamNetworkingSockets.SendMessageToConnection(hConn, pData,
                                                             cbData, nSendFlags)
        local pOutMessageNumber_out = new_int64_arr()
        local res = SendMessageToConnection_native(this, hConn, pData, cbData,
                                                   nSendFlags,
                                                   pOutMessageNumber_out)

        return res, DEREF_GCSAFE(pOutMessageNumber_out)
    end
    ISteamNetworkingSockets.send_message_to_connection =
        ISteamNetworkingSockets.SendMessageToConnection

    local SendMessages_native = vtable_entry(this, 12,
                                             "void(__thiscall*)(void*, int, SteamNetworkingMessage_t *const *, int64_t *)")
    function ISteamNetworkingSockets.SendMessages(nMessages, pMessages)
        local pOutMessageNumberOrResult_out = new_int64_arr()
        SendMessages_native(this, nMessages, pMessages,
                            pOutMessageNumberOrResult_out)
    ...
    local FlushMessagesOnConnection_native =
        vtable_entry(this, 13, "int(__thiscall*)(void*, unsigned int)")
    function ISteamNetworkingSockets.FlushMessagesOnConnection(hConn)
        return FlushMessagesOnConnection_native(this, hConn)
    end
    ISteamNetworkingSockets.flush_messages_on_connection =
        ISteamNetworkingSockets.FlushMessagesOnConnection

    local ReceiveMessagesOnConnection_native =
        vtable_entry(this, 14,
                     "int(__thiscall*)(void*, unsigned int, SteamNetworkingMessage_t **, int)")
    function ISteamNetworkingSockets.ReceiveMessagesOnConnection(hConn,
                                                                 ppOutMessages,
                                                                 nMaxMessages)
        return ReceiveMessagesOnConnection_native(this, hConn, ppOutMessages,
                                                  nMaxMessages)
    end
    ISteamNetworkingSockets.receive_messages_on_connection =
        ISteamNetworkingSockets.ReceiveMessagesOnConnection

    local GetConnectionInfo_native = vtable_entry(this, 15,
                                                  "bool(__thiscall*)(void*, unsigned int, SteamNetConnectionInfo_t *)")
    ...
    ISteamNetworkingSockets.get_connection_info =
        ISteamNetworkingSockets.GetConnectionInfo

    local GetQuickConnectionStatus_native =
        vtable_entry(this, 16,
                     "bool(__thiscall*)(void*, unsigned int, SteamNetworkingQuickConnectionStatus *)")
    ...
    ISteamNetworkingSockets.get_quick_connection_status =
        ISteamNetworkingSockets.GetQuickConnectionStatus

    local GetDetailedConnectionStatus_native =
        vtable_entry(this, 17,
                     "int(__thiscall*)(void*, unsigned int, char *, int)")
    function ISteamNetworkingSockets.GetDetailedConnectionStatus(hConn, pszBuf,
                                                                 cbBuf)
        return GetDetailedConnectionStatus_native(this, hConn, pszBuf, cbBuf)
    end
    ISteamNetworkingSockets.get_detailed_connection_status =
        ISteamNetworkingSockets.GetDetailedConnectionStatus

    local GetListenSocketAddress_native =
        vtable_entry(this, 18,
                     "bool(__thiscall*)(void*, unsigned int, SteamNetworkingIPAddr *)")
    ...
    ISteamNetworkingSockets.get_listen_socket_address =
        ISteamNetworkingSockets.GetListenSocketAddress

    local CreateSocketPair_native = vtable_entry(this, 19,
                                                 "bool(__thiscall*)(void*, unsigned int *, unsigned int *, bool, const SteamNetworkingIdentity *, const SteamNetworkingIdentity *)")
    function ISteamNetworkingSockets.CreateSocketPair(bUseNetworkLoopback,
                                                      pIdentity1, pIdentity2)
        local pOutConnection1_out = new_unsigned_int_arr()
        local pOutConnection2_out = new_unsigned_int_arr()
        local res = CreateSocketPair_native(this, pOutConnection1_out,
                                            pOutConnection2_out,
                                            bUseNetworkLoopback, pIdentity1,
                                            pIdentity2)

        return res, DEREF_GCSAFE(pOutConnection1_out),
               DEREF_GCSAFE(pOutConnection2_out)
    end
    ISteamNetworkingSockets.create_socket_pair =
        ISteamNetworkingSockets.CreateSocketPair

    local GetIdentity_native = vtable_entry(this, 20,
                                            "bool(__thiscall*)(void*, SteamNetworkingIdentity *)")
    ...
    local InitAuthentication_native = vtable_entry(this, 21,
                                                   "int(__thiscall*)(void*)")
    function ISteamNetworkingSockets.InitAuthentication()
        return InitAuthentication_native(this)
    end
    ISteamNetworkingSockets.init_authentication =
        ISteamNetworkingSockets.InitAuthentication

    local GetAuthenticationStatus_native =
        vtable_entry(this, 22,
                     "int(__thiscall*)(void*, SteamNetAuthenticationStatus_t *)")
    ...
    ISteamNetworkingSockets.set_connection_poll_group =
        ISteamNetworkingSockets.SetConnectionPollGroup

    local ReceiveMessagesOnPollGroup_native =
        vtable_entry(this, 26,
                     "int(__thiscall*)(void*, unsigned int, SteamNetworkingMessage_t **, int)")
    function ISteamNetworkingSockets.ReceiveMessagesOnPollGroup(hPollGroup,
                                                                ppOutMessages,
                                                                nMaxMessages)
        return ReceiveMessagesOnPollGroup_native(this, hPollGroup,
                                                 ppOutMessages, nMaxMessages)
    end
    ISteamNetworkingSockets.receive_messages_on_poll_group =
        ISteamNetworkingSockets.ReceiveMessagesOnPollGroup

    local ReceivedRelayAuthTicket_native =
        vtable_entry(this, 27,
                     "bool(__thiscall*)(void*, const void *, int, SteamDatagramRelayAuthTicket *)")
    function ISteamNetworkingSockets.ReceivedRelayAuthTicket(pvTicket, cbTicket,
                                                             pOutParsedTicket)
        return ReceivedRelayAuthTicket_native(this, pvTicket, cbTicket,
                                              pOutParsedTicket)
    end
    ISteamNetworkingSockets.received_relay_auth_ticket =
        ISteamNetworkingSockets.ReceivedRelayAuthTicket

    local FindRelayAuthTicketForServer_native =
        vtable_entry(this, 28,
                     "int(__thiscall*)(void*, const SteamNetworkingIdentity &, int, SteamDatagramRelayAuthTicket *)")
    function ISteamNetworkingSockets.FindRelayAuthTicketForServer(
        identityGameServer, nRemoteVirtualPort, pOutParsedTicket)
        return FindRelayAuthTicketForServer_native(this, identityGameServer,
                                                   nRemoteVirtualPort,
                                                   pOutParsedTicket)
    end
    ISteamNetworkingSockets.find_relay_auth_ticket_for_server =
        ISteamNetworkingSockets.FindRelayAuthTicketForServer

    local ConnectToHostedDedicatedServer_native =
        vtable_entry(this, 29,
                     "unsigned int(__thiscall*)(void*, const SteamNetworkingIdentity &, int, int, const SteamNetworkingConfigValue_t *)")
    function ISteamNetworkingSockets.ConnectToHostedDedicatedServer(
        identityTarget, nRemoteVirtualPort, nOptions, pOptions)
        return ConnectToHostedDedicatedServer_native(this, identityTarget,
                                                     nRemoteVirtualPort,
                                                     nOptions, pOptions)
    end
    ISteamNetworkingSockets.connect_to_hosted_dedicated_server =
        ISteamNetworkingSockets.ConnectToHostedDedicatedServer

    local GetHostedDedicatedServerPort_native =
        vtable_entry(this, 30, "uint16_t(__thiscall*)(void*)")
    function ISteamNetworkingSockets.GetHostedDedicatedServerPort()
        return GetHostedDedicatedServerPort_native(this)
    end
    ISteamNetworkingSockets.get_hosted_dedicated_server_port =
        ISteamNetworkingSockets.GetHostedDedicatedServerPort

    local GetHostedDedicatedServerPOPID_native =
        vtable_entry(this, 31, "unsigned int(__thiscall*)(void*)")
    function ISteamNetworkingSockets.GetHostedDedicatedServerPOPID()
        return GetHostedDedicatedServerPOPID_native(this)
    end
    ISteamNetworkingSockets.get_hosted_dedicated_server_pop_id =
        ISteamNetworkingSockets.GetHostedDedicatedServerPOPID

    local GetHostedDedicatedServerAddress_native =
        vtable_entry(this, 32,
                     "int(__thiscall*)(void*, SteamDatagramHostedAddress *)")
    ...
    ISteamNetworkingSockets.get_hosted_dedicated_server_address =
        ISteamNetworkingSockets.GetHostedDedicatedServerAddress

    local CreateHostedDedicatedServerListenSocket_native = vtable_entry(this,
                                                                        33,
                                                                        "unsigned int(__thiscall*)(void*, int, int, const SteamNetworkingConfigValue_t *)")
    function ISteamNetworkingSockets.CreateHostedDedicatedServerListenSocket(
        nLocalVirtualPort, nOptions, pOptions)
        return CreateHostedDedicatedServerListenSocket_native(this,
                                                              nLocalVirtualPort,
                                                              nOptions, pOptions)
    end
    ISteamNetworkingSockets.create_hosted_dedicated_server_listen_socket =
        ISteamNetworkingSockets.CreateHostedDedicatedServerListenSocket

    local GetGameCoordinatorServerLogin_native =
        vtable_entry(this, 34,
                     "int(__thiscall*)(void*, SteamDatagramGameCoordinatorServerLogin *, int *, void *)")
    function ISteamNetworkingSockets.GetGameCoordinatorServerLogin(pLoginInfo,
                                                                   pcbSignedBlob,
                                                                   pBlob)
        return GetGameCoordinatorServerLogin_native(this, pLoginInfo,
                                                    pcbSignedBlob, pBlob)
    end
    ISteamNetworkingSockets.get_game_coordinator_server_login =
        ISteamNetworkingSockets.GetGameCoordinatorServerLogin

    local ConnectP2PCustomSignaling_native =
        vtable_entry(this, 35,
                     "unsigned int(__thiscall*)(void*, void* *, const SteamNetworkingIdentity *, int, int, const SteamNetworkingConfigValue_t *)")
    function ISteamNetworkingSockets.ConnectP2PCustomSignaling(pSignaling,
                                                               pPeerIdentity,
                                                               nRemoteVirtualPort,
                                                               nOptions,
                                                               pOptions)
        return ConnectP2PCustomSignaling_native(this, pSignaling, pPeerIdentity,
                                                nRemoteVirtualPort, nOptions,
                                                pOptions)
    end
    ISteamNetworkingSockets.connect_p2p_custom_signaling =
        ISteamNetworkingSockets.ConnectP2PCustomSignaling

    local ReceivedP2PCustomSignal_native =
        vtable_entry(this, 36,
                     "bool(__thiscall*)(void*, const void *, int, void* *)")
    function ISteamNetworkingSockets.ReceivedP2PCustomSignal(pMsg, cbMsg,
                                                             pContext)
        return ReceivedP2PCustomSignal_native(this, pMsg, cbMsg, pContext)
    end
    ISteamNetworkingSockets.received_p2p_custom_signal =
        ISteamNetworkingSockets.ReceivedP2PCustomSignal

    local GetCertificateRequest_native =
        vtable_entry(this, 37,
                     "bool(__thiscall*)(void*, int *, void *, char * &)")
    function ISteamNetworkingSockets.GetCertificateRequest(pcbBlob, pBlob,
                                                           errMsg)
        return GetCertificateRequest_native(this, pcbBlob, pBlob, errMsg)
    end
    ISteamNetworkingSockets.get_certificate_request =
        ISteamNetworkingSockets.GetCertificateRequest

    local SetCertificate_native = vtable_entry(this, 38,
                                               "bool(__thiscall*)(void*, const void *, int, char * &)")
    function ISteamNetworkingSockets.SetCertificate(pCertificate, cbCertificate,
                                                    errMsg)
        return SetCertificate_native(this, pCertificate, cbCertificate, errMsg)
    end
    ISteamNetworkingSockets.set_certificate =
        ISteamNetworkingSockets.SetCertificate

    local RunCallbacks_native = vtable_entry(this, 39,
                                             "void(__thiscall*)(void*)")
...
local ISteamNetworkingUtils = {
    version = "SteamNetworkingUtils003",
    version_number = 3
}

index_funcs.ISteamNetworkingUtils = function()
    local this = proc_bind("steamnetworkingsockets.dll",
                           "SteamNetworkingUtils_LibV3",
                           "void*(__thiscall*)(unsigned int, unsigned int)")()

    local AllocateMessage_native = vtable_entry(this, 0,
                                                "SteamNetworkingMessage_t *(__thiscall*)(void*, int)")
    function ISteamNetworkingUtils.AllocateMessage(cbAllocateBuffer)
        return AllocateMessage_native(this, cbAllocateBuffer)
    end
    ISteamNetworkingUtils.allocate_message =
        ISteamNetworkingUtils.AllocateMessage

    local GetRelayNetworkStatus_native =
        vtable_entry(this, 1,
                     "int(__thiscall*)(void*, SteamRelayNetworkStatus_t *)")
    ...
    ISteamNetworkingUtils.get_relay_network_status =
        ISteamNetworkingUtils.GetRelayNetworkStatus

    local GetLocalPingLocation_native = vtable_entry(this, 2,
                                                     "float(__thiscall*)(void*, SteamNetworkPingLocation_t &)")
    function ISteamNetworkingUtils.GetLocalPingLocation(result)
        return GetLocalPingLocation_native(this, result)
    end
    ISteamNetworkingUtils.get_local_ping_location =
        ISteamNetworkingUtils.GetLocalPingLocation

    local EstimatePingTimeBetweenTwoLocations_native =
        vtable_entry(this, 3,
                     "int(__thiscall*)(void*, const SteamNetworkPingLocation_t &, const SteamNetworkPingLocation_t &)")
    function ISteamNetworkingUtils.EstimatePingTimeBetweenTwoLocations(
        location1, location2)
        return EstimatePingTimeBetweenTwoLocations_native(this, location1,
                                                          location2)
    end
    ISteamNetworkingUtils.estimate_ping_time_between_two_locations =
        ISteamNetworkingUtils.EstimatePingTimeBetweenTwoLocations

    local EstimatePingTimeFromLocalHost_native =
        vtable_entry(this, 4,
                     "int(__thiscall*)(void*, const SteamNetworkPingLocation_t &)")
    ...
    ISteamNetworkingUtils.estimate_ping_time_from_local_host =
        ISteamNetworkingUtils.EstimatePingTimeFromLocalHost

    local ConvertPingLocationToString_native =
        vtable_entry(this, 5,
                     "void(__thiscall*)(void*, const SteamNetworkPingLocation_t &, char *, int)")
    function ISteamNetworkingUtils.ConvertPingLocationToString(location, pszBuf,
                                                               cchBufSize)
        return ConvertPingLocationToString_native(this, location, pszBuf,
                                                  cchBufSize)
    end
    ISteamNetworkingUtils.convert_ping_location_to_string =
        ISteamNetworkingUtils.ConvertPingLocationToString

    local ParsePingLocationString_native =
        vtable_entry(this, 6,
                     "bool(__thiscall*)(void*, const char *, SteamNetworkPingLocation_t &)")
    ...
    ISteamNetworkingUtils.parse_ping_location_string =
        ISteamNetworkingUtils.ParsePingLocationString

    local CheckPingDataUpToDate_native =
        vtable_entry(this, 7, "bool(__thiscall*)(void*, float)")
    ...
    ISteamNetworkingUtils.check_ping_data_up_to_date =
        ISteamNetworkingUtils.CheckPingDataUpToDate

    local GetPingToDataCenter_native = vtable_entry(this, 8,
                                                    "int(__thiscall*)(void*, unsigned int, unsigned int *)")
    ...
    ISteamNetworkingUtils.get_ping_to_data_center =
        ISteamNetworkingUtils.GetPingToDataCenter

    local GetDirectPingToPOP_native = vtable_entry(this, 9,
                                                   "int(__thiscall*)(void*, unsigned int)")
    function ISteamNetworkingUtils.GetDirectPingToPOP(popID)
        return GetDirectPingToPOP_native(this, popID)
    end
    ISteamNetworkingUtils.get_direct_ping_to_pop =
    ...
    local GetPOPList_native = vtable_entry(this, 11,
                                           "int(__thiscall*)(void*, unsigned int *, int)")
    ...
    local GetLocalTimestamp_native = vtable_entry(this, 12,
                                                  "long long(__thiscall*)(void*)")
    function ISteamNetworkingUtils.GetLocalTimestamp()
        return GetLocalTimestamp_native(this)
    end
    ISteamNetworkingUtils.get_local_timestamp =
        ISteamNetworkingUtils.GetLocalTimestamp

    local SetDebugOutputFunction_native =
        vtable_entry(this, 13,
                     "void(__thiscall*)(void*, int, void (__thiscall*)(int, const char *))")
    function ISteamNetworkingUtils.SetDebugOutputFunction(eDetailLevel, pfnFunc)
        eDetailLevel = to_enum_required(eDetailLevel,
                                        enums.ESteamNetworkingSocketsDebugOutputType,
                                        "detail_level is required")
        return SetDebugOutputFunction_native(this, eDetailLevel, pfnFunc)
    end
    ISteamNetworkingUtils.set_debug_output_function =
        ISteamNetworkingUtils.SetDebugOutputFunction

    local SetConfigValue_native = vtable_entry(this, 14,
                                               "bool(__thiscall*)(void*, int, int, intptr_t, int, const void *)")
    function ISteamNetworkingUtils.SetConfigValue(eValue, eScopeType, scopeObj,
                                                  eDataType, pArg)
        eValue = to_enum_required(eValue, enums.ESteamNetworkingConfigValue,
                                  "value is required")
        eScopeType = to_enum_required(eScopeType,
                                      enums.ESteamNetworkingConfigScope,
                                      "scope_type is required")
        eDataType = to_enum_required(eDataType,
                                     enums.ESteamNetworkingConfigDataType,
                                     "data_type is required")
        return SetConfigValue_native(this, eValue, eScopeType, scopeObj,
                                     eDataType, pArg)
    end
    ISteamNetworkingUtils.set_config_value =
        ISteamNetworkingUtils.SetConfigValue

    local GetConfigValue_native = vtable_entry(this, 15,
                                               "int(__thiscall*)(void*, int, int, intptr_t, int *, void *, size_t *)")
    function ISteamNetworkingUtils.GetConfigValue(eValue, eScopeType, scopeObj,
                                                  pResult, cbResult)
        eValue = to_enum_required(eValue, enums.ESteamNetworkingConfigValue,
                                  "value is required")
        eScopeType = to_enum_required(eScopeType,
        ...
        local res = GetConfigValue_native(this, eValue, eScopeType, scopeObj,
                                          pOutDataType_out, pResult, cbResult)

        return res, DEREF_GCSAFE(pOutDataType_out)
    end
    ISteamNetworkingUtils.get_config_value =
        ISteamNetworkingUtils.GetConfigValue

    local GetConfigValueInfo_native = vtable_entry(this, 16,
                                                   "bool(__thiscall*)(void*, int, const char **, int *, int *, int *)")
    function ISteamNetworkingUtils.GetConfigValueInfo(eValue, pOutName)
        ...
        local res = GetConfigValueInfo_native(this, eValue, pOutName,
                                              pOutDataType_out, pOutScope_out,
                                              pOutNextValue_out)

        return res, DEREF_GCSAFE(pOutDataType_out), DEREF_GCSAFE(pOutScope_out),
               DEREF_GCSAFE(pOutNextValue_out)
    end
    ISteamNetworkingUtils.get_config_value_info =
        ISteamNetworkingUtils.GetConfigValueInfo

    local GetFirstConfigValue_native = vtable_entry(this, 17,
                                                    "int(__thiscall*)(void*)")
    function ISteamNetworkingUtils.GetFirstConfigValue()
        return GetFirstConfigValue_native(this)
    end
    ISteamNetworkingUtils.get_first_config_value =
        ISteamNetworkingUtils.GetFirstConfigValue

    local SteamNetworkingIPAddr_ToString_native =
        vtable_entry(this, 18,
                     "void(__thiscall*)(void*, const SteamNetworkingIPAddr &, char *, uint32_t, bool)")
    function ISteamNetworkingUtils.SteamNetworkingIPAddr_ToString(addr, buf,
                                                                  cbBuf,
                                                                  bWithPort)
        return SteamNetworkingIPAddr_ToString_native(this, addr, buf, cbBuf,
                                                     bWithPort)
    end
    ISteamNetworkingUtils.steam_networking_ip_addr_to_string =
        ISteamNetworkingUtils.SteamNetworkingIPAddr_ToString

    local SteamNetworkingIPAddr_ParseString_native =
        vtable_entry(this, 19,
                     "bool(__thiscall*)(void*, SteamNetworkingIPAddr *, const char *)")
    function ISteamNetworkingUtils.SteamNetworkingIPAddr_ParseString(pAddr,
                                                                     pszStr)
        return SteamNetworkingIPAddr_ParseString_native(this, pAddr, pszStr)
    end
    ISteamNetworkingUtils.steam_networking_ip_addr_parse_string =
        ISteamNetworkingUtils.SteamNetworkingIPAddr_ParseString

    local SteamNetworkingIdentity_ToString_native =
        vtable_entry(this, 20,
                     "void(__thiscall*)(void*, const SteamNetworkingIdentity &, char *, uint32_t)")
    function ISteamNetworkingUtils.SteamNetworkingIdentity_ToString(identity,
                                                                    buf, cbBuf)
        return SteamNetworkingIdentity_ToString_native(this, identity, buf,
                                                       cbBuf)
    end
    ISteamNetworkingUtils.steam_networking_identity_to_string =
        ISteamNetworkingUtils.SteamNetworkingIdentity_ToString

    local SteamNetworkingIdentity_ParseString_native =
        vtable_entry(this, 21,
                     "bool(__thiscall*)(void*, SteamNetworkingIdentity *, const char *)")
    function ISteamNetworkingUtils.SteamNetworkingIdentity_ParseString(
        pIdentity, pszStr)
        return SteamNetworkingIdentity_ParseString_native(this, pIdentity,
                                                          pszStr)
    end
    ISteamNetworkingUtils.steam_networking_identity_parse_string =
...
local ISteamGameServerStats = {
    version = "SteamGameServerStats001",
    version_number = 1
}

index_funcs.ISteamGameServerStats = function()
    local this = get_steam_vtable_bind("steam_api.dll", "SteamClient", 14,
                             "void*(__thiscall*)(void*, int, int, const char *)")(
                     hSteamUser, hSteamPipe, "SteamGameServerStats001")

    local RequestUserStats_native = vtable_entry(this, 0,
                                                 "uint64_t(__thiscall*)(void*, SteamID)")
    local RequestUserStats_info = {
        struct = typeof([[
			struct {
    ...
    function ISteamGameServerStats.RequestUserStats(steamIDUser, callback)
        ...
        if callback ~= nil then
            res = register_call_result(res, callback, 1800,
                                       RequestUserStats_info)
        end

        return res
    end
    ISteamGameServerStats.request_user_stats =
        ISteamGameServerStats.RequestUserStats

    local GetUserStatInt32_native = vtable_entry(this, 1,
                                                 "bool(__thiscall*)(void*, SteamID, const char *, int32_t *)")
    ...
    ISteamGameServerStats.get_user_stat_int32 =
        ISteamGameServerStats.GetUserStatInt32

    local GetUserStatFloat_native = vtable_entry(this, 2,
                                                 "bool(__thiscall*)(void*, SteamID, const char *, float *)")
    ...
    ISteamGameServerStats.get_user_stat_float =
        ISteamGameServerStats.GetUserStatFloat

    local GetUserAchievement_native = vtable_entry(this, 3,
                                                   "bool(__thiscall*)(void*, SteamID, const char *, bool *)")
    ...
    ISteamGameServerStats.get_user_achievement =
        ISteamGameServerStats.GetUserAchievement

    local SetUserStatInt32_native = vtable_entry(this, 4,
                                                 "bool(__thiscall*)(void*, SteamID, const char *, int32_t)")
    ...
    ISteamGameServerStats.set_user_stat_int32 =
        ISteamGameServerStats.SetUserStatInt32

    local SetUserStatFloat_native = vtable_entry(this, 5,
                                                 "bool(__thiscall*)(void*, SteamID, const char *, float)")
    ...
    ISteamGameServerStats.set_user_stat_float =
        ISteamGameServerStats.SetUserStatFloat

    local UpdateUserAvgRateStat_native =
        vtable_entry(this, 6,
                     "bool(__thiscall*)(void*, SteamID, const char *, float, double)")
    function ISteamGameServerStats.UpdateUserAvgRateStat(steamIDUser, pchName,
                                                         flCountThisSession,
                                                         dSessionLength)
        steamIDUser = to_steamid_required(steamIDUser,
                                          "steamid_user is required")
        return UpdateUserAvgRateStat_native(this, steamIDUser, pchName,
                                            flCountThisSession, dSessionLength)
    end
    ISteamGameServerStats.update_user_avg_rate_stat =
        ISteamGameServerStats.UpdateUserAvgRateStat

    local SetUserAchievement_native = vtable_entry(this, 7,
                                                   "bool(__thiscall*)(void*, SteamID, const char *)")
    ...
    ISteamGameServerStats.set_user_achievement =
        ISteamGameServerStats.SetUserAchievement

    local ClearUserAchievement_native = vtable_entry(this, 8,
                                                     "bool(__thiscall*)(void*, SteamID, const char *)")
    ...
    ISteamGameServerStats.clear_user_achievement =
        ISteamGameServerStats.ClearUserAchievement

    local StoreUserStats_native = vtable_entry(this, 9,
                                               "uint64_t(__thiscall*)(void*, SteamID)")
    ...
    ISteamGameServerStats.store_user_stats =
...
index_funcs_extra.ISteamFriends = function()
    ...
    --
    local steamfriends002 = get_steam_vtable_bind("steam_api.dll", "SteamClient", 8,
                                        "void*(__thiscall*)(void*, int, int, const char *)")(
                                hSteamUser, hSteamPipe, "SteamFriends002")

    local SetPersonaState_native = vtable_entry(steamfriends002, 3,
                                                "void(__thiscall*)(void*, int)")
    M.ISteamFriends.SetPersonaState = function(ePersonaState)
        ePersonaState = to_enum_required(ePersonaState, enums.EPersonaState,
                                         "ePersonaState is required")
    ...
    local SendMsgToFriend_native = vtable_entry(steamfriends002, 19,
                                                "bool(__thiscall*)(void*, SteamID, int, const char*, int)")
    M.ISteamFriends.SendMsgToFriend = function(steamIDFriend, eFriendMsgType,
                                               pvMsgBody)
        steamIDFriend = to_steamid_required(steamIDFriend,
                                            "steamIDFriend is required")
        eFriendMsgType = to_enum_required(eFriendMsgType, enums.EChatEntryType,
                                          "eFriendMsgType is required")
        local cubMsgBody = string_len(pvMsgBody)
        return SendMsgToFriend_native(steamfriends002, steamIDFriend,
                                      eFriendMsgType, pvMsgBody, cubMsgBody)
    end
...
index_funcs_extra.ISteamMatchmaking = function()
    local matchframework001 = client.create_interface("matchmaking.dll",
                                                      "MATCHFRAMEWORK_001")

    local GetMatchSession_native = vtable_entry(matchframework001, 13,
                                                "void*(__thiscall*)(void*)")
...
index_funcs_extra.ISteamNetworkingUtils = function()
    local GetNetworkConfigURL_native = proc_bind("steamnetworkingsockets.dll",
                                                 "SteamDatagram_GetNetworkConfigURL",
                                                 "char *(__thiscall*)(unsigned int, unsigned int)")
